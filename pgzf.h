...
static inline u4i _zlib_raw_deflate_all(u1i *dst, u4i dlen, u1i *src, u4i slen,
                                        int level) {
    z_stream Z, *z;
    u4i ret;
    z = &Z;
...
static inline u4i _pgzf_deflate(u1v *dst, u1v *src, int level) {
    ...
    z_size = _zlib_raw_deflate_all(dst->buffer + PGZF_HEAD_SIZE, z_size, src->buffer,
                                   src->size, level);
...
static inline int _read_pgzf_header(FILE *in, u1v *src, u4i *hoff, u4i *zsval,
                                    u8i *zxval) {
    u4i off, val, sl, end;
    ...
    *zxval = 0;
    ...
    if((src->buffer[off + 2] != 0x08) || (src->buffer[off + 2] & 0xE0))
...
if(pgz->task == PGZF_TASK_DEFLATE) {
    ...
    if(pz->rw_mode == PGZF_MODE_R) {
        if(pgz->src->size) {    // loaded header, had set zsval and zxval
            ...
            ret = _read_pgzf_header(pz->file, pgz->src, &pgz->soff, &pgz->zsval,
                                    &pgz->zxval);
            if(pgz->src->size == 0) {
                pz->eof = 1;
                pz->ridx++;
            ...
            if(ret != PGZF_FILETYPE_PGZF) {
                fprintf(stderr,
                        " -- Error: not a PGZF format at %u block, in %s -- %s:%d --\n",
                        pz->ridx, __FUNCTION__, __FILE__, __LINE__);
                fflush(stderr);
                pz->error = 1;
                pz->ridx++;
                break;
        ...
        pgz->soff = 0;
        if(pgzf_inflate_core(pgz->dst->buffer, &dsz, pgz->src->buffer + hsize,
                             pgz->zsval - hsize, 1) == 0) {
            clear_u1v(pgz->src);
            break;
        }
        ...
        while(!pz->error) {
            ...
            if(pgz->src->size < bsz) {
                ...
                rsize = fread(pgz->src->buffer + pgz->src->size, 1, bsz - pgz->src->size,
                              pz->file);
            ...
            if(pz->step == 0) {
                ...
                if(ret != PGZF_FILETYPE_GZ && ret != PGZF_FILETYPE_PGZF) {
                    if(pgz->src->size == pgz->soff) {
                        ...
                        fprintf(stderr,
                                " -- failed in read gzip header, ret = %d in %s -- %s:%d "
                                "--\n",
                ...
                if(pgz->src->size >= pgz->soff + PGZF_TAIL_SIZE) {
                    ...
                    memmove(pgz->src->buffer, pgz->src->buffer + pgz->soff,
            ...
            while(pgz->dst->size < bufsize && pgz->soff < pgz->src->size) {
                ...
                ret =
                    pgzf_inflate_raw_core(pz->z, pgz->dst->buffer + pgz->dst->size, &dsz,
                                          pgz->src->buffer + pgz->soff, &ssz, Z_NO_FLUSH);
                pgz->dst->size += dsz;
                pgz->soff += ssz;
                if(ret == Z_STREAM_END) {
                    pz->step = 2;
            ...
            if(pgz->dst->size == bufsize) {
                if(pgz->soff < pgz->src->size) {
                    if(pz->ncpu > 1) {
                        ...
                        append_array_u1v(pz->srcs[next], pgz->src->buffer + pgz->soff,
                                         pgz->src->size - pgz->soff);
                    }
                }
                pgz->soff = pgz->src->size = 0;
        ...
        rsize = fread(pgz->dst->buffer + pgz->dst->size, 1, bufsize - pgz->dst->size,
                      pz->file);
...
static inline PGZF *open_pgzf_writer(FILE *out, u4i buffer_size, int ncpu, int level) {
    ...
    pz->tot_out = 0;
    if(level == 0)
        level = Z_DEFAULT_COMPRESSION;    // disable level 0, set to default level 6
...
static inline int write_index_pgzf(PGZF *pz) {
    ...
    for(i = 64, x = 1; i + PGZF_INDEX_BIN <= pz->boffs->size; i += PGZF_INDEX_BIN, x++) {
        ...
        if(fseek(pz->file, pz->offset + pz->boffs->buffer[x] + PGZF_HEAD_ZX_OFFSET,
                 SEEK_SET) == -1) {
            perror("fseek error in write_index_pgzf");
            return 0;
        }
    ...
    if(pz->rw_mode == PGZF_MODE_R) {
        ...
        if(pz->seekable) {
            ...
            if(fseek(pz->file, pz->offset + zsval - 4, SEEK_SET) == -1) {
                fprintf(stderr,
                        " ** ERROR: failed to read uncompress block size in the first "
                        "block ERR(1) **\n");
                return NULL;
            }
...
static inline size_t read_pgzf4filereader(void *obj, void *dat, size_t len) {
    return read_pgzf((PGZF *)obj, dat, len);
}
...
static inline size_t write_pgzf4filewriter(void *obj, void *dat, size_t len) {
    return write_pgzf((PGZF *)obj, dat, len);
}
