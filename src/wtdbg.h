...
static inline u8i proc_alignments_core(Graph *g, int ncpu, int raw, rdregv *regs,
    ...
    for (ii = 0; ii < in; ii++) {
        ...
        }
        {
            ...
            kbmmapv *hits;
            BitsVec *cigars = (BitsVec *) malloc(sizeof(BitsVec));
                ...
                if (rid < qe) {
                    ...
                    if ((rid - qb) % world_size == 0) {
                        ...
                        if (last_mission.closed == 0) {
                            size_t size;
                            MPI_Recv(&size, 1, MPI_INT32_T, idx, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                            ...
                            cigars->bits = (uint8_t *) realloc(cigars->bits, (cigars->cap * cigars->n_bit + 15) / 8);
...
//    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    {
        ...
        while (1) {
            thread_wait_one(mdbg);
            MPI_Recv(&mission, sizeof(mission), MPI_BYTE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            if (mission.closed == 1) {
