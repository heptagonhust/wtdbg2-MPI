...
typedef struct {
    uint64_t idx : 46, cnt : 18, fix : 1, rank : 45, score : 18;
} rnk_ref_t;
...
#define edge_hashequals(E1, E2)                  \
    (EDGEHASH(E1).node1 == EDGEHASH(E2).node1 && \
     EDGEHASH(E1).node2 == EDGEHASH(E2).node2 && \
...
typedef struct {
    ...
    u4i n_fix,
            only_fix;    // first n sequences are accurate contigs; only_fix means whether to include other pacbio sequenes
...
static const char *colors[2][2] = {{"blue", "green"},
                                   {"red",  "gray"}};
...
static inline int hit2rdregs_graph(Graph *g, rdregv *regs, int qlen, kbm_map_t *hit,
                                   BitsVec *cigars, u4v *maps[3]) {
    KBM *kbm;
    u8i ndoff;
    ...
    }
    {
        ...
        if(x + 1 + hit->qb != hit->qe || y + 1 + hit->tb != hit->te) {
            fprintf(stderr, " -- something wrong in %s -- %s:%d --\n", __FUNCTION__,
                    __FILE__, __LINE__);
            fflush(stderr);
            print_hit_kbm(g->kbm, g->kbm->reads->buffer[hit->qidx].tag,
                          g->kbm->reads->buffer[hit->qidx].rdlen, hit, cigars, NULL,
                          stderr);
            abort();
        }
#endif
...
#endif
                push_rdregv(regs, (rd_reg_t) {ndoff, hit->tidx, hit->qdir, beg, end,
                                              mask | closed});
            }
            if (hit->qdir) {
                ndoff--;
...
#endif
                push_rdregv(regs, (rd_reg_t) {ndoff, hit->qidx, hit->qdir, beg, end,
                                              mask | closed});
#if 0
...
static inline void clip_read_algo(int clps[2], rdclpv *brks, rdclpv *chis, int avg_dep,
                                  int min_dep) {
    rd_clp_t *c;
    u4i i;
    int dep, x, y, max, mx, my;
    sort_array(brks->buffer, brks->size, rd_clp_t,
               num_cmpgt((a.pos << 1) | a.dir, (b.pos << 1) | b.dir));
    x = y = 0;
    max = 0;
    dep = 0;
    mx = my = 0;
    ...
    sort_array(chis->buffer, chis->size, rd_clp_t,
               num_cmpgtx(a.pos, b.pos, b.dir, a.dir));
    dep = 0;
    max = 0;
    mx = 0;
...
static inline void clip_read_core(Graph *g, u4i rid, hitlnkv *lnks, rdclpv *brks,
                                  rdclpv *chis) {
    read_t *rd;
    hit_lnk_t *lnk;
    rd_hit_t *hit;
    ...
    thread_beg_loop(mclp) ;
            if (mclp->task == 1) {
                ...
                for (rid = mclp->t_idx; rid < mclp->g->reads->size; rid += mclp->n_cpu) {
                    ...
                    sort_array(lnks->buffer, lnks->size, hit_lnk_t,
                               num_cmpgt(g->rdhits->buffer[b.idx].lnks[0].cnt,
                                         g->rdhits->buffer[a.idx].lnks[0].cnt));
                    lnk = &rd->hits;
                    for (i = 0; i < lnks->size; i++) {
                    ...
                    lnk->flg = 0;
                    {
                        ...
                        for (i = 0; i < lnks->size; i++) {
                            if (lnks->buffer[i].idx != chks->buffer[i].idx ||
                                lnks->buffer[i].flg != chks->buffer[i].flg) {
                                fprintf(stderr, " -- something wrong in %s -- %s:%d --\n",
                                        __FUNCTION__, __FILE__, __LINE__);
                                fflush(stderr);
                                abort();
    ...
    thread_beg_loop(mupd) ;
            ...
            if (1) {
                for (i = 1; i < mupd->nd->cnt; i++) {
                    if (regs->buffer[mupd->nd->idx + i].rid ==
                        regs->buffer[mupd->nd->idx + i - 1].rid) {
...
static inline void mul_update_regs_graph(Graph *g, rdregv *regs, rnkrefv *nds, u4i ncpu,
                                         u8i upds[3]) {
    u8i idx, vt;
    u4i i, j, pass;
    read_t *rd;
    ...
    encap_regv(
            g->regs,
            regs->size + 1);    // make sure next_ref_regv in this function is thread-safe
    ...
    for (idx = 0; idx < nds->size + ncpu; idx++) {
        ...
        if (mupd->nd) {
            ...
            for (j = 0; j < mupd->nd->cnt; j++) {
                ...
                rd = ref_readv(g->reads, hit->rid);
                if (rd->visit >=
                    mupd->vt) {    // previous node had changed the layout of this read, need to check conflict again
                    mupd->ins->buffer[j] =
                            check_read_reg_conflict_core(g, hit, &conflict);
                    if (conflict) {
                        hit->closed = 1;
                    }
                }
            ...
            do {
                ...
                if (mupd->nd->fix) {
                    ...
                    if (pass < g->min_node_cov ||
                        pass < (u4i) (mupd->nd->cnt * (1 - g->node_max_conflict))) {
                        upds[1]++;
                        closed = 1;
                        continue;
                    } else {
...
static inline u8i chainning_hits_core(kbmmapv *hits, BitsVec *cigars, int uniq_hit,
                                      float aln_var) {
    kbm_map_t HIT;
    u8i idx, bst, ite, lst, lsx, nrm;
    u4i state;
    sort_array(hits->buffer, hits->size, kbm_map_t,
               num_cmpgtxx(a.qidx, b.qidx, a.tidx, b.tidx, a.qdir, b.qdir));
    nrm = 0;
    for (idx = lst = lsx = 0; idx <= hits->size; idx++) {
        state = 0;
        if (idx == hits->size) {
            state = 1;
        } else if (hits->buffer[lst].qidx == hits->buffer[idx].qidx &&
                   hits->buffer[lst].tidx == hits->buffer[idx].tidx) {
        ...
        if (state) {
            if (idx > lst + 1) {
                if (simple_chain_all_maps_kbm(hits->buffer + lst, idx - lst, cigars, &HIT,
                                              cigars, aln_var)) {
                    hits->buffer[lst++] = HIT;
                    while (lst < idx) {
            ...
            if (state == 1) {
                ...
                if (uniq_hit && idx > lsx + 1) {
                    ...
                    for (ite = bst + 1; ite < idx; ite++) {
                        ...
                        if ((hits->buffer[ite].aln > hits->buffer[bst].aln) ||
                            (hits->buffer[ite].aln == hits->buffer[bst].aln &&
                             hits->buffer[ite].mat > hits->buffer[bst].mat)) {
                            bst = ite;
                        }
                    }
...
static inline u8i load_alignments_core(Graph *g, FileReader *pws, int raw, rdregv *regs,
                                       u4v *maps[3]) {
    kbmmapv *hits;
    BitsVec *cigars;
    u4v *cgs;
    ...
    while ((ncol = readtable_filereader(pws)) != -1) {
        if ((pws->n_line % 100000) == 0) {
            fprintf(KBM_LOGF, "\r%llu", pws->n_line);
            fflush(KBM_LOGF);
        }
        ...
        if ((cu = get_cuhash(g->kbm->tag2idx, get_col_str(pws, 0))) == NULL) {
            if (nwarn < mwarn) {
                fprintf(stderr,
                        " -- Cannot find read \"%s\" in LINE:%llu in %s -- %s:%d --\n",
                        get_col_str(pws, 0), pws->n_line, __FUNCTION__, __FILE__,
                        __LINE__);
                fflush(stderr);
                nwarn++;
            }
        ...
        if (g->corr_mode) {
            ...
            if (nwarn < mwarn) {
                fprintf(
                        stderr,
                        " -- inconsisitent read length \"%s\" %d != %d in %s -- %s:%d --\n",
                        qtag, qlen, g->kbm->reads->buffer[hit->qidx].rdlen, __FUNCTION__,
                        __FILE__, __LINE__);
                fflush(stderr);
                nwarn++;
            }
        ...
        if ((cu = get_cuhash(g->kbm->tag2idx, get_col_str(pws, 5))) == NULL) {
            if (nwarn < mwarn) {
                fprintf(stderr,
                        " -- Cannot find read \"%s\" in LINE:%llu in %s -- %s:%d --\n",
                        get_col_str(pws, 5), pws->n_line, __FUNCTION__, __FILE__,
                        __LINE__);
                fflush(stderr);
                nwarn++;
            }
        ...
        if (num_diff(hit->qe - hit->qb, hit->te - hit->tb) >
            (int) num_max(g->par->aln_var * hit->aln, 1.0))
            continue;
            ...
            if (cgstr[0] >= '0' && cgstr[0] <= '9') {
                ...
                switch (cgstr[0]) {
                    ...
                    default:
                        fprintf(
                                stderr,
                                " -- Bad cigar '%c' \"%s\" in LINE:%llu in %s -- %s:%d --\n",
                                cgstr[0], get_col_str(pws, 14), pws->n_line, __FUNCTION__,
                                __FILE__, __LINE__);
                        fflush(stderr);
                        exit(1);
                }
...
static inline u8i merge_hits_into_graph(Graph *g, int raw, rdregv *regs, u4v *maps[3], kbm_map_t *hit,
                                        BitVec *rdflags, u8i nhit, u8i i, mdbg_struct *mdbg, kbmmapv *hits,
                                        BitsVec *cigars) {
    for (i = 0; i < hits->size; i++) {
...
static inline u8i proc_alignments_core(Graph *g, int ncpu, int raw, rdregv *regs,
                                       u4v *maps[3], char *prefix, char *dump_kbm) {
    kbm_map_t *hit;
    kbm_read_t *pb;
    BitVec *rdflags;
    BufferedWriter *bw;
    ...
    rdflags = (!g->corr_mode && g->par->skip_contained) ? init_bitvec(g->kbm->reads->size)
                                                        : NULL;
    ...
    for (ii = 0; ii < in; ii++) {
        ...
        transfer_kbm(g->kbm, g->par, g->rpar, &(g->corr_mode), &(g->corr_min), &(g->corr_max), &(g->corr_cov),
                     world_rank);
        {
            ...
            for (rid = qb; rid < qe; rid++) {
                ...
                if (idx == 0) {
                    ...
                    if ((rdflags == NULL || get_bitvec(rdflags, rid) == 0)) {
                        memcpy(&mdbg->reg, &mission, sizeof(reg_t));
                        thread_wake(mdbg);
                    }
                } else {
                    ...
                    if (last_mission.closed == 0) {
                        ...
                        cigars->bits = (uint8_t *) realloc(last_pointer,
            ...
            fprintf(stderr, "MPI distribute done\n");
            for (rid = 0; rid < ncpu; rid++) {
            ...
            fprintf(stderr, "MPI recycle master done\n");
            for (rid = 0; rid < (world_size - 1) * ncpu; rid++) {
                mission = (reg_t) {0, rid, 0, 0, 0, 1, 0};
                int idx = -1, flag = 0;
                MPI_Status stat;
                while (idx < 0) {
...
//    else
    ...
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    {
        ...
        while (1) {
            ...
            fprintf(stderr, "mdbg free now from process %d\n", world_rank);
            memcpy(&last_mission, &mdbg->reg, sizeof(reg_t));
            MPI_Send(&last_mission, sizeof(last_mission), MPI_BYTE, 0, 2, MPI_COMM_WORLD);
            if (last_mission.closed == 0) {
...
//                fprintf(KBM_LOGF, "[%s]slave mdbg[%d] cigar size %d\n", date(), last_mission.rid, mdbg->aux->cigars->size);
            ...
            mdbg->reg.closed = 1;
            MPI_Recv(&mission, sizeof(mission), MPI_BYTE, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &stat);
            if (count == 0 && stat.MPI_TAG == 1) {
                count = 1;
            }
            if (mission.closed == 0) {
                memcpy(&mdbg->reg, &mission, sizeof(reg_t));
...
static inline void build_nodes_graph(Graph *g, u8i maxbp, int ncpu, FileReader *pws,
                                     int rdclip, char *prefix, char *dump_kbm) {
    kbm_map_t *hit, HIT;
    BitVec *rks;
    u4v *maps[3];
    FILE *clplog;
    ...
    if (raw) {
        ...
        {
            fprintf(KBM_LOGF, "[%s] sorting rdhits ... ", date());
            fflush(KBM_LOGF);
        ...
        // clip reads
        if (rdclip) {
            fprintf(KBM_LOGF, "[%s] clipping ... ", date());
            fflush(KBM_LOGF);
            ...
            for (rid = 0; rid < g->reads->size; rid++) {
                ...
                clp += (g->reads->buffer[rid].clps[1] - g->reads->buffer[rid].clps[0]) *
                       KBM_BIN_SIZE;
                fprintf(clplog, "%s\t%d\t%d\t%d\n", g->kbm->reads->buffer[rid].tag,
                        g->kbm->reads->buffer[rid].rdlen,
                        g->reads->buffer[rid].clps[0] * KBM_BIN_SIZE,
                        g->reads->buffer[rid].clps[1] * KBM_BIN_SIZE);
            }
            fclose(clplog);
            fprintf(KBM_LOGF, "%.2f%% bases\n", ((tot - clp) * 100.0) / tot);
            fflush(KBM_LOGF);
        }
        fprintf(KBM_LOGF, "[%s] generating regs ... ", date());
        fflush(KBM_LOGF);
        rd_hit_t *rh;
        u8i cgoff;
        cgoff = 0;
        ...
        for (idx = 0; idx < g->rdhits->size; idx++) {
            ...
            hit2rdregs_graph(g, regs, g->reads->buffer[hit->qidx].corr_bincnt, hit,
                             g->cigars, maps);
        }
        free_bitsvec(g->cigars);
        g->cigars = init_bitsvec(1024, 3);
    ...
    if (!g->corr_mode) {
        ...
        for (idx = 0; idx < g->kbm->bins->size; idx++) {
            if (get_bitvec(rks, idx)) {
                ...
                push_rdregv(regs, (rd_reg_t) {idx, rid, 0, (idx - rd->binoff) * g->reglen,
                                              (idx + 1 - rd->binoff) * g->reglen, 0});
            }
        }
        free_bitvec(rks);
    }
    ...
    nd = NULL;
    fprintf(KBM_LOGF, "[%s] generating intervals ... ", date());
    fflush(KBM_LOGF);
    for (idx = 0; idx < regs->size; idx++) {
        if (0) {
            kbm_read_t *rd1, *rd2;
            ...
            fprintf(stdout, "%s\t%c\t%d\t%d\t%s\t%c\t%d\t%d\n", rd1->tag, '+',
                    (int) ((r->node) - rd1->binoff) * g->reglen,
                    (int) (r->node - rd1->binoff + 1) * g->reglen, rd2->tag, "+-"[r->dir],
                    r->beg, r->end);
        }
    ...
    if (!KBM_LOG)
        fprintf(KBM_LOGF, "[%s] Intervals: kept %llu, discarded %llu\n", date(), upds[0],
                upds[1]);
    print_proc_stat_info(0);
}
...
uint32_t estimate_genome_size(Graph *g, unsigned long long tot_bp, FILE *out) {
    ...
    for (i = g->min_node_cov + 1; i < 256; i++) {
        if (kcnts[i] > pmax) {
            peak = i;
            pmax = kcnts[i];
        } else if (i > avg && kcnts[i] < 0.8 * pmax)
            break;
    }
...
static u8i mask_nodes_by_cov_graph(Graph *g, FILE *out) {
    ...
    for (i = 0; i < g->nodes->size; i++) {
        ...
        if (n->regs.cnt > g->max_node_cov || n->regs.cnt < g->min_node_cov) {
            ...
            ret++;
            if (out)
                fprintf(out, "MASK_COV\tN%llu\t%u\t%u\n", (u8i) i, (u4i) n->regs.cnt,
                        n->cov);
        }
    }
    return ret;
...
static inline void build_edges_graph(Graph *g, int ncpu, FILE *log) {
    ...
    for (rid = 0; rid < g->kbm->reads->size; rid++) {
        ...
        while (idx) {
            ...
            for (i = 0; i < regs->size; i++) {
                ...
                if (exists) {
                    if (g->edges->buffer[*u].cov < WT_MAX_EDGE_COV)
                        g->edges->buffer[*u].cov++;
                } else {
                    *u = g->edges->size;
    ...
    for (idx = 1; idx <= offs->size; idx++) {
        ...
        if (1) {
            g->edges->buffer[offs->buffer[lst].idx].off =
                    offs->buffer[(lst + idx) / 2].off;
            g->edges->buffer[offs->buffer[lst].idx].cov = idx - lst;
        } else {
            g->edges->buffer[offs->buffer[lst].idx].off =
                    estimate_edge_length(offs->buffer + lst, idx - lst, idxs);
        ...
        if (0) {
            ...
            fprintf(stdout, "\n");
            for (m = lst + idxs[0]; m < lst + idxs[1]; m++)
                fprintf(stdout, "%u\t", offs->buffer[m].off);
            fprintf(stdout, "\n");
        }
        lst = idx;
    }
    ...
    for (idx = 1; idx < g->edges->size; idx++) {
        if (g->edges->buffer[idx].cov < g->min_edge_cov) {
            if (g->store_low_cov_edge)
                g->edges->buffer[idx].closed = WT_EDGE_CLOSED_LESS;
            else
                continue;
        }
        if (g->nodes->buffer[g->edges->buffer[idx].node1].closed ||
            g->nodes->buffer[g->edges->buffer[idx].node2].closed) {
    ...
    dog_psort_array(
            ...
            num_cmpgtx(
                    (a.flg
                     ? ((g->edges->buffer[a.idx].node2 << 1) | !g->edges->buffer[a.idx].dir2)
                     : ((g->edges->buffer[a.idx].node1 << 1) | g->edges->buffer[a.idx].dir1)),
                    (b.flg
                     ? ((g->edges->buffer[b.idx].node2 << 1) | !g->edges->buffer[b.idx].dir2)
                     : ((g->edges->buffer[b.idx].node1 << 1) | g->edges->buffer[b.idx].dir1)),
    ...
    for (lst = idx = 1; idx < g->erefs->size; idx++) {
        ...
        if (lst < idx) {
            ...
            for (x = lst; x + 1 < idx; x++) {
                ...
                if (g->edges->buffer[g->erefs->buffer[x].idx].closed ==
                    WT_EDGE_CLOSED_NULL)
                    n->edges[f1.flg].cnt++;
            }
            if (g->edges->buffer[g->erefs->buffer[x].idx].closed == WT_EDGE_CLOSED_NULL)
                n->edges[f1.flg].cnt++;
        }
        lst = idx;
        f1 = f2;
...
static inline void load_nodes_graph(Graph *g, FileReader *clp, FileReader *nds) {
    ...
    while ((ncol = readtable_filereader(clp)) != -1) {
        ...
        if ((rid = getval_cuhash(g->kbm->tag2idx, get_col_str(clp, 0))) == MAX_U4) {
            if (nwarn < 10) {
                fprintf(stderr,
                        " -- Cannot find read \"%s\" in LINE:%llu in %s -- %s:%d --\n",
                        get_col_str(clp, 0), clp->n_line, __FUNCTION__, __FILE__,
                        __LINE__);
                fflush(stderr);
                nwarn++;
            }
    ...
    while ((ncol = readtable_filereader(nds)) != -1) {
        ...
        if (get_col_str(nds, 0)[get_col_len(nds, 0) - 1] ==
            '*') {    // assert get_col_len(nds, 0) > 0
            closed = 1;
        } else {
            closed = 0;
        }
                ...
                if ((reg->rid = getval_cuhash(g->kbm->tag2idx, str)) == MAX_U4) {
                    ...
                    if (nwarn < 10) {
                        fprintf(stderr,
                                " -- Cannot find read \"%s\" in LINE:%llu in %s -- %s:%d "
                                "--\n",
                                str, nds->n_line, __FUNCTION__, __FILE__, __LINE__);
                        fflush(stderr);
                        nwarn++;
                    }
                    continue;
