...
if(eidx != MAX_U4) {
    ...
    kswx_overlap_align_core(xs, cigars, qe - qb, seq1->buffer + qb, te - tb,
                            seq2->buffer + tb, 1, cc->M, cc->X, (cc->I + cc->D) / 2,
                            (cc->I + cc->D) / 2, cc->E, mem_cache);
    XX.qb += qb;
    XX.qe += qb;
    XX.tb += tb;
    XX.te += tb;
    ...
    if(cns_debug) {
        fprintf(
            stderr,
            "JOINT\t%llu\tqe = %d -> %d\tte = %d -> %d\t[%5d,%5d]\t[%5d,%5d,%d,%d,%d]\n",
            ref_edgecnsv(cc->rs, eidx)->idx, XX.qe, b, XX.te, e, (int)seq1->size,
            (int)seq2->size, XX.aln, XX.mat, XX.mis, XX.ins, XX.del);
        fflush(stderr);
    }
...
static inline CTGCNS *init_ctgcns(void *obj, iter_cns_block itercns,
                                  info_cns_block infocns, u4i ncpu, int shuffle_rds,
                                  u4i seqmax, int winlen, int winmin, int fail_skip,
...
static inline void reset_ctgcns(CTGCNS *cc, void *obj, iter_cns_block itercns,
                                info_cns_block infocns) {
    cc->obj = obj;
    cc->itercns = itercns;
    cc->infocns = infocns;
...
static inline void print_lays_ctgcns(CTGCNS *cc, FILE *out) {
    ...
    while((cc->sq = cc->itercns(cc->obj))) {
        ...
        {
            if(cc->sq->rdtag) {
                fprintf(out, "S\t%s\t%c\t%u\t%u\t", cc->sq->rdtag, "+-"[cc->sq->rddir],
                        cc -> sq -> rdoff, (u4i)cc -> sq -> seq -> size);
            } else {
                fprintf(out, "S\tR%llu\t%c\t%u\t%u\t", (u8i)cc->sq->rdidx,
                        "+-"[cc->sq->rddir], cc -> sq -> rdoff,
                        (u4i)cc -> sq -> seq -> size);
            }
    ...
    while(cc->state) {
        if(cc->state == 1) {
            ...
            if(cc->sq == NULL || cc->sq->chridx != cc->chridx) {
                ...
                fwdbitpush_tripog(mcns->g, cc->sq->seq->bits, 0, cc->sq->seq->size,
                                  cc->sq->rbeg, cc->sq->rend);
                cc->sq = NULL;
            }
        } else if(cc->state == 2) {
                ...
                if(cc->rs->size) {
                    ...
                    for(i = 0; i < cc->rs->size; i++) {
                        ...
                        if(edge->end > edge->beg) {
                            fast_fwdbits2basebank(cc->cns, cc->seq->bits,
                                                  edge->soff + edge->beg,
                                                  edge->end - edge->beg);
            ...
            if(mcns->edge.idx) {
                ...
                if(cns_debug) {
                    thread_beg_syn(mcns);
                    fprintf(stderr, "%llu_%s_N%u_N%u\t%d\t%d\t", mcns->edge.idx,
                            cc->tag->string, mcns->edge.node1, mcns->edge.node2,
                            mcns->g->seqs->nseq, (u4i)mcns->g->cns->size);
                ...
                array_heap_push(cc->heap->buffer, cc->heap->size, cc->heap->cap,
                                edge_cns_t, mcns->edge, num_cmp(a.idx, b.idx));
                ZEROS(&mcns->edge);
            }
            thread_beg_syn(mcns);
            while(cc->heap->size) {
                ...
                if(edge->idx == cc->widx) {
                    if(!cns_debug && cc->print_progress &&
                       (cc->widx % cc->print_progress) == 0) {
                        fprintf(stderr, "\r%u contigs %llu edges %llu bases", cc->cidx,
                                cc->widx, cc->bases);
                        fflush(stderr);
                    }
                    push_edgecnsv(cc->rs, *edge);
                    array_heap_remove(cc->heap->buffer, cc->heap->size, cc->heap->cap,
                                      edge_cns_t, 0, num_cmp(a.idx, b.idx));
                    cc->widx++;
                } else {
                    break;
                }
...
static inline lay_seq_t *_push_padding_ref_samblock(SAMBlock *sb, lay_seq_t *sq) {
    ...
    while(sb->cidx < sq->chridx || (sb->cidx == sq->chridx && sb->bidx <= sq->bidx)) {
        ...
        fast_fwdbits2basebank(st->seq, sb->refs->rdseqs->bits, off + st->rdoff,
                              st->rend - st->rbeg);
        array_heap_push(
            sb->heap->buffer, sb->heap->size, sb->heap->cap, u4i,
            offset_layseqr(sb->seqs, st),
            num_cmpxx(ref_layseqr(sb->seqs, a)->chridx, ref_layseqr(sb->seqs, b)->chridx,
                      ref_layseqr(sb->seqs, a)->bidx, ref_layseqr(sb->seqs, b)->bidx,
                      ref_layseqr(sb->seqs, a)->rdidx, ref_layseqr(sb->seqs, b)->rdidx));
        if(st->rend >= len) {
            sb->cidx++;
            sb->bidx = 0;
            sb->bidx2 = sb->bidx;
...
static inline lay_seq_t *iter_samblock(void *obj) {
    ...
    do {
        ...
        while(sb->lidx == MAX_U4) {
            ...
            // chr
            if((chr = getval_cuhash(sb->refs->rdhash, get_col_str(sb->fr, 2))) ==
               MAX_U4) {
                continue;
            }
                ...
                switch(*ptr) {
                    ...
                    default:
                        ...
                        fprintf(stderr, " -- Bad cigar %d '%c' in %s -- %s:%d --\n",
                                (int)(ptr - get_col_str(sb->fr, 5)), *ptr, __FUNCTION__,
                                __FILE__, __LINE__);
                        fflush(stderr);
                        abort();
                }
                    ...
                    if(off == nxt) {
                        if(sl) {
                            if(sl->rend >= sl->rbeg + minlen) {
                                ...
                                array_heap_push(
                                    sb->heap->buffer, sb->heap->size, sb->heap->cap, u4i,
                                    offset_layseqr(sb->seqs, sl),
                                    num_cmpxx(ref_layseqr(sb->seqs, a)->chridx,
                                              ref_layseqr(sb->seqs, b)->chridx,
                                              ref_layseqr(sb->seqs, a)->bidx,
                                              ref_layseqr(sb->seqs, b)->bidx,
                                              ref_layseqr(sb->seqs, a)->rdidx,
                                              ref_layseqr(sb->seqs, b)->rdidx));
                            } else {
            ...
            if(sl) {
                if(sl->rend >= sl->rbeg + minlen) {
                    ...
                    array_heap_push(sb->heap->buffer, sb->heap->size, sb->heap->cap, u4i,
                                    offset_layseqr(sb->seqs, sl),
                                    num_cmpxx(ref_layseqr(sb->seqs, a)->chridx,
                                              ref_layseqr(sb->seqs, b)->chridx,
                                              ref_layseqr(sb->seqs, a)->bidx,
        ...
        if(sb->lidx != MAX_U4) {
            ...
            array_heap_remove(
                sb->heap->buffer, sb->heap->size, sb->heap->cap, u4i, 0,
                num_cmpxx(
                    ref_layseqr(sb->seqs, a)->chridx, ref_layseqr(sb->seqs, b)->chridx,
                    ref_layseqr(sb->seqs, a)->bidx, ref_layseqr(sb->seqs, b)->bidx,
                    ref_layseqr(sb->seqs, a)->rdidx, ref_layseqr(sb->seqs, b)->rdidx));
