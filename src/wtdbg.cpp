...
static struct option prog_opts[] = {{"cpu",                           1, 0, 't'},
                                    {"input",                         1, 0, 'i'},
                                    {"err-free-seq",                  1, 0, 'I'},
                                    {"force",                         0, 0, 'f'},
                                    {"prefix",                        1, 0, 'o'},
...
int usage(int level) {
    printf(
            ...
            "             Example: '-e 3 -x ont -S 1' in parsing order, -e will be 3, -S "
            "will be 1\n"
            " -g <number> Approximate genome size (k/m/g suffix allowed) [0]\n"
            " -X <float>  Choose the best <float> depth from input reads(effective with -g) "
            "[50]\n"
            " -L <int>    Choose the longest subread and drop reads shorter than <int> (5000 "
            ...
            " -s <float>  Min similarity, calculated by kmer matched length / aligned "
            "length, [0.05]\n"
    ...
    if (level > 0) {
        printf(
                ...
                "   See -l 2048\n"
                " --aln-min-match <int>\n"
                "   See -m 200. Here the num of matches counting basepair of the matched "
                "kmer's regions\n"
                ...
                "filtering, default: 256\n"
                " --aln-strand <int>\n"
                "   1: forward, 2: reverse, 3: both. Please don't change the deault vaule 3, "
                ...
                "--realn-kmer-subsampling=1, --realn-min-length=2048, --realn-min-match=200, "
                "--realn-min-similarity=0.1, --realn-max-var=0.25\n"
                ...
                "other reads\n"
                " --corr-mode <float>\n"
                "   Default: 0.0. If set > 0 and set --g <genome_size>, will turn on "
                "correct-align mode.\n"
                "   Wtdbg will select <genome_size> * <corr-mode> bases from reads of middle "
                "length, and align them aginst all reads.\n"
                "   Then, wtdbg will correct them using POACNS, and query corrected "
                "sequences against all reads again\n"
                "   In correct-align mode, --aln-bestn = unlimited, --no-read-clip, "
                "--no-chaining-clip. Will support those features in future\n"
                " --corr-min <int>\n"
                " --corr-max <int>\n"
                "   For each read to be corrected, uses at least <corr-min> alignments, and "
                "at most <corr-max> alignments\n"
                "   Default: --corr_min = 5, --corr-max = 10\n"
                " --corr-cov <float>\n"
                "   Default: 0.75. When aligning reads to be corrected, the alignments "
                "should cover at least <corr-cov> of read length\n"
                " --corr-block-size <int>\n"
                ...
                "<--tidy-reads>. Please add --tidy-name or set --tidy-reads to nagetive "
                "value\n"
                "   if want to rename reads. Set to 0 bp to disable tidy. Suggested value is "
                "5000 for pacbio RSII reads\n"
                " --tidy-name\n"
                ...
                "   Default: 50.0. Retaining 50.0 folds of genome coverage, combined with -g "
                "and --rdcov-filter.\n"
                " --rdcov-filter [0|1]\n"
                ...
                "selecting).\n"
                "   kbm indexs sequences into BINs of 256 bp in size, so that many parameter "
                "should be times of 256 bp. There are: --node-len, --node-ovl, "
                "--aln-min-length, --aln-dovetail ."
                "   Other parameters are counted in BINs, --dp-max-gap, --dp-max-var .\n"
                " --node-matched-bins <int>\n"
                "   Min matched bins in a node, default:1\n"
                " --node-ovl <int>\n"
                "   Default: 256. Max overlap size between two adjacent intervals in any "
                "read. It is used in selecting best nodes representing reads in graph\n"
                " --node-drop <float>\n"
                "   Default: 0.25. Will discard an node when has more this ratio intervals "
                ...
                "   When the sequence depth is low, have a try with --edge-min 2. Or very "
                "high, try --edge-min 4\n"
                " --drop-low-cov-edges\n"
                ...
                "   depth >= <--ttr-cutoff-depth>, and none of their edges have depth "
                "greater than depth * <--ttr-cutoff-ratio 0.5>\n"
                ...
                "   Please note that, once kbm-index is mmaped by kbm -R <kbm-index> start, "
                "will just get the shared memory in minute time.\n"
                ...
                "   other source of alignments to `wtdbg`. When --load-alignment, will only "
                "reading long sequences but skip building kbm index\n"
                ...
                "`wtdbg`'s <--prefix>.clps\n"
                " --load-nodes <sting>\n"
                "   Load dumped nodes from previous execution for fast construct the "
                ...
                " --no-read-length-sort\n"
                "   Defaultly, `wtdbg` sorts input sequences by length DSC. The order of "
                ...
                "chimera.\n"
                "   When building edges, clipped region won't contribute. However, `wtdbg` "
                "will use them in the final linking of unitigs\n"
                " --no-chainning-clip\n"
                "   Defaultly, performs alignments chainning in read clipping\n"
                "   ** If '--aln-bestn 0 --no-read-clip', alignments will be parsed "
                "directly, and less RAM spent on recording alignments\n"
                "\n");
    }
    return (level < 0) ? 1 : 0;
}
...
int main(int argc, char **argv) {
    ...
    uint32_t i, j, k;
    int c, opt_idx, ncpu, only_fix, realign, node_cov, max_node_cov, exp_node_cov,
            min_bins, edge_cov, store_low_cov_edge, reglen, regovl, bub_step, tip_step,
            rep_step;
    int frgtip_len, ttr_n_cov;
    int quiet, tidy_reads, filter_rd_strategy, tidy_rdtag, less_out, tip_like, cut_tip,
            rep_filter, out_alns, cnn_filter, log_rep, rep_detach, del_iso, rdclip, chainning,
            uniq_hit, bestn, rescue_low_edges;
    int min_ctg_len, min_ctg_nds, max_trace_end, max_overhang, overwrite, node_order,
            fast_mode, corr_min, corr_max, corr_bsize, corr_bstep, mem_stingy, num_index;
    ...
    rpar->aln_var = 0.25;
    opt_flags = 0;
    while ((c = getopt_long(argc, argv, "ht:i:fo:x:E:k:p:K:S:l:m:s:RvqVe:L:Ag:X:",
                            prog_opts, &opt_idx)) != -1) {
        switch (c) {
            case 't':
                ncpu = atoi(optarg);
            ...
            case 'x': {
                ...
                do {
                    ...
                    if (ptr) *ptr = 0;
                    if (KBM_LOG) {
                        fprintf(KBM_LOGF, " -- Preset: '%s' --", beg);
                        fflush(KBM_LOGF);
                    }
                    if (strcasecmp(beg, "rs") == 0 || strcasecmp(beg, "rsII") == 0) {
                        seq_type = 1;
                    } else if (strcasecmp(beg, "sq") == 0 ||
                               strcasecmp(beg, "sequel") == 0) {
                        seq_type = 2;
                    } else if (strcasecmp(beg, "ont") == 0 ||
                               strcasecmp(beg, "nanopore") == 0) {
                        seq_type = 3;
                    } else if (strcasecmp(beg, "ccs") == 0 ||
                               strcasecmp(beg, "corrected") == 0) {
                        seq_type = 4;
                        tidy_reads = 5000;
                    } else {
                ...
                break;
            case 'k':
                par->ksize = atoi(optarg);
                opt_flags |= (1 << 1);
                break;
            case 'p':
                par->psize = atoi(optarg);
                opt_flags |= (1 << 0);
                break;
            case 'K':
                fval = atof(optarg);
                par->kmax = fval;
                par->ktop = fval - par->kmax;
                opt_flags |= (1 << 6);
                break;
            case 'E':
                par->kmin = atoi(optarg);
                break;
            case 'S':
                par->kmer_mod = UInt(atof(optarg) * KBM_N_HASH);
                opt_flags |= (1 << 2);
                break;
            case 'g':
                ...
                break;
            case 'X':
                genome_depx = atof(optarg);
                break;
            case 2009:
                filter_rd_strategy = atoi(optarg);
                break;
            case 2005:
                par->max_bgap = atoi(optarg);
                break;
            case 2006:
                par->max_bvar = atoi(optarg);
                break;
            case 2007:
                par->pgap = atoi(optarg);
                break;
            case 2008:
                par->pvar = atoi(optarg);
                break;
            case 'l':
                par->min_aln = atoi(optarg) / KBM_BIN_SIZE;
                break;
            case 'm':
                par->min_mat = atoi(optarg);
                break;
            case 2004:
                par->aln_var = atof(optarg);
                break;
            case 's':
                par->min_sim = atof(optarg);
                opt_flags |= (1 << 3);
                break;
            case 2010:
                corr_mode = atof(optarg);
                break;
            case 2012:
                corr_min = atoi(optarg);
                break;
            case 2013:
                corr_max = atoi(optarg);
                break;
            case 2014:
                corr_cov = atof(optarg);
                break;
            case 2015:
                corr_bsize = atoi(optarg);
                break;
            case 2016:
                corr_bstep = atoi(optarg);
                break;
            case 2011:
                uniq_hit = 0;
                break;
            ...
            case 1005:
                ...
                break;
            case 1006:
                node_drop = atof(optarg);
                break;
            case 'e':
                edge_cov = atoi(optarg);
                break;
            case 1007:
                node_cov = atoi(optarg);
                break;
            case 1008:
                max_node_cov = atoi(optarg);
                break;
            case 1009:
                ttr_n_cov = atoi(optarg);
                break;
            case 1010:
                ttr_e_cov = atof(optarg);
                break;
            case 2002:
                load_seqs = optarg;
                break;
            ...
            case 2001:
                ...
                break;
            case 1013:
                push_cplist(pws, optarg);
                break;
            case 1014:
                par->strand_mask = atoi(optarg);
                break;
            case 1015:
                bub_step = atoi(optarg);
                break;
            case 1016:
                tip_step = atoi(optarg);
                break;
            case 1017:
                min_ctg_len = atoi(optarg);
                break;
            case 1018:
                min_ctg_nds = atoi(optarg);
                break;
            case 1019:
                less_out = 1;
                break;
            case 1020:
                par->min_bin_degree = atoi(optarg);
                break;
            case 1021:
            ...
            case 1027:
                ...
                break;
            case 1028:
                par->max_hit = atoi(optarg);
                break;
            case 1029:
                par->ksampling = atoi(optarg);
                break;
            case 'R':
                realign = 1;
                break;
            case 3001:
                rpar->psize = atoi(optarg);
                break;
            case 3002:
                rpar->kmer_mod = UInt(atof(optarg) * KBM_N_HASH);
                break;
            case 3003:
                rpar->min_aln = atoi(optarg) / KBM_BIN_SIZE;
                break;
            case 3004:
                rpar->min_mat = atoi(optarg);
                break;
            case 3005:
                rpar->min_sim = atof(optarg);
                break;
            case 3006:
                rpar->aln_var = atof(optarg);
                break;
            case 'A':
    ...
    if (optind < argc) {
        fprintf(stderr,
                "WARNING: unused command-line arguments. For multiple input files, "
                "please apply multiple -i.\n");
    ...
    if (load_seqs == NULL && load_kbm == NULL && pbs->size + ngs->size == 0) {
        fprintf(stderr,
                "ERROR: please specify the input with -i/--load-seqs/--load-kbm\n");
        return 1;
    }
    ...
    if (par->ksize + par->psize > KBM_MAX_KSIZE) {
        fprintf(stderr, " -- Invalid kmer size %d+%d=%d > %d in %s -- %s:%d --\n",
                par->ksize, par->psize, par->ksize + par->psize, KBM_MAX_KSIZE,
    ...
    if (load_nodes && load_clips) {
        ...
        fprintf(KBM_LOGF, "[%s] Done, %llu nodes\n", date(),
                (unsigned long long) g->nodes->size);
    } else {
    ...
    if (load_nodes == NULL || strlen(load_nodes) != strlen(prefix) + strlen(".1.nodes") ||
        strncmp(load_nodes, prefix, strlen(prefix)) ||
        strcmp(load_nodes + strlen(prefix), ".1.nodes")) {
    ...
    if (1) {
        ...
        fprintf(KBM_LOGF, "[%s] masked %llu high coverage nodes (>%d or <%d)\n", date(),
                (unsigned long long) cnt, max_node_cov, node_cov);
    }
    if (cnn_filter) {
        cnt = mask_nodes_by_connectivity_graph(g, ncpu, evtlog);
        fprintf(KBM_LOGF,
                "[%s] masked %llu repeat-like nodes by local subgraph analysis\n", date(),
    ...
    fprintf(KBM_LOGF, "[%s] Done, %llu edges\n", date(),
            (unsigned long long) g->edges->size);
    if (ttr_n_cov) {
    ...
    if (0) {
        ...
        fprintf(KBM_LOGF,
                "[%s] masked %llu regions(%d bp) as unreliable, total regs %llu\n",
    ...
    if (!less_out) generic_print_graph(g, print_dot_graph, prefix, ".2.dot.gz");
    {
        ...
        if (bub + tip) {
            fprintf(KBM_LOGF, "[%s] %llu bubbles; %llu tips; %llu yarns;\n", date(), bub,
                    tip, yarn);
            fflush(KBM_LOGF);
        }
    }
    ...
    if (0) {
        ...
        cnt = print_ctgs_graph(g, cnt, g->major_nctg, g->ctgs->size, prefix, ".rtg.lay",
                               ncpu, evtlog);
    }
