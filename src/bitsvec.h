...
static inline void u8byte2bits(uint64_t val, uint8_t *dat, uint64_t offset,
                               uint8_t size) {
    uint8_t i, *v;
    v = (uint8_t *)&val;
...
#else
    for(i = 0; i < size; i++) {
        if(dat[offset >> 3] & (1U << (offset & 0x7U)))
            v[7 - (i >> 3)] |= 1U << (i & 0x7U);
        offset++;
    }
#endif
...
static const obj_desc_t bitsvec_obj_desc = {"bitsvec_obj_desc",
                                            sizeof(BitsVec),
                                            1,
                                            {1},
                                            {offsetof(BitsVec, bits)},
                                            {(obj_desc_t *)&OBJ_DESC_DATA},
                                            bitsvec_obj_desc_cnt,
                                            NULL};
...
static inline int encap_bitsvec(BitsVec *vec, u8i n) {
    ...
    fLog(mtrace_file, "object bitsvec rawbits %p resized %lld\n", vec, (vec->cap * vec->n_bit + 15) / 8);
    return 1;
}
...
static inline u2i gets_bitsvec(BitsVec *vec, u8i idx) {
    u8i off;
    off = (idx * vec->n_bit);
    return ((((u2i)vec->bits[(off >> 3) + 1]) << 8) | vec->bits[(off >> 3) + 0]) >>
           (off & 0x07);
}
