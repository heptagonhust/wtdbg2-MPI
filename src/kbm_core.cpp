...
void transfer_kbm(KBM *kbm, KBMPar *par, KBMPar *rpar, u4i *corr_mode, u4i *corr_min, u4i *corr_max, float *corr_cov,
                  int world_rank) {
    size_t size;
    ...
    if (world_rank == 0) {
        ...
        fprintf(KBM_LOGF, "vector size: %3d: %d\n", world_rank, size);
        fflush(stderr);
    }
    MPI_Bcast(&size, 1, MPI_UINT64_T, 0, MPI_COMM_WORLD);
    if (world_rank != 0) {
    ...
    for (int i = 0; i < KBM_N_HASH; i++) {
        ...
        if (kbm->hashs[i]->ones->sums)
            MPI_Bcast(kbm->hashs[i]->ones->sums, kbm->hashs[i]->ones->sum_size * 2 + 1, MPI_UINT64_T, 0,
                      MPI_COMM_WORLD);
        if (world_rank != 0) {
        ...
        if (kbm->hashs[i]->dels) {
            ...
            if (kbm->hashs[i]->dels->sums)
                MPI_Bcast(kbm->hashs[i]->dels->sums, kbm->hashs[i]->dels->sum_size * 2 + 1, MPI_UINT64_T, 0,
                          MPI_COMM_WORLD);
            if (world_rank != 0) {
