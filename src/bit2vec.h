...
static const obj_desc_t bit2vec_obj_desc = {"bit2vec_obj_desc",
                                            sizeof(Bit2Vec),
                                            1,
                                            {1},
                                            {offsetof(Bit2Vec, bits)},
                                            {(obj_desc_t *)&OBJ_DESC_DATA},
                                            bit2vec_obj_desc_cnt,
                                            NULL};
...
static inline int encap_bit2vec(Bit2Vec *vec, uint32_t n) {
    ...
    fLog(mtrace_file, "object bit2vec rawbits %p resized %lld\n", vec, (vec->cap * 2 + 7) / 8);
...
static inline void set_bit2vec(Bit2Vec *vec, uint64_t idx, uint8_t dat) {
    vec->bits[idx >> 2] = (vec->bits[idx >> 2] & (~(3U << ((idx & 0x03U) << 1)))) |
                          ((dat & 0x03) << ((idx & 0x03U) << 1));
}
