...
int kbm_usage() {
    ...
    fprintf(stdout,
            "             e.g. `mkfifo pipe` then `while true; do cat pipe && sleep 1; "
            "done | kbm -t 8 -I -d ref.fa -i - -Hk 21 -S 4`\n");
    fprintf(stdout,
            "             then `cat 1.fq >pipe; cat 2.fq >pipe`, fastq format is better "
            "in interaction\n");
    ...
    fprintf(stdout,
            " -F          Filter low frequency kmers by a 4G-bytes array (max_occ=3 "
            "2-bits). Here, -E must greater than 1\n");
    fprintf(
        stdout,
    ...
    fprintf(stdout,
            "             please note that subsampling kmers will have less matched "
            "length\n");
    ...
    fprintf(stdout,
            "             If you are using shared kbmidx by other process using -D too, "
            "it will bring wrong behavior\n");
    fprintf(stdout,
    ...
    fprintf(stdout,
            ...
            "length, [0.05]\n");
    fprintf(stdout,
            " -r <float>  Max length variation of two aligned fragments, [0.25]\n");
...
#endif
    fprintf(stdout,
            "Server start: {kbm -R <wt.fa.kbmidx> start}, will mmap wt.fa.kbmidx into "
            "mmeory\n");
    fprintf(stdout,
            "Server  list: {kbm -R <wt.fa.kbmidx> list [10]}, will list the object tree "
            "in file\n");
    fprintf(stdout,
...
int kbm_main(int argc, char **argv) {
    ...
    u4i qidx, i;
    int c, ncpu, buffered_read, overwrite, quiet, tidy_reads, tidy_rdtag, skip_ctn,
        chainning, interactive, server, tree_maxcnt;
    ...
    while(
        (c = getopt(argc, argv,
                    "hi:d:o:fIt:k:p:K:E:FO:S:B:G:D:X:Y:Z:x:y:z:l:m:n:s:cr:CT:W:R:qvV")) !=
        -1) {
        switch(c) {
    ...
    if(optind < argc) {
        server = 0;
        if(strcasecmp("start", argv[optind]) == 0)
            server = 1;
        else if(strcasecmp("stop", argv[optind]) == 0)
            server = 2;
    ...
    if(loadf) {
        if(server == 1) {
            ...
            fprintf(KBM_LOGF,
                    ...
                    kbm->par->ksize, kbm->par->psize, kbm->par->kmer_mod / KBM_N_HASH);
        ...
        fprintf(KBM_LOGF,
                "[%s] Done. %u sequences, %llu bp, parameter('-k %d -p %d -S %d')\n",
                date(), (u4i)kbm->reads->size, (u8i)kbm->rdseqs->size, kbm->par->ksize,
                kbm->par->psize, kbm->par->kmer_mod / KBM_N_HASH);
        ...
        fprintf(KBM_LOGF, "\n[%s] Done, %u reads, %llu bp, %u bins\n", date(),
                (u4i)kbm->reads->size, tot_bp, (u4i)kbm->bins->size);
        fflush(KBM_LOGF);
    ...
    if(qrys->size) {
        ...
        if(readline_filereader(fr) > 0) {
            if(strcasecmp(fr->line->string, "#pairwise_test") == 0) {
                ...
                par.refmode = 1;
                maln->cc = init_ctgcns(kb, iter_kbmblock, info_kbmblock, 1, 1,
                                       maln->corr_max, 200, 100, 1, 512, &par);
                maln->lay = maln->out;
        ...
        if(run_mode == 0 || run_mode == 3) {
            while(readseq_filereader(fr, seq)) {
                if((qidx % 100) == 0) {
                    fprintf(KBM_LOGF, "\r%u\t%llu", qidx, nhit);
                    fflush(KBM_LOGF);
                }
            ...
            while((nc = readtable_filereader(fr)) >= 0) {
                ...
                aux->bmax =
                    kbm->reads->buffer[tidx].binoff + kbm->reads->buffer[tidx].bincnt;
                fprintf(out, "%s <-> %s\n", kbm->reads->buffer[qidx].tag,
                        kbm->reads->buffer[tidx].tag);
                thread_wake(maln);
                thread_wait(maln);
                if(maln->rdlen && !maln->interactive) {
                    for(i = 0; i < aux->hits->size; i++) {
            ...
            while((nc = readtable_filereader(fr)) >= 0) {
                ...
                print_exists_index_kbm(kbm, kbm->reads->buffer[qidx].tag, kbm->rdseqs,
                                       kbm->reads->buffer[qidx].rdoff,
            ...
            while((nc = readtable_filereader(fr)) >= 0) {
                ...
                fprintf(out, ">%s\n", kbm->reads->buffer[qidx].tag);
                println_fwdseq_basebank(kbm->rdseqs, kbm->reads->buffer[qidx].rdoff,
                                        kbm->reads->buffer[qidx].rdlen, out);
                fflush(out);
            }
        }
        free_filereader(fr);
            ...
            if(maln->rdlen) {
                ...
                for(i = 0; i < aux->hits->size; i++) {
                    if(par->test_mode == 0 && skip_ctn) {
                        ...
                        if((hit->tb <= margin &&
                            hit->te + margin >=
                                (int)kbm->reads->buffer[hit->tidx].bincnt * KBM_BSIZE) &&
                           (hit->qb > margin ||
                            hit->qe + margin <
                                (int)kbm->reads->buffer[hit->qidx].bincnt * KBM_BSIZE)) {
