...
static const u1i base_bit_table[256] = {
    ...
    4, 0, 4, 1, 4, 4, 4, 2, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 3, 4,
    4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 0, 4, 1, 4, 4, 4, 2, 4, 4, 4, 4,
...
#define bits2revbit(bits, off) \
    ((~((bits)[(off) >> 5] >> (((~(off)) & 0x1FU) << 1))) & 0x03U)
...
static inline void dna_rev_seqs(u8i *seqs, u8i seq_size) {
    ...
    for(i = 0; i < j; i++) {
        seqs[i] = ~seqs[i];
        seqs[i] = ((seqs[i] & 0x3333333333333333LLU) << 2) |
                  ((seqs[i] & 0xCCCCCCCCCCCCCCCCLLU) >> 2);
        seqs[i] = ((seqs[i] & 0x0F0F0F0F0F0F0F0FLLU) << 4) |
                  ((seqs[i] & 0xF0F0F0F0F0F0F0F0LLU) >> 4);
...
static inline void dna_shl_seqs(u8i *seqs, u8i seq_size, u1i base_val) {
    ...
    seqs[i] =
        (seqs[i] << d) | (((u8i)(base_val & 0x03U)) << ((32 - (seq_size & 0x1FU)) << 1));
}
...
static inline u8i seq2revkmer(char *seq, u4i ksize) {
    ...
    kmer = 0;
    for(i = 0; i < ksize; i++)
        kmer = (kmer << 2) | ((~base_bit_table[(int)seq[ksize - 1 - i]]) & 0x03);
    return kmer;
}
...
#define reverse_dna_coord(x, y, tot_len) \
    {                                    \
        x = x ^ y;                       \
        y = x ^ y;                       \
        x = x ^ y;                       \
...
#define old_bit2bits(bits, off, bit)                                     \
    {                                                                    \
        if(((off)&0x1FU) == 0) (bits)[(off) >> 5] = 0;                   \
        (bits)[(off) >> 5] |= ((u8i)(bit)) << (((~(off)) & 0x1FU) << 1); \
    }
...
#define bit2bits(bits, off, bit)                                                 \
    {                                                                            \
        ...
        __off2 = (((~(off)) & 0x1FU) << 1);                                      \
        (bits)[__off1] =                                                         \
            ((bits)[__off1] & (~(0x3LLU << __off2))) | (((u8i)(bit)) << __off2); \
    }
...
static inline u8i sub_seqbits(u8i *src, u8i off, u1i len) {
    ...
    off1 = off >> 5;
    off2 = (off & 0x1FU) << 1;
    return ((src[off1] << off2) | (((src[off1 + 1] >> (62 - off2)) >> 2))) >>
           ((32 - len) << 1);
}
...
static inline int cmpgt_seqbits(u8i *bits, u8i off1, u8i off2, u4i _len) {
    ...
    for(i = 0; i < len; i += 32) {
        v[0] =
            (bits[idxs[0]] << offs[0]) | (((bits[idxs[0] + 1] >> (62 - offs[0])) >> 2));
        v[1] =
            (bits[idxs[1]] << offs[1]) | (((bits[idxs[1] + 1] >> (62 - offs[1])) >> 2));
        if(v[0] > v[1]) {
            return 1;
        } else if(v[0] < v[1]) {
            return 0;
...
static inline size_t basebank_obj_desc_cnt(void *obj, int idx) {
    return ((((BaseBank *)obj)->size + 31) / 32 + 1) * 8;
    idx = idx;
}
...
static const obj_desc_t basebank_obj_desc = {"BaseBank",
                                             sizeof(BaseBank),
                                             1,
                                             {1},
                                             {offsetof(BaseBank, bits)},
                                             {(obj_desc_t *)&OBJ_DESC_DATA},
                                             basebank_obj_desc_cnt,
                                             basebank_obj_desc_post_load};
...
static inline void normalize_basebank(BaseBank *bnk) {
    if(bnk->size < bnk->cap) {
        if(bnk->size & 0x1FU) {
            bnk->bits[bnk->size >> 5] =
                bnk->bits[bnk->size >> 5] & (MAX_U8 << (64 - ((bnk->size & 0x1FU) << 1)));
        }
    }
}
...
static inline u1i get_basebank(BaseBank *bnk, u8i off) {
    return bits2bit(bnk->bits, off);
}
...
static inline void print_lines_basebank(BaseBank *bnk, u8i off, u8i len, FILE *out,
                                        int linewidth) {
    u8i i, b, e;
    char *buf;
...
#define println_fwdseq_basebank(bnk, off, len, out) \
    println_seq_basebank(bnk, off, len, out)
...
static inline u8i sub32_basebank(BaseBank *bnk, u8i off) {
    return sub32seqbits(bnk->bits, off);
}

static inline u8i sub4_basebank(BaseBank *bnk, u8i off) {
    return sub4seqbits(bnk->bits, off);
}

// assert(len > 0 && len <= 32)
static inline u8i subbits_basebank(BaseBank *bnk, u8i off, u1i len) {
    return sub_seqbits(bnk->bits, off, len);
}
...
static inline int bitsearch_basebank(BaseBank *bnk, u8i *_off, u8i len, u8i bits,
                                     u1i size, int max_occ) {
    u8i off, end, k, mask;
    u1i b;
    int ret;
    off = *_off;
...
static inline int hzbitsearch_basebank(BaseBank *bnk, u8i *_off, u8i len, u8i bits,
                                       u1i size, int max_occ) {
...
if(_mradix->task == 1) {
    ...
    for(i = beg; i < end; i++) {
        lcp = 0;
        for(j = 0; j < klen; j += 32) {
            mask = sub32seqbits(bb->bits, offs[i - 1] + j) ^
                   sub32seqbits(bb->bits, offs[i] + j);
            if(mask == 0) {
                lcp += 32;
            } else {
...
static inline void msd_radix_sort_u4_basebank(BaseBank *bb, u4v *offs, u1v *lcps,
                                              u1i _klen, u4i ncpu, FILE *log) {
...
static const obj_desc_t seqbank_obj_desc = {
    "SeqBank",
    sizeof(SeqBank),
    5,
    {1, 1, 1, 1, 1},
    {offsetof(SeqBank, rdseqs), offsetof(SeqBank, rdtags), offsetof(SeqBank, rdoffs),
     offsetof(SeqBank, rdlens), offsetof(SeqBank, rdhash)},
    {&basebank_obj_desc, &cplist_deep_obj_desc, &u8v_obj_desc, &u4v_obj_desc,
     &cuhash_obj_desc},
    NULL,
    rebuild_rdhash_seqbank};
...
static inline void free_seqbank(SeqBank *sb) {
    ...
    for(i = 0; i < sb->rdtags->size; i++)
        if(sb->rdtags->buffer[i]) free(sb->rdtags->buffer[i]);
...
static inline void rebuild_rdhash_seqbank(void *_sb, size_t aux) {
    ...
    sb = (SeqBank *)_sb;
    clear_cuhash(
        sb->rdhash);    // hash size is not changed, thus there won't have hash re-size
    for(i = 0; i < sb->rdtags->size; i++) {
        put_cuhash(sb->rdhash, (cuhash_t){get_cplist(sb->rdtags, i), i});
    }
}

static inline void push_seqbank(SeqBank *sb, char *tag, int tag_len, char *seq,
                                int seq_len) {
    char *ptr;
    if(tag && tag_len) {
...
static inline void fwdbitpush_seqbank(SeqBank *sb, char *tag, int tag_len, u8i *bits,
                                      u8i off, u4i len) {
    char *ptr;
    if(tag && tag_len) {
        ptr = (char*)malloc(tag_len + 1);
...
static inline void revbitpush_seqbank(SeqBank *sb, char *tag, int tag_len, u8i *bits,
                                      u8i off, u4i len) {
    char *ptr;
    if(tag && tag_len) {
        ptr = (char*)malloc(tag_len + 1);
...
static inline u4i find_seqbank(SeqBank *sb, char *tag) {
    cuhash_t *e;
    if((e = get_cuhash(sb->rdhash, tag)))
        return e->val;
    else
        return MAX_U4;
}
...
static inline u4i num_n50(u4v *lens, FILE *out) {
    ...
    if(out) {
        fprintf(
            out,
            "TOT %llu, CNT %u, AVG %u, MAX %u, N50 %u, L50 %u, N90 %u, L90 %u, Min %u",
            tot, (u4i)lens->size, avg, max, n50, l50, n90, l90, min);
        fflush(out);
    }
    return n50;
}
