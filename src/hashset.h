...
#define init_hashset_macro(hash_type, hash_ele_type)                              \
    ...
    static const obj_desc_t hash_type##_obj_desc = {                              \
        TOSTR(_hashset_##hash_type),                                              \
        sizeof(hash_type),                                                        \
        3,                                                                        \
        {1, 1, 1},                                                                \
        {offsetof(hash_type, array), offsetof(hash_type, ones),                   \
         offsetof(hash_type, dels)},                                              \
        {(obj_desc_t *)&OBJ_DESC_DATA, (obj_desc_t *)&bitvec_obj_desc,            \
         (obj_desc_t *)&bitvec_obj_desc},                                         \
        hash_type##_obj_desc_cnt,                                                 \
        NULL};                                                                    \
    ...
    static inline uint64_t hash_type##_find_next_prime(uint64_t num) {            \
        if(num % 2 == 0) num++;                                                   \
        while(1) {                                                                \
            if(hash_type##_is_prime(num)) return num;                             \
            num += 2;                                                             \
        }                                                                         \
    }                                                                             \
    ...
    static inline void set_userdata_##hash_type(hash_type *set, void *userdata) { \
        set->userdata = userdata;                                                 \
    }                                                                             \
    static inline hash_type *init_##hash_type(uint32_t size) {                    \
        return init2_##hash_type(size, 0.67f);                                    \
    }

#define get_hashset_macro(hash_type, hash_ele_type, hash_key_type, hash_key_code,       \
                          hash_key_equal, hash_val_type, hash_ele2val)                  \
    ...
    static inline hash_ele_type *ref_##hash_type(hash_type *set, size_t off) {          \
        return set->array + off;                                                        \
    }                                                                                   \
...
#define prepare_hashset_macro(hash_type, hash_ele_type, hash_key_type, hash_key_code,   \
                              hash_key_equal)                                           \
    static inline void encap_##hash_type(hash_type *set, size_t num);                   \
    static inline hash_ele_type *prepare_##hash_type(hash_type *set, hash_key_type key, \
                                                     int *exists) {                     \
        hash_ele_type *e;                                                               \
        size_t hc, d;                                                                   \
        if(set->dels == NULL) {                                                         \
...
#define exists_hashset_macro(hash_type, hash_ele_type, hash_key_type, hash_key_code, \
                             hash_key_equal)                                         \
...
#define remove_hashset_macro(hash_type, hash_ele_type, hash_key_type, hash_key_code, \
                             hash_key_equal)                                         \
...
#define reset_iter_hashset_macro(hash_type)                     \
    static inline void reset_iter_##hash_type(hash_type *set) { \
        set->iter_ptr = 0;                                      \
    }

#define ref_iter_hashset_macro(hash_type, hash_ele_type)                                 \
    static inline hash_ele_type *ref_iter2_##hash_type(hash_type *set,                   \
                                                       size_t *iter_ptr) {               \
        if(set->dels) {                                                                  \
...
#define count_hashset_macro(hash_type)                        \
    static inline int64_t count_##hash_type(hash_type *set) { \
        return set->count;                                    \
    }
...
#define freeze_hashset_macro(hash_type, hash_ele_type, hash_code_macro)             \
    static inline int freeze_##hash_type(hash_type *set, float load_factor) {       \
        ...
        sort_array_adv(set->count, hvs[a] > hvs[b], swap_var(hvs[a], hvs[b]);       \
                       swap_var(set->array[a], set->array[b]));                     \
...
#define encap_hashset_macro(hash_type, hash_ele_type, hash_code_macro)              \
    static inline void encap_##hash_type(hash_type *set, size_t num) {              \
        ...
        fLog(mtrace_file, "object hashset %s %p resized %lld\n", #hash_ele_type, set, n * set->e_size);\
        ...
        for(i = 0; (i = next_one_bitvec(set->ones, i)) < set->size; i++) {          \
            ...
            while(1) {                                                              \
                ...
                one_bitvec(ones, hc);                                               \
                if(hc < set->size && get_bitvec(set->ones, hc) &&                   \
                   get_bitvec(set->dels, hc) == 0) {                                \
    ...
    static inline size_t offsetof_##hash_type(hash_type *set, hash_ele_type *ptr) { \
        return ptr - set->array;                                                    \
    }
...
#define define_hashtable(hash_type, hash_ele_type, hash_code_macro, hash_equal_macro, \
                         hash_key_type, hash_key_code, hash_key_equal, hash_val_type, \
                         hash_ele2val)                                                \
    init_hashset_macro(hash_type, hash_ele_type);                                     \
    get_hashset_macro(hash_type, hash_ele_type, hash_key_type, hash_key_code,         \
                      hash_key_equal, hash_val_type, hash_ele2val);                   \
    prepare_hashset_macro(hash_type, hash_ele_type, hash_key_type, hash_key_code,     \
                          hash_key_equal);                                            \
    exists_hashset_macro(hash_type, hash_ele_type, hash_key_type, hash_key_code,      \
                         hash_key_equal);                                             \
    add_hashset_macro(hash_type, hash_ele_type, hash_code_macro, hash_equal_macro);   \
    put_hashset_macro(hash_type, hash_ele_type);                                      \
    remove_hashset_macro(hash_type, hash_ele_type, hash_key_type, hash_key_code,      \
                         hash_key_equal);                                             \
...
#define define_hashset(hash_type, hash_ele_type, hash_code_macro, hash_equal_macro)     \
    define_hashtable(hash_type, hash_ele_type, hash_code_macro, hash_equal_macro,       \
                     hash_ele_type, hash_code_macro, hash_equal_macro, hash_ele_type *, \
                     ITSELF)
...
define_hashtable(uuhash, uuhash_t, uuhash_code, uuhash_equals, u4i, u32hashcode,
                 uuhash_key_equals, u4i, KV_HASH_GET_VAL);
...
define_hashtable(uihash, uihash_t, uihashcode, uihashequals, u4i, u32hashcode,
                 uihashkeyequals, b4i, KV_HASH_GET_VAL);
...
define_hashtable(UUhash, UUhash_t, UUhashcode, UUhashequals, u8i, u64hashcode,
                 UUhashkeyequals, u8i, KV_HASH_GET_VAL);
...
define_hashtable(cuhash, cuhash_t, cuhash_code, cuhash_equals, char *, __string_hashcode,
                 cuhash_key_equals, u4i, KV_HASH_GET_VAL);
static const obj_desc_t cuhash_struct_deep_obj_desc = {
    "cuhash_struct_deep_obj_desc",
    sizeof(cuhash_t),
    1,
    {1},
    {offsetof(cuhash_t, key)},
    {(obj_desc_t *)&OBJ_DESC_CHAR_ARRAY},
    NULL,
    NULL};
static const obj_desc_t cuhash_deep_obj_desc = {
    "cuhash_deep_obj_desc",
    sizeof(cuhash),
    3,
    {1, 1, 1},
    {offsetof(cuhash, array), offsetof(cuhash, ones), offsetof(cuhash, dels)},
    {(obj_desc_t *)&cuhash_struct_deep_obj_desc, (obj_desc_t *)&bitvec_obj_desc,
     &bitvec_obj_desc},
    cuhash_obj_desc_cnt,
    NULL};
...
define_hashtable(cihash, cihash_t, cihash_code, cihash_equals, char *, __string_hashcode,
                 cihash_key_equals, b4i, KV_HASH_GET_VAL);
...
define_hashtable(clhash, clhash_t, clhash_code, clhash_equals, char *, __string_hashcode,
                 clhash_key_equals, u8i, KV_HASH_GET_VAL);
...
define_hashtable(cchash, cchash_t, cchash_code, cchash_equals, char *, __string_hashcode,
                 cchash_key_equals, char *, KV_CCHASH_GET_VAL);
