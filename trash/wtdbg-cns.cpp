...
if(mcns->task == MCNS_TASK_DBGCNS) {
    ...
    for(i = 1; i < cns->qblks->size; i++) {
        ...
        nbeg = branched_dynamic_programming_alignment(
            dag, mcns->cns->qseqs->buffer + blk->off, blk->len, g, bnodes, bedges,
            mem_buffer);
        if(nbeg == 0) {
    ...
    for(i = 0; i < dag->cns->size; i++)
        add_char_string(mcns->cns->seq, bit_base_table[dag->cns->buffer[i]]);
    ...
    kswx_overlap_align_core(xs, cigars, qe - qb, mcns->seq1->buffer + qb, te - tb,
                            mcns->seq2->buffer + tb, 1, mcns->M, mcns->X, mcns->I,
                            mcns->D, mcns->E, mem_cache);
    xs[0]->qb += qb;
    xs[0]->qe += qb;
    xs[0]->tb += tb;
    xs[0]->te += tb;
...
uint32_t run(int reglen, int ksize, int Z, int W, int M, int X, int I, int D, int E,
             int H, int L, int XX, int OO, int EE, int cns_model, f4i pM, f4i pX, f4i pI,
             f4i pD, int candidate_mode, int corr_struct, uint32_t ncpu, FileReader *fr,
             FILE *out) {
    String *tag, *seq;
    u1v *cseqs;
    u4v *cxs, *cys, *tes, *qes;
            ...
            if(mcns->task == cns_model && mcns->cns->seq->size) {
                if(cns_debug) {
                    fprintf(stderr, "%s_%d_N%u_N%u\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%s\n",
                            tag->string, mcns->eid, mcns->node1, mcns->node2,
                            mcns->cns->qlen, mcns->cns->seq->size, mcns->cns->max_score,
                            mcns->cns->alns[0], mcns->cns->alns[1], mcns->cns->alns[2],
                            mcns->cns->alns[3], mcns->cns->seq->string);
                }
                cxs->buffer[mcns->eid] = cseqs->size;
                for(j = 0; j < mcns->cns->seq->size; j++)
                    push_u1v(cseqs, base_bit_table[(int)mcns->cns->seq->string[j]]);
            ...
            if(tag->size) {
                ...
                if(mcns->task == cns_model && mcns->cns->seq->size) {
                    if(cns_debug) {
                        ...
                        fprintf(stderr, "%s_%d_N%u_N%u\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%s\n",
                                tag->string, mcns->eid, mcns->node1, mcns->node2,
                                mcns->cns->qlen, mcns->cns->seq->size,
                                mcns->cns->max_score, mcns->cns->alns[0],
                                mcns->cns->alns[1], mcns->cns->alns[2],
                                mcns->cns->alns[3], mcns->cns->seq->string);
                        if(0) {
                            u4i j, x;
                            for(j = x = 0; j < mcns->cns->cigars->size; j++) {
                                if(mcns->cns->cigars->buffer[j] == DBGCNS_PATH_I) {
                                    ...
                                    fputc("ACGT"[mcns->cns->cns->buffer[x]], stderr);
                                    x++;
                                }
                            }
                            fputc('\n', stderr);
                ...
                for(i = 1; i < eid; i++) {
                    ...
                    if(mcns->task == MCNS_TASK_OVERLAP && mcns->ret.aln > 0) {
                        if(cns_debug) {
                            fprintf(stderr, "#%s_%d\t%d\t%d\t%d", tag->string,
                                    mcns->eid - 1, (int)mcns->seq1->size, mcns->ret.qb,
                                    mcns->ret.qe);
                            fprintf(stderr, "\t%s_%d\t%d\t%d\t%d", tag->string, mcns->eid,
                                    (int)mcns->seq2->size, mcns->ret.tb, mcns->ret.te);
                            fprintf(stderr, "\t%d\t%d\t%d\t%d\t%d\n", mcns->ret.aln,
                                    mcns->ret.mat, mcns->ret.mis, mcns->ret.ins,
                                    mcns->ret.del);
                        }
                        {
                            b = mcns->ret.qe;
                    ...
                    mcns->eid = i;
                    clear_u1v(mcns->seq1);
                    append_array_u1v(mcns->seq1, cseqs->buffer + cxs->buffer[i - 1],
                                     cys->buffer[i - 1] - cxs->buffer[i - 1]);
                    clear_u1v(mcns->seq2);
                    append_array_u1v(mcns->seq2, cseqs->buffer + cxs->buffer[i],
                                     cys->buffer[i] - cxs->buffer[i]);
                    mcns->ret = KSWX_NULL;
                ...
                if(mcns->task == MCNS_TASK_OVERLAP && mcns->ret.aln > 0) {
                    if(cns_debug) {
                        fprintf(stderr, "#%s_%d\t%d\t%d\t%d", tag->string, mcns->eid - 1,
                                (int)mcns->seq1->size, mcns->ret.qb, mcns->ret.qe);
                        fprintf(stderr, "\t%s_%d\t%d\t%d\t%d", tag->string, mcns->eid,
                                (int)mcns->seq2->size, mcns->ret.tb, mcns->ret.te);
                        fprintf(stderr, "\t%d\t%d\t%d\t%d\t%d\n", mcns->ret.aln,
                                mcns->ret.mat, mcns->ret.mis, mcns->ret.ins,
...
int usage() {
    printf(
        ...
        " -f          Force overwrite\n"
        " -j <int>    Expected length of node, or say the overlap length of two adject "
        "units in layout file, [1000] bp\n"
        "-----------------BEG DBG options---------------------------------\n"
        ...
        " -c <int>    Candidate strategy, 0: best-kmers, 1: median length, 2: first "
        "(include), 3: first (exclude), 4: longest, 5, shortest, [0]\n"
...
int main(int argc, char **argv) {
    ...
    if(infs->size)
        fr = open_all_filereader(infs->size, infs->buffer, 1);
    else
    ...
    if(outf) {
        ...
        out = stdout;
    run(reglen, ksize, Z, W, M, X, I, D, E, H, L, XX, OO, EE, cns_model, pM, pX, pI, pD,
        candidate_mode, corr_struct, ncpu, fr, out);
    close_filereader(fr);
    if(outf) fclose(out);
    free_cplist(infs);
