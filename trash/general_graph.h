...
#define ge_edge_hashequals(E1, E2)                   \
    (GEEDGEHASH(E1).node1 == GEEDGEHASH(E2).node1 && \
     GEEDGEHASH(E1).node2 == GEEDGEHASH(E2).node2 && \
     GEEDGEHASH(E1).dir1 == GEEDGEHASH(E2).dir1 &&   \
     GEEDGEHASH(E1).dir2 == GEEDGEHASH(E2).dir2)
...
#define define_simple_geg_callback(tag, node_tag, node_tag_t, edge_tag, edge_tag_t)   \
    void tag##nodeclr(void *aux) {                                                    \
        clear_##node_tag((node_tag *)aux);                                            \
    }                                                                                 \
    ...
    void tag##nodedel(void *aux, u8i idx) {                                           \
        UNUSED(aux);                                                                  \
        UNUSED(idx);                                                                  \
    }                                                                                 \
    void tag##edgeclr(void *aux) {                                                    \
        clear_##edge_tag((edge_tag *)aux);                                            \
    }                                                                                 \
    ...
    void tag##edgedel(void *aux, u8i idx) {                                           \
        UNUSED(aux);                                                                  \
        UNUSED(idx);                                                                  \
    }                                                                                 \
    static inline void tag##_set_callbacks_gegraph(GEGraph *g, node_tag *nodeaux,     \
                                                   edge_tag *edgeaux) {               \
        set_callbacks_gegraph(g, (void *)nodeaux, (void *)edgeaux, tag##nodeclr,      \
                              tag##nodeadd, tag##nodedel, tag##edgeclr, tag##edgeadd, \
                              tag##edgedel);                                          \
    }
...
static inline void set_callbacks_gegraph(GEGraph *g, void *nodeaux, void *edgeaux,
                                         geg_clr_node_callback nodeclr,
                                         geg_add_node_callback nodeadd,
                                         geg_del_node_callback nodedel,
                                         geg_clr_edge_callback edgeclr,
...
static inline ge_edge_t *prepare_edge_gegraph(GEGraph *g, u8i node1, int dir1, u8i node2,
                                              int dir2, int *exists) {
    ge_node_t *n;
    ge_edge_t *e;
    ge_edge_ref_t *f;
...
static inline ge_edge_ref_t *single_edge_gegraph(GEGraph *g, ge_node_t *n, int dir,
                                                 int *info) {
    ge_edge_ref_t *f, *ret;
    uint64_t idx;
    ret = NULL;
    if(info) {
        *info = GEG_TRACE_MSG_ZERO;
        ...
        while(idx) {
            ...
            if(ret) {
                *info = GEG_TRACE_MSG_MORE;
                return NULL;
            } else {
                *info = GEG_TRACE_MSG_ONE;
                ret = f;
            }
        }
    } else {
...
static inline ge_edge_ref_t *edge_node2node_gegraph(GEGraph *g, u8i node1, int dir1,
                                                    u8i node2, int dir2) {
    ge_node_t *n;
    ge_edge_ref_t *f;
    ge_edge_t *e;
...
static inline void print_dot_gegraph(GEGraph *g, FILE *out) {
    ...
    for(i = 1; i < g->edges->size; i++) {
        ...
        fprintf(out, " N%llu -> N%llu [label=\"%c%c:%d:%d\" color=%s]\n", (u8i)e->node1,
                (u8i)e->node2, "+-"[e->dir1], "+-"[e->dir2], e -> cov, e -> off,
                colors[e->dir1][e->dir2]);
        fprintf(out, " N%llu -> N%llu [label=\"%c%c:%d:%d\" color=%s]\n", (u8i)e->node2,
                (u8i)e->node1, "-+"[e->dir2], "-+"[e->dir1], e -> cov, e -> off,
                colors[!e->dir2][!e->dir1]);
    }
    fprintf(out, "}\n");
    fflush(out);
}
