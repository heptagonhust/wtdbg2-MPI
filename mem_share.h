    ...
    (((a) > (b))                           \
         ...
         : (((a) < (b))                    \
                ...
                : (((c) > (d))             \
                       ...
                       : (((c) < (d)) ? -1 \
                                      : (((e) > (f)) ? 1 : (((e) < (f)) ? -1 : 0))))))
#define num_cmpgtxx(a, b, c, d, e, f) \
    (((a) > (b))                      \
         ? 1                          \
         : (((a) < (b)) ? 0 : (((c) > (d)) ? 1 : (((c) < (d)) ? 0 : ((e) > (f))))))
#define num_abs(n) ((n) < 0 ? -(n) : (n))

#ifndef SWAP_TMP
#define SWAP_TMP
#define swap_tmp(a, b, t) \
    {                     \
        t = a;            \
        a = b;            \
        b = t;            \
...
static inline size_t encap_list(void **buffer, size_t e_size, size_t size, size_t cur_cap,
    ...
    if(ptr == NULL) {
        fprintf(stderr,
                ...
                "%p in %s -- %s:%d --\n",
                (unsigned long long)(e_size * (cap + n_head)),
                (unsigned long long)(e_size * (cur_cap + n_head)), *buffer, __FUNCTION__,
                __FILE__, __LINE__);
        print_backtrace(stderr, 20);
        abort();
    }
...
static inline int get_linux_proc_info(u8i *rss, u8i *vsize, double *utime,
                                      double *stime) {
    int c, n_spc;
    char str[64];
    FILE *fp;
...
static inline void print_proc_stat_info(int signum) {
    ...
    fprintf(log,
            "** PROC_STAT(%d) **: real %.3f sec, user %.3f sec, sys %.3f sec, maxrss "
            "%.1f kB, maxvsize %.1f kB\n",
            signum, _sig_proc_deamon->rtime, _sig_proc_deamon->utime,
            _sig_proc_deamon->stime, _sig_proc_deamon->max_rss / 1024.0,
...
u8i cb, ce;
cb = microtime();
get_linux_sys_info((u8i *)&_proc_deamon->memtotal, (u8i *)&_proc_deamon->memavail,
                   (int *)&_proc_deamon->ncpu);
...
get_linux_proc_info(&rss, &vsz, (double *)&_proc_deamon->utime,
                    (double *)&_proc_deamon->stime);
...
#define END_STAT_PROC_INFO(log)                                                        \
    ...
    if(log) {                                                                          \
        fprintf(log,                                                                   \
                "** PROC_STAT(TOTAL) **: real %.3f sec, user %.3f sec, sys %.3f sec, " \
                "maxrss %.1f kB, maxvsize %.1f kB\n---\n",                             \
                _sig_proc_deamon->rtime, _sig_proc_deamon->utime,                      \
                _sig_proc_deamon->stime, _sig_proc_deamon->max_rss / 1024.0,           \
                _sig_proc_deamon->max_vsz / 1024.0);                                   \
    }                                                                                  \
...
static inline size_t mem_size_round(size_t size) {
    return (size + 7) & 0xFFFFFFFFFFFFFFF8LLU;
}

static inline uint8_t mem_size_gap(size_t size) {
    return (size & 0x07U) ? 8 - (size & 0x07U) : 0;
}
...
typedef struct obj_desc_t {
    const char *tag;
    size_t
        size;    // If size = 0, it is virtual, mem_type will be OR with MEM_PTR_TYPE_DUMP. See OBJ_DESC_CHAR_ARRAY
...
static inline size_t mem_size_obj(void *obj, uint8_t mem_type, const obj_desc_t *desc,
    ...
    switch(mem_type) {
        ...
        case 2:
            for(m = 0; m < cnt; m++)
                if(((void **)obj)[m]) size += mem_size_round(desc->size);
            break;
        case 1:
            size += mem_size_round(
                cnt *
                desc->size);    // TODO: if desc == &OBJ_DESC_DATA, mem_size_round may waste many memory
        case 0: break;
    }
    ...
    for(m = 0; m < cnt; m++) {
        ...
        for(i = 0; i < desc->n_child; i++) {
            if(desc->mem_type[i] & 0x01) {
                size +=
                    mem_size_obj(*((void **)(ref + desc->addr[i])),
                                 desc->mem_type[i] | (desc->size ? 0 : MEM_PTR_TYPE_DUMP),
                                 desc->desc[i], 0, desc->cnt ? desc->cnt(ref, i) : 1);
            } else {
                size +=
                    mem_size_obj(ref + desc->addr[i],
                                 desc->mem_type[i] | (desc->size ? 0 : MEM_PTR_TYPE_DUMP),
                                 desc->desc[i], 0, desc->cnt ? desc->cnt(ref, i) : 1);
            }
        }
...
static inline void mem_dup_obj(void **ret, void *obj, size_t aux_data, uint8_t mem_type,
    ...
    if(desc->n_child == 0 && desc->post == NULL) {
        ...
        for(m = 0; m < cnt; m++) {
            ...
            for(i = 0; i < desc->n_child; i++) {
                if(desc->mem_type[i] & 0x01) {
                    mem_dup_obj((void **)(chd + desc->addr[i]),
                                *((void **)(ref + desc->addr[i])), aux_data,
                                desc->mem_type[i] | (desc->size ? 0 : MEM_PTR_TYPE_DUMP),
                                desc->desc[i], desc->cnt ? desc->cnt(ref, i) : 1);
                } else {
                    mem_dup_obj((void **)(chd + desc->addr[i]), ref + desc->addr[i],
                                aux_data,
                                desc->mem_type[i] | (desc->size ? 0 : MEM_PTR_TYPE_DUMP),
                                desc->desc[i], desc->cnt ? desc->cnt(ref, i) : 1);
                }
...
static inline size_t mem_dump_obj(void *obj, uint8_t mem_type, const obj_desc_t *desc,
                                  size_t offset, size_t cnt, FILE *out, int mem_free) {
    void *ref;
    size_t size, m;
    int i;
    ...
    if((mem_type & 0x02) == 0 && desc->n_child == 0) {
        ...
        for(m = 0; m < cnt; m++) {
            ...
            for(i = 0; i < desc->n_child; i++) {
                if(desc->mem_type[i] & 0x01) {
                    size = mem_dump_obj(
                        *((void **)(ref + desc->addr[i])),
                        desc->mem_type[i] | (desc->size ? 0 : MEM_PTR_TYPE_DUMP),
                        desc->desc[i], size, desc->cnt ? desc->cnt(ref, i) : 1, out,
                        mem_free);
                } else {
                    size = mem_dump_obj(
                        ref + desc->addr[i],
    ...
    if(mem_free) {
        if((mem_type & 0x02) && desc->size)
            for(m = 0; m < cnt; m++) free(((void **)obj)[m]);
        if((mem_type & 0x01) && ((mem_type & 0x02) | desc->size)) free(obj);
    }
    return size;
}

static inline size_t mem_load_obj(void *obj, size_t aux_data, uint8_t mem_type,
                                  const obj_desc_t *desc, size_t addr_beg, size_t cnt) {
    size_t addr, m;
    int i;
    void *ref, **ptr;
                ...
                *ptr = (void *)addr;
                addr =
                    mem_load_obj(*ptr, aux_data,
                                 desc->mem_type[i] | (desc->size ? 0 : MEM_PTR_TYPE_DUMP),
                                 desc->desc[i], addr, desc->cnt ? desc->cnt(ref, i) : 1);
            } else {
                addr =
                    mem_load_obj((void *)ptr, aux_data,
                                 desc->mem_type[i] | (desc->size ? 0 : MEM_PTR_TYPE_DUMP),
                                 desc->desc[i], addr, desc->cnt ? desc->cnt(ref, i) : 1);
            }
        }
...
static inline size_t mem_tree_obj(FILE *out, void *obj, size_t mem_type,
                                  const obj_desc_t *desc, size_t addr, size_t cnt,
                                  size_t max_cnt, int level, int max_level) {
    size_t m;
    int i, j;
    void *ref, **ptr;
    if(obj == NULL) return 0;
    ...
    if(max_level <= 0 || level <= max_level)
        fprintf(out, "%llu\n", (u8i)(addr - (size_t)obj));
    for(m = 0; m < cnt; m++) {
        if(cnt > 1 && (max_level <= 0 || level <= max_level) &&
           (max_cnt == 0 || m < max_cnt)) {
            for(j = 0; j < level; j++) fputc('-', out);
            fprintf(out, "[%llu/%llu]\n", (u8i)m, (u8i)cnt);
        }
            ...
            if(desc->mem_type[i] & 0x01) {
                ...
                if(max_cnt && m >= max_cnt) {
                    addr = mem_tree_obj(
                        out, (void *)addr,
                        desc->mem_type[i] | (desc->size ? 0 : MEM_PTR_TYPE_DUMP),
                        desc->desc[i], addr, desc->cnt ? desc->cnt(ref, i) : 1, max_cnt,
                        2, 1);
                } else {
                    addr = mem_tree_obj(
                        out, (void *)addr,
...
static inline const obj_desc_t *mem_locate_obj(void *obj, size_t *_mem_type,
                                               const obj_desc_t *desc, u4i *trace_childs,
                                               size_t *trace_cnts, u4i ntrace,
                                               size_t *addr, size_t *cnt, int selected) {
    ...
    if(selected) {
        if(trace_childs[0] >= (u4i)desc->n_child) {
            fprintf(
                stderr,
                " -- Illegal trace child (%d >= %d) in OBJ_DESC[%s] in %s -- %s:%d --\n",
                trace_childs[0], desc->n_child, desc->tag, __FUNCTION__, __FILE__,
                __LINE__);
            return desc;
        }
        if(trace_cnts[0] >= *cnt) {
            fprintf(stderr,
                    " -- Illegal trace cont (%llu >= %llu) in OBJ_DESC[%s] in %s -- "
                    "%s:%d --\n",
                    (u8i)trace_cnts[0], (u8i)*cnt, desc->tag, __FUNCTION__, __FILE__,
                    __LINE__);
            return desc;
        }
    }
    if(desc->n_child == 0) {
        switch(mem_type) {
            case 2:
            case 3: *addr += (*cnt) * mem_size_round(desc->size);
            default: return desc;
        }
    }
    ret = desc;
        ...
        for(i = 0; i < nc; i++) {
            ...
            if(desc->mem_type[i] & 0x01) {
                ...
                *cnt = desc->cnt ? desc->cnt(ref, i) : 1;
                ret = mem_locate_obj((void *)*addr, _mem_type, desc->desc[i],
                                     trace_childs + 1, trace_cnts + 1, ntrace - 1, addr,
                                     cnt, (selected && m + 1 == ns && i == nc));
            } else {
                *_mem_type = desc->mem_type[i] | (desc->size ? 0 : MEM_PTR_TYPE_DUMP);
                *cnt = desc->cnt ? desc->cnt(ref, i) : 1;
                ret = mem_locate_obj((void *)ptr, _mem_type, desc->desc[i],
                                     trace_childs + 1, trace_cnts + 1, ntrace - 1, addr,
                                     cnt, (selected && m + 1 == ns && i == nc));
            }
        }
...
static inline size_t mem_dump_free_obj_file(void *obj, size_t mem_type,
                                            const obj_desc_t *desc, size_t cnt,
...
static inline void register_mem_share_file_lock(char *file) {
    ...
    if(mem_share_lock_size == 0) {
        if((sig_term = signal(SIGTERM, sig_cleanup_mem_share_file_locks)) == SIG_IGN)
            signal(SIGTERM, SIG_IGN);
        if((sig_int = signal(SIGINT, sig_cleanup_mem_share_file_locks)) == SIG_IGN)
            signal(SIGINT, SIG_IGN);
        if((sig_hup = signal(SIGHUP, sig_cleanup_mem_share_file_locks)) == SIG_IGN)
            signal(SIGHUP, SIG_IGN);
        if((sig_kill = signal(SIGKILL, sig_cleanup_mem_share_file_locks)) == SIG_IGN)
...
static inline void print_tree_obj_file(FILE *out, const obj_desc_t *desc, char *path,
...
static inline void *mem_read_obj_file(const obj_desc_t *desc, char *path, size_t *size,
...
static inline void *mem_read_sub_obj_file(const obj_desc_t *desc, u4i *trace_childs,
                                          size_t *trace_cnts, u4i ntrace, char *path,
                                          size_t *size) {
...
static inline void *mem_load_obj_file(const obj_desc_t *desc, char *_path, size_t *size,
                                      size_t *mem_type, size_t *cnt, size_t *aux_data) {
    void *mem;
    ...
    mem = mmap(0, (sz + psize - 1) / psize * psize, PROT_READ | PROT_WRITE, MAP_SHARED,
               fd, 0);
    if(mem == MAP_FAILED) {
        perror("Cannot mmap");
        return NULL;
...
static inline void *mem_find_obj_file(const obj_desc_t *desc, char *_path, size_t *size,
    ...
    *size = msg[1];
    ...
    mem = mmap(addr, ((*size) + 4 * sizeof(size_t) + psize - 1) / psize * psize, prot,
