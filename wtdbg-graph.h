...
static inline u8i mask_nodes_by_edge_cov_graph(Graph *g, u4i min_node_cov,
                                               float min_edge_cov_ratio, FILE *out) {
    node_t *n;
    edge_t *e;
    edge_ref_t *f;
    u8i idx, nid, ret;
...
static inline int evaluate_node_connectivity_graph(Graph *g, u8i nid, u4v *rds,
                                                   subnodehash *nodes, subedgev *edges,
                                                   ptrrefv *stack) {
    node_t *nd;
    read_t *rd;
    reg_t *rg;
        ...
        while(idx) {
            ...
            while(stack->size) {
                ...
                while(edx) {
                    ...
                    e->node->flag |= n1->flag;
                    push_ptrrefv(stack,
                                 (ptr_ref_t){offset_subnodehash(nodes, e->node), e->dir});
                }
                if(exists) break;
            }
            if(exists) break;
...
static inline void print_subgraph_dot(Graph *g, u8i id, subnodehash *nodes,
                                      subedgev *edges, FILE *out) {
    subnode_t *n;
    subedge_t *e;
    u8i idx;
    int k;
    ...
    while((n = ref_iter_subnodehash(nodes))) {
        if(n->closed) continue;
        fprintf(out, "N%llu [label=\"N%llu(%llu)\"]\n", (u8i)n->node, (u8i)n->node,
                (u8i)g->nodes->buffer[n->node].rep_idx);
        for(k = 0; k < 2; k++) {
            idx = n->edges[k].idx;
            while(idx) {
                e = ref_subedgev(edges, idx);
    ...
    while(idx) {
        ...
        if(f->flg) {
            fprintf(out, "N%llu\t%c\tN%llu\t%c\t%d\t%d\n", (u8i)e->node2, "+-"[!e->dir1],
                    (u8i)e -> node1, "+-"[e->dir1], e -> cov, e -> off);
        } else {
            fprintf(out, "N%llu\t%c\tN%llu\t%c\t%d\t%d\n", (u8i)e->node1, "+-"[e->dir1],
                    (u8i)e -> node2, "+-"[e->dir2], e -> cov, e -> off);
        }
    }
}

static inline edge_ref_t *first_living_edge_graph(Graph *g, node_t *n, int dir,
                                                  int *info) {
    edge_ref_t *f, *ret;
    u8i idx;
    ret = NULL;
...
static inline edge_ref_t *edge_node2node_graph(Graph *g, u8i node1, int dir1, u8i node2,
                                               int dir2) {
    node_t *n;
    edge_ref_t *f;
    edge_t *e;
...
static inline int cal_offset_traces_graph(Graph *g, tracev *path, u8i beg, u8i end,
                                          int offset) {
    trace_t *t;
    node_t *n;
    reg_t *r;
...
static inline u8i true_linear_unique_trace_graph(Graph *g, tracev *path, u8i max_step,
                                                 u8i visit, int *msg) {
    trace_t *t;
    node_t *n;
    edge_t *e;
    ...
    while(step < max_step) {
        t = ref_tracev(path, path->size - 1);
        f = first_living_edge_graph(g, ref_nodev(g->nodes, t->node), !t->dir, &info);
        if(info == WT_TRACE_MSG_MORE) {
            if(path->size > 1) path->size--;
            if(msg) *msg = -1 - info;
            break;
        }
        n = ref_nodev(g->nodes, t->node);
        n->bt_visit = visit;
        f = first_living_edge_graph(g, ref_nodev(g->nodes, t->node), t->dir, &info);
        if(info == WT_TRACE_MSG_ZERO) {
            if(msg) *msg = info;
            break;
        } else if(info == WT_TRACE_MSG_MORE) {
            if(path->size > 1) path->size--;
            if(msg) *msg = info;
            break;
        }
        ...
        n = ref_nodev(g->nodes, f->flg ? e->node1 : e->node2);
        if(n->bt_visit == visit) {
            if(msg) *msg = WT_TRACE_MSG_VISITED;
            break;
        }
        dir = f->flg ? !e->dir1 : e->dir2;
        t->edges[t->dir] = *f;
...
static inline u8i true_linear_unique_path_graph(Graph *g, pathv *path, u8i max_step,
                                                u8i visit, int *msg) {
    path_t *t;
    frg_t *n;
    lnk_t *e;
...
static inline u4i rescue_low_cov_transitive_edges_graph(Graph *g, u8i nid, u8v *edges,
                                                        UUhash *hash) {
    node_t *n, *w, *v;
    edge_ref_t *f, *f2, *f3;
    edge_t *e, *e1, *e2;
    ...
    for(k = 0; k < 2; k++) {
        ...
        for(i = 0; i < edges->size; i++) {
            ...
            while(idx) {
                ...
                idx = f2->next;
                if(f2->flg) {
                    nid3 = g->edges->buffer[f2->idx].node1;
                    k3 = !g->edges->buffer[f2->idx].dir1;
                } else {
                    nid3 = g->edges->buffer[f2->idx].node2;
                    k3 = g->edges->buffer[f2->idx].dir2;
                }
                ...
                off1 = off2 =
                    (g->regs->buffer[n->regs.idx].end -
                     g->regs->buffer[n->regs.idx].beg) +
                    (g->regs->buffer[v->regs.idx].end - g->regs->buffer[v->regs.idx].beg);
                off1 += e1->off;
                off2 +=
                    e->off + e2->off +
                    (g->regs->buffer[w->regs.idx].end - g->regs->buffer[w->regs.idx].beg);
            ...
            if(0) {
                ...
                while(idx) {
                    ...
                    if(k3 != k4) continue;
                    v = ref_nodev(g->nodes, nid3);
                    off1 = off2 = (g->regs->buffer[n->regs.idx].end -
                                   g->regs->buffer[n->regs.idx].beg) +
                                  (g->regs->buffer[w->regs.idx].end -
                                   g->regs->buffer[w->regs.idx].beg);
                    off1 += e->off;
                    off2 += e1->off + e2->off +
                            (g->regs->buffer[v->regs.idx].end -
                             g->regs->buffer[v->regs.idx].beg);
                    r = ref_regv(g->regs, w->regs.idx);
                    if(num_diff(off1, off2) >= num_min(off1, off2)) continue;
                    yes = 1;
                    //revive_edge_graph(g, e);
...
static inline u4i rescue_low_cov_tip_edges_core(Graph *g, u8i nid) {
    ...
    for(k = 0; k < 2; k++) {
        ...
        while(idx) {
            ...
            if(ee == NULL || e->cov > ee->cov ||
               (e->cov == ee->cov && w->regs.cnt > ww->regs.cnt)) {
                ee = e;
                ww = w;
            }
        }
        if(ee) {
            revive_edge_graph(g, ee);
            ret++;
        }
    }
    return ret;
...
static inline int rescue_mercy_edge_core_graph(Graph *g, u4i rid, BitVec *tips[2]) {
    ...
    while(idx) {
        ...
        if(s) {
            if(d == (1 << (!r->dir))) {
                f = edge_node2node_graph(g, s->node, (0b100 >> c) & 0x01, r->node,
                                         !((0b100 >> d) & 0x01));
                if(f) {
                    e = ref_edgev(g->edges, f->idx);
                    revive_edge_graph(g, e);
...
static inline u8i rescue_weak_tip_lnks_graph(Graph *g) {
    ...
    for(k = 0; k < 2; k++) {
        for(i = 0; i < weaks[k]->size; i++) {
            ...
            if(weaks[0]->buffer[e->frg2] == weaks[k]->buffer[i] ||
               weaks[1]->buffer[e->frg2] == weaks[k]->buffer[i]) {
                ret++;
...
static inline u4i reduce_transitive_edges_core_graph(Graph *g, u8i nid, u8v *edges,
                                                     UUhash *hash, u4i closed_val) {
    node_t *n, *w, *v;
    edge_ref_t *f, *f2, *f3;
    edge_t *e, *e1, *e2;
    UUhash_t *u;
    ...
    for(k = 0; k < 2; k++) {
        ...
        while(idx) {
            ...
            if(e->closed < closed_val) {
                put_UUhash(hash, (UUhash_t){f->flg ? e->node1 : e->node2,
                                            (idx << 1) | e->closed});
                push_u8v(edges, idx);
            }
            idx = f->next;
...
static inline u4i myers_transitive_reduction_core_graph(Graph *g, u8i nid, float _fuzz,
                                                        int closed) {
    node_t *n, *w, *x;
    edge_ref_t *f, *f2;
    edge_t *e, *e2;
...
static inline u4i myers_transitive_reduction_core_frg_graph(Graph *g, u8i nid,
                                                            float _fuzz) {
    frg_t *n, *w, *x;
    edge_ref_t *f, *f2;
    lnk_t *e, *e2;
...
static inline u4i detach_repetitive_frg_core_graph(Graph *g, u8i nid, u4i max_dist,
                                                   u8i visit, u8v *nds, u8v *heap) {
    frg_t *n, *w, *x;
    edge_ref_t *f;
    lnk_t *e;
    u8i idx, wid, xid, eid;
    ...
    for(k = 0; k < 2; k++) {
        ...
        while(idx) {
            ...
            if(w->bt_visit == visit) {
                ...
                fprintf(stderr,
                        " -- F%llu:%c -> F%llu:%c already visited in %s -- %s:%d --\n",
                        nid, "+-"[k], wid, "+-"[w->rep_dir], __FUNCTION__, __FILE__,
                        __LINE__);
                fflush(stderr);
                return 0;
            }
            ...
            if(w->rep_idx < max_dist) {
                array_heap_push(
                    heap->buffer, heap->size, heap->cap, u8i, wid,
                    num_cmp(g->frgs->buffer[a].rep_idx, g->frgs->buffer[b].rep_idx));
            }
        }
    }
    ...
    while(heap->size) {
        wid = heap->buffer[0];
        array_heap_remove(
            heap->buffer, heap->size, heap->cap, u8i, 0,
            num_cmp(g->frgs->buffer[a].rep_idx, g->frgs->buffer[b].rep_idx));
        w = ref_frgv(g->frgs, wid);
        ...
        while(idx) {
            ...
            if(x->rep_idx < max_dist) {
                array_heap_push(
                    heap->buffer, heap->size, heap->cap, u8i, xid,
                    num_cmp(g->frgs->buffer[a].rep_idx, g->frgs->buffer[b].rep_idx));
            }
        }
    }
        ...
        for(k = 0; k < 2; k++) {
            ...
            while(idx) {
                ...
                if((w->bt_idx >> 1) == bidx) {
                    //cut_lnk_graph(g, e);
                    cut_lnk_core_graph(
                        g, e, WT_EDGE_CLOSED_HARD);    // in case of loop cut and revive
                }
            }
        }
    }
...
static inline u4i reduce_transitive_lnks_core_graph(Graph *g, u8i nid, u8v *lnks,
                                                    UUhash *hash, u4i closed_val) {
    frg_t *n, *w, *v;
    edge_ref_t *f, *f2, *f3;
    lnk_t *e, *e1, *e2;
    ...
    for(k = 0; k < 2; k++) {
        ...
        while(idx) {
            ...
            if(e->closed < closed_val) {
                put_UUhash(
                    hash, (UUhash_t){f->flg ? e->frg1 : e->frg2, (idx << 1) | e->closed});
                push_u8v(lnks, idx);
            }
            idx = f->next;
        }
        ...
        for(i = 0; i < lnks->size; i++) {
            ...
            while(idx) {
                ...
                idx = f2->next;
                if(f2->flg) {
                    nid3 = g->lnks->buffer[f2->idx].frg1;
                    k3 = !g->lnks->buffer[f2->idx].dir1;
                } else {
                    nid3 = g->lnks->buffer[f2->idx].frg2;
                    k3 = g->lnks->buffer[f2->idx].dir2;
                }
                e2 = ref_lnkv(g->lnks, f2->idx);
                //if(e2->closed) continue;
                if(e2->closed >= closed_val) continue;
...
static inline u8i trim_tip_core_graph(Graph *g, uint16_t max_step, tracev *path, u8i nid,
                                      int hard_trim) {
    trace_t *t, T;
    node_t *n;
    edge_ref_t *f;
        ...
        while(idx) {
            ...
            if(step2 > step) {
                ...
                break;
                      ...
                      (e->cov > g->edges->buffer[t->edges[dir].idx].cov ||
                       (step == 1 && e->cov == g->edges->buffer[t->edges[dir].idx].cov &&
                        e->off >= g->edges->buffer[t->edges[dir].idx].off))) {
                found = 1;
                break;
            }
...
static inline u4i safe_cut_redundant_lnks_graph(Graph *g, frgbtv *bts, frg_bt_t *b1,
                                                frg_bt_t *b2) {
    u4i ret;
    ret = 0;
    if(0) {
...
static inline u4i pop_bubble_core_graph(Graph *g, uint16_t max_step, btv *bts, u4v *heap,
                                        u8i nid, u4i dir, u8i visit, int safe) {
    bt_t *bt, *tb;
    node_t *n;
    edge_ref_t *f;
    edge_t *e;
    ...
    bt->ending = 0;
    clear_u4v(heap);
    array_heap_push(heap->buffer, heap->size, heap->cap, u4i, bts->size - 1,
                    num_cmpx(bts->buffer[a].step, bts->buffer[b].step,
                             bts->buffer[b].score, bts->buffer[a].score));
    n->bt_visit = visit;
    n->bt_idx = bts->size - 1;
    n->single_in = 1;
    unclosed = 0;
    while(heap->size && heap->size < WT_MAX_BTIDX) {
        bidx = array_heap_pop(heap->buffer, heap->size, heap->cap, u4i,
                              num_cmpx(bts->buffer[a].step, bts->buffer[b].step,
                                       bts->buffer[b].score, bts->buffer[a].score));
        ...
        for(i = lst; i < bts->size; i++) {
                ...
                if(tb->dir == bts->buffer[tb->n->bt_idx].dir) {
                    if(tb->ind && bts->buffer[tb->n->bt_idx].ind) {
                        if(tb->step == bts->buffer[tb->n->bt_idx].step) {
                            return safe_cut_redundant_edges_graph(
                                g, bts, tb, ref_btv(bts, tb->n->bt_idx));
                        } else {
                            return safe_cut_redundant_edges_graph(
                                g, bts, ref_btv(bts, tb->n->bt_idx), tb);
                        }
                    } else if(tb->ind) {
                        return safe_cut_redundant_edges_graph(
                            g, bts, tb, ref_btv(bts, tb->n->bt_idx));
            ...
            if(tb->n->unvisit == 0) {
                ...
                if(count_living_edges_graph(g, tb->n, tb->dir)) {
                    array_heap_push(heap->buffer, heap->size, heap->cap, u4i,
                                    tb->n->bt_idx,
                                    num_cmpx(bts->buffer[a].step, bts->buffer[b].step,
                                             bts->buffer[b].score, bts->buffer[a].score));
                }
...
static inline u4i pop_frg_bubble_core_graph(Graph *g, uint16_t max_step, frgbtv *bts,
                                            u4v *heap, u8i nid, u4i dir, u8i visit) {
    frg_bt_t *bt, *tb;
    frg_t *n;
    edge_ref_t *f;
    lnk_t *e;
        ...
        for(i = lst; i < bts->size; i++) {
            ...
            if(tb->n->bt_visit != visit) {
                ...
                if(tb->dir == bts->buffer[tb->n->bt_idx].dir) {
                    if(tb->ind && bts->buffer[tb->n->bt_idx].ind) {
                        if(tb->step == bts->buffer[tb->n->bt_idx].step) {
                            return safe_cut_redundant_lnks_graph(
                                g, bts, tb, ref_frgbtv(bts, tb->n->bt_idx));
                        } else {
                            return safe_cut_redundant_lnks_graph(
                                g, bts, ref_frgbtv(bts, tb->n->bt_idx), tb);
                        }
                    } else if(tb->ind) {
                        return safe_cut_redundant_lnks_graph(
                            g, bts, tb, ref_frgbtv(bts, tb->n->bt_idx));
...
static inline u4i resolve_yarn_core_graph(Graph *g, u4i max_step, btv *bts, u4v *heap,
                                          u8i nid, u4i dir, u8i visit) {
    bt_t *bt, *tb;
    node_t *n, *m;
    edge_ref_t *f;
    edge_t *e;
    ...
    while(heap->size && bts->size < WT_MAX_BTIDX) {
        ...
        if(n_in == 1) {
            if(heap->size == 0 && tip == 1) {
                return pop_bubble_backtrace_graph(g, bts,
                                                  tip ? tip_idx : heap->buffer[0]);
...
static inline u8i gen_unitigs_graph(Graph *g) {
    ...
    fprintf(KBM_LOGF, "[%s] ", date());
    num_n50(lens, KBM_LOGF);
    fprintf(KBM_LOGF, "\n");
    free_u4v(lens);
    return nutg;
}
...
static inline seqletv *path2seqlets_graph(Graph *g, pathv *path) {
    ...
    for(i = 0; i < path->size; i++) {
        ...
        if(p1) {
            ...
            off =
                p1->off + (p1->dir ? (int)(frg1->len - (t1->off + g->reglen)) : t1->off);
            f = p1->lnks + p1->dir;
            l = ref_lnkv(g->lnks, f->idx);
            len = l->off + (p1->dir ? t1->off + g->reglen : frg1->len - t1->off) +
                  (p2->dir ? frg2->len - t2->off : t2->off + g->reglen);
            push_seqletv(qs, (seqlet_t){t1->node, p1->dir ^ t1->dir, t2->node,
                                        p2->dir ^ t2->dir, off, len});
        }
        if(p2->dir) {
            for(j = p2->tx + 1; j <= p2->ty; j++) {
                ...
                push_seqletv(
                    qs, (seqlet_t){t2->node, !t2->dir, t1->node, !t1->dir, off, len});
            }
        } else {
            for(j = p2->tx + 1; j <= p2->ty; j++) {
                ...
                len = t2->off - t1->off + g->reglen;
                push_seqletv(qs,
                             (seqlet_t){t1->node, t1->dir, t2->node, t2->dir, off, len});
            }
        }
        p1 = p2;
        frg1 = frg2;
...
static inline u8i gen_contigs_graph(Graph *g, FILE *out) {
    ...
    for(nid = 0; nid < g->frgs->size; nid++) {
        ...
        if(out) {
            for(i = 0; i < path->size; i++) {
                t = ref_pathv(path, i);
                fprintf(out, "ctg%d\tF%d\t%c\t%d\n", (int)g->ctgs->size, t->frg,
                        "+-*@"[t->dir], t -> off *KBM_BIN_SIZE);
            }
        }
...
static inline void n50_stat_contigs_graph(Graph *g) {
    ...
    for(i = 0; i < g->major_nctg; i++) {
        ...
        if(len <= 0) {
            fprintf(stderr,
                    " -- seqlet[ctg%llu off=%d, len=%d, sum=%d]  in %s -- %s:%d --\n", i,
                    (int)q->off, (int)q->len, len, __FUNCTION__, __FILE__, __LINE__);
            fflush(stderr);
            continue;
        }
        push_u4v(lens, len);
    }
    fprintf(KBM_LOGF, "[%s] Estimated: ", date());
    num_n50(lens, KBM_LOGF);
    fprintf(KBM_LOGF, "\n");
    free_u4v(lens);
}
...
static inline void print_dot_subgraph(Graph *g, subnodehash *nodes, subedgev *edges,
                                      FILE *out) {
    subnode_t *n1, *n2;
    subedge_t *e;
    u4i k, idx;
    ...
    while((n1 = ref_iter_subnodehash(nodes))) {
        ...
        fprintf(out,
                "N%llu [label=\"N%llu(%llu) %d:%d:%d\" style=filled fillcolor=\"%s\" "
                "color=\"%s\"]\n",
                (u8i)n1->node, (u8i)n1->node, (u8i)g->nodes->buffer[n1->node].rep_idx,
                n1->cov, n1->visit, n1->bt_open, n1->fix ? "yellow" : "white",
                n1->visit ? "green" : (n1->cov > 2 ? "blue" : "black"));
    }
    reset_iter_subnodehash(nodes);
    while((n1 = ref_iter_subnodehash(nodes))) {
        if(n1->closed) continue;
        for(k = 0; k < 2; k++) {
            ...
            while(idx) {
                ...
                fprintf(out, " N%llu -> N%llu [label=\"%c%c:%d:%d\" color=\"%s\" %s]\n",
                        (u8i)n1->node, (u8i)n2->node, "+-"[k], "+-"[e->dir], e -> cov,
                        e -> visit, e -> cov > 1 ? "blue" : "black",
                        e->visit ? "style=dashed" : "");
            }
        }
    }
    fprintf(out, "}\n");
...
static inline void fprintf_dot_subgraph(Graph *g, subnodehash *nodes, subedgev *edges,
...
static inline subedge_t *find_edge_subgraph(subnodehash *nodes, subedgev *edges,
                                            u4i node1, int dir1, u4i node2, int dir2) {
    subnode_t *n;
    subedge_t *e;
    u8i idx;
...
static inline int cut_edge_core_subgraph(subnodehash *nodes, subedgev *edges, u4i node1,
                                         int dir1, u4i node2, int dir2) {
    subnode_t *n;
    subedge_t *e, *p;
    u8i idx;
    n = ref_subnodehash(nodes, node1);
...
static inline int cut_edge_subgraph(subnodehash *nodes, subedgev *edges, u4i node1,
                                    int dir1, u4i node2, int dir2) {
    ...
    if(log) {
        for(i = 0; i < lnks->size; i++) {
            ...
            fprintf(log, "F%d[%c:%d] -> F%d[%c:%d] = %d, cov=%d\n", l->frg1,
                    "+-"[l->dir1], l -> tidx1, l -> frg2, "+-"[l->dir2], l -> tidx2,
                    l -> off, l -> cov);
        }
    }
    cov = 0;
    ...
    psort_array(
        ...
        num_cmpgt(
            (a.flg ? ((g->lnks->buffer[a.idx].frg2 << 1) | !g->lnks->buffer[a.idx].dir2)
                   : ((g->lnks->buffer[a.idx].frg1 << 1) | g->lnks->buffer[a.idx].dir1)),
            (b.flg
                 ? ((g->lnks->buffer[b.idx].frg2 << 1) | !g->lnks->buffer[b.idx].dir2)
                 : ((g->lnks->buffer[b.idx].frg1 << 1) | g->lnks->buffer[b.idx].dir1))));
    push_edgerefv(g->lrefs, (edge_ref_t){g->lnks->size, 0, 0});
    memset(next_ref_lnkv(g->lnks), 0, sizeof(lnk_t));
    g->lnks->size--;
    g->lrefs->size--;
    ...
    for(lst = idx = 1; idx <= g->lrefs->size; idx++) {
        ...
        if(lst < idx) {
            ...
            for(x = lst; x + 1 < (int)idx; x++) {
                ...
                if(g->lnks->buffer[g->lrefs->buffer[x].idx].closed == WT_EDGE_CLOSED_NULL)
                    frg->lnks[F1.flg].cnt++;
            }
            if(g->lnks->buffer[g->lrefs->buffer[x].idx].closed == WT_EDGE_CLOSED_NULL)
                frg->lnks[F1.flg].cnt++;
...
static inline int gen_seq_traces_graph(Graph *g, tracev *path, String *seq) {
    ...
    for(i = 0; i < path->size; i++) {
        ...
        if(t1) {
                ...
                while(r1->node == t1->node && r2->node == t2->node) {
                    if(r1->rid > r2->rid) {
                        ...
                        if(r1->beg < r2->beg) {
                            ...
                            seq_basebank(g->kbm->rdseqs,
                                         g->kbm->reads->buffer[r1->rid].rdoff +
                                             (r1->end * KBM_BIN_SIZE),
                                         inc, seq->string + seq->size);
                            seq->size += inc;
                        } else {
                            if(!(t1->dir ^ r1->dir)) {
                                r1++;
            ...
            if(inc > 0) {
                inc = 0;
                while(inc++ < e->off * KBM_BIN_SIZE) add_char_string(seq, 'N');
            } else if(inc < 0) {
                if(seq->size + inc < 0)
                    seq->size = 0;
                else
        ...
        if(t1->dir ^ reg->dir)
            revseq_basebank(
                g->kbm->rdseqs,
                g->kbm->reads->buffer[reg->rid].rdoff + (reg->beg * KBM_BIN_SIZE), inc,
                seq->string + seq->size);
        else
            seq_basebank(
                g->kbm->rdseqs,
...
for(i = mlay->t_idx; i < mlay->path->size; i += mlay->n_cpu) {
    ...
    sort_array(mlay->regs->buffer + lay->roff, lay->rcnt, lay_reg_t,
               num_cmpgt(b.end - b.beg, a.end - a.beg));
    if(lay->rcnt == 0 && mlay->log) {
        thread_beg_syn(mlay);
        fprintf(mlay->log, " -- N%llu(%c) -> N%llu(%c) has no read path --\n",
                (u8i)let->node1, "+-"[let->dir1], (u8i)let -> node2, "+-"[let->dir2]);
        fflush(mlay->log);
        thread_end_syn(mlay);
    }
}
    ...
    for(i = beg; i < end; i++) {
        ...
        len = len * KBM_BIN_SIZE;
        {
            ...
            fprintf(bw->out, ">ctg%llu nodes=%llu len=%u\n", uid, (u8i)path->size + 1,
                    len);
            if(log)
                fprintf(log, "OUTPUT_CTG\tctg%d -> ctg%d nodes=%llu len=%u\n", (int)i,
                        (int)uid, (u8i)path->size + 1, len);
            for(j = 0; j < lays->size; j++) {
                ...
                fprintf(bw->out, "E\t%d\tN%llu\t%c\tN%llu\t%c\n",
                        (int)t->off * KBM_BIN_SIZE, (u8i)t->node1, "+-"[t->dir1],
                        (u8i)t -> node2, "+-"[t->dir2]);
                regs = thread_access(mlay, (j % ncpu))->regs;
                for(c = 0; c < lay->rcnt; c++) {
                    reg = ref_layregv(regs, lay->roff + c);
                    fprintf(bw->out, "%c\t%s\t%c\t%d\t%d\t", "Ss"[reg->view],
                            g -> kbm -> reads -> buffer[reg->rid].tag, "+-"[reg->dir],
                            reg -> beg *KBM_BIN_SIZE,
                            (reg->end - reg->beg) * KBM_BIN_SIZE);
                    if(reg->dir) {
                        print_revseq_basebank(g->kbm->rdseqs,
                                              g->kbm->reads->buffer[reg->rid].rdoff +
                                                  reg->beg * KBM_BIN_SIZE,
                                              (reg->end - reg->beg) * KBM_BIN_SIZE,
                                              bw->out);
                    } else {
                        print_seq_basebank(g->kbm->rdseqs,
                                           g->kbm->reads->buffer[reg->rid].rdoff +
                                               reg->beg * KBM_BIN_SIZE,
                                           (reg->end - reg->beg) * KBM_BIN_SIZE, bw->out);
                    }
                    fprintf(bw->out, "\n");
                }
            }
            ...
            while(r1->node == t1->node && r2->node == t2->node) {
                if(r1->rid > r2->rid) {
                    ...
                    if(r1->beg < r2->beg) {
                        ...
                        beg = r1->beg * KBM_BIN_SIZE;
                        end = r2->end * KBM_BIN_SIZE;
                        fprintf(out, "S\t%s\t", g->kbm->reads->buffer[rid].tag);
                        fprintf(out, "+\t%d\t%d\t", (int)beg, (int)(end - beg));
                        encap_string(str, end - beg);
                        fwdseq_basebank(g->kbm->rdseqs,
                                        g->kbm->reads->buffer[rid].rdoff + beg, end - beg,
                                        str->string);
                        ...
                        beg = r2->beg * KBM_BIN_SIZE;
                        end = r1->end * KBM_BIN_SIZE;
                        fprintf(out, "S\t%s\t", g->kbm->reads->buffer[rid].tag);
                        fprintf(out, "-\t%d\t%d\t", (int)beg, (int)(end - beg));
                        encap_string(str, end - beg);
                        revseq_basebank(g->kbm->rdseqs,
...
static inline u8i print_utgs_graph(Graph *g, char *prefix, char *utg, char *lay) {
    ...
    for(i = uid = 0; i < g->utgs->size; i++) {
        ...
        if(gen_seq_traces_graph(g, path, seq) < g->min_ctg_len ||
           (int)path->size < g->min_ctg_nds) {
            o_seq = files[1];
            o_lay = files[3];
        } else {
            o_seq = files[0];
        ...
        fprintf(o_lay, ">utg%llu len=%d nodes=%llu\n", (unsigned long long)uid, seq->size,
                (unsigned long long)path->size);
        print_traces_graph(g, path, o_lay);
    }
    free_string(seq);
    fprintf(KBM_LOGF, "[%s] %llu unitigs (>= %d bp), total %llu bp\n", date(),
            (unsigned long long)cnt, g->min_ctg_len, (unsigned long long)tot);
    fclose(files[0]);
    fclose(files[1]);
    fclose(files[2]);
    fclose(files[3]);
...
static inline void get_subgraph_nodes_graph(Graph *g, ptrrefhash *nodes, u8v *stack,
                                            uint16_t max_step, u4i closed_val) {
    node_t *n;
    edge_ref_t *f;
    edge_t *e;
    ptr_ref_t *p, *pp;
...
static inline u8i print_local_dot_graph(Graph *g, FILE *out) {
    ...
    while((p = ref_iter_ptrrefhash(hash))) {
        ...
        fprintf(out, "N%llu [label=\"{N%llu %d | %s | %c_%d_%d}\"]\n", i, i, n->regs.cnt,
                g->kbm->reads->buffer[r->rid].tag, "FR"[r->dir], r -> beg,
                r -> end - r->beg);
    }
    reset_iter_ptrrefhash(hash);
    while((p = ref_iter_ptrrefhash(hash))) {
            ...
            while(idx) {
                ...
                if(f->flg) {
                    ...
                    fprintf(out, "N%llu -> N%llu [label=\"%c%c:%d:%d\" color=%s]\n", i,
                            (unsigned long long)e->node1, "+-"[k], "+-"[!e->dir1],
                            e -> cov, e -> off, colors[k][!e->dir1]);
                } else {
                    //if(!exists_ptrrefhash(hash, (ptr_ref_t){e->node2, 0})) continue;
                    fprintf(out, "N%llu -> N%llu [label=\"%c%c:%d:%d\" color=%s]\n", i,
                            (unsigned long long)e->node2, "+-"[k], "+-"[e->dir2],
                            e -> cov, e -> off, colors[k][e->dir2]);
                }
            }
        }
    }
...
static inline u8i print_dot_full_graph(Graph *g, FILE *out) {
    ...
    for(i = 0; i < g->nodes->size; i++) {
        ...
        fprintf(bw->out, "N%llu [label=\"{N%llu %d | %s | %c_%d_%d}\"]\n", i, i,
                n->regs.cnt, g->kbm->reads->buffer[r->rid].tag, "FR"[r->dir], r -> beg,
                r -> end - r->beg);
    }
    for(i = 0; i < g->nodes->size; i++) {
        if((i % 1000) == 0) flush_bufferedwriter(bw);
            ...
            while(idx) {
                ...
                if(f->flg) {
                    fprintf(bw->out, "N%llu -> N%llu [label=\"%c%c:%d:%d\" color=%s%s]\n",
                            i, (unsigned long long)e->node1, "+-"[k], "+-"[!e->dir1],
                            e -> cov, e -> off, colors[k][!e->dir1],
                            e->closed ? " style=dashed" : "");
                } else {
                    fprintf(bw->out, "N%llu -> N%llu [label=\"%c%c:%d:%d\" color=%s%s]\n",
                            i, (unsigned long long)e->node2, "+-"[k], "+-"[e->dir2],
                            e -> cov, e -> off, colors[k][e->dir2],
                            e->closed ? " style=dashed" : "");
                }
            }
...
static inline u8i print_nodes_graph(Graph *g, FILE *out) {
    ...
    for(i = 0; i < g->nodes->size; i++) {
        ...
        for(j = 0; j < n->regs.cnt; j++) {
            r = ref_regv(g->regs, n->regs.idx + j);
            fprintf(out, "\t%s_%c_%d_%d", g->kbm->reads->buffer[r->rid].tag, "FR"[r->dir],
                    r -> beg, r -> end - r->beg);
...
static inline u8i print_reads_graph(Graph *g, FILE *out) {
    ...
    for(i = 0; i < g->kbm->reads->size; i++) {
        rd = ref_readv(g->reads, i);
        fprintf(out, "%s\t%d\t%u", g->kbm->reads->buffer[i].tag,
                g->kbm->reads->buffer[i].rdlen, rd->regs.cnt);
        idx = rd->regs.idx;
        while(idx) {
            r = ref_regv(g->regs, idx);
            fprintf(out, "\tN%llu%s:%c_%d_%d", (unsigned long long)r->node,
                    (r->closed ? "*" : (g->nodes->buffer[r->node].closed ? "!" : "")),
                    "FR"[r->dir], r -> beg, r -> end - r->beg);
            idx = r->read_link;
        }
        fprintf(out, "\n");
    }
...
static inline u8i print_frgs_nodes_graph(Graph *g, FILE *out) {
    ...
    for(i = 0; i < g->frgs->size; i++) {
        frg = ref_frgv(g->frgs, i);
        if(frg->closed) continue;
        fprintf(out, "F%u\t%d\t%d\t%u\t%u", i, frg->length, frg->len, frg->tcnt,
                frg->ty - frg->tx);
        for(j = 0; j < frg->tcnt; j++) {
            ...
            if(j < frg->tx || j >= frg->ty) {
                ...
                if(n->rep_idx == MAX_REP_IDX) {
                    fprintf(out, "\tn%llu:%c:%d:::%d", t->node, "+-"[t->dir], t -> off,
                            t -> cov);
                } else {
                    fprintf(out, "\tn%llu:%c:%d:F%llu:%c:%d", t->node, "+-"[t->dir],
                            t -> off, (u8i)n -> rep_idx, "+-"[n->rep_dir], t -> cov);
                }
            } else {
...
static inline u8i print_frgs_dot_graph(Graph *g, FILE *_out) {
    ...
    for(i = 0; i < g->frgs->size; i++) {
        ...
        if(r2 == NULL) continue;
        fprintf(out,
                "F%llu [label=\"{F%llu %u %u/%u | { {N%llu:%c | %s | %c_%d_%d} | "
                "{N%llu:%c | %s | %c_%d_%d}}}\"]\n",
                i, i, frg->ty - frg->tx, frg->len, frg->length, t1->node, "+-"[t1->dir],
                g -> kbm -> reads -> buffer[r1->rid].tag, "FR"[r1->dir], r1 -> beg,
                r1 -> end - r1->beg, t2->node, "+-"[t2->dir],
                g -> kbm -> reads -> buffer[r2->rid].tag, "FR"[r2->dir], r2 -> beg,
                r2 -> end - r2->beg);
    }
    for(i = 0; i < g->frgs->size; i++) {
        if((i % 1000) == 0) flush_bufferedwriter(bw);
        ...
        for(k = 0; k < 2; k++) {
            ...
            while(idx) {
                ...
                if(f->flg) {
                    ...
                    fprintf(out,
                            "F%llu -> F%llu [label=\"%c%c:%d:%d\" color=%s style=%s]\n",
                            i, (u8i)e->frg1, "+-"[k], "+-"[!e->dir1], e -> cov, e -> off,
                            colors[k][!e->dir1], e->weak ? "dashed" : "solid");
                } else {
                    //if(g->frgs->buffer[e->frg2].ty - g->frgs->buffer[e->frg2].tx < (u4i)g->min_ctg_nds) continue;
                    fprintf(out,
                            "F%llu -> F%llu [label=\"%c%c:%d:%d\" color=%s style=%s]\n",
                            i, (u8i)e->frg2, "+-"[k], "+-"[e->dir2], e -> cov, e -> off,
                            colors[k][e->dir2], e->weak ? "dashed" : "solid");
                }
            }
        }
    }
