...
static inline size_t _read_data_file(void *obj, void *dat, size_t len) {
    return fread(dat, 1, len, (FILE *)obj);
}
static inline void _close_input_file(void *obj) {
    if(obj) fclose((FILE *)obj);
}
static inline void _close_input_proc(void *obj) {
    if(obj) pclose((FILE *)obj);
}
            ...
            switch(fc->file_attr) {
                ...
                default:
                    ...
                    while(fr->running) {
                        ...
                        if(fr->flag == 1) break;
                        fr->bufcnt[fr->widx] =
                            _read(_file, fr->buffer[fr->widx], fr->bufmax);
                        fr->widx = !fr->widx;
                        fr->flag = 1;
...
static inline int push_user_filereader(FileReader *fr, void *_file, read_data_func _read,
                                       close_input_func _close) {
    file_src_t *f;
    f = next_ref_filesrcv(fr->files);
    f->_file = _file;
...
static inline int asyn_readline_filereader(FileReader *fr, String *line) {
    ...
    if(fr->rollback) {
        fr->rollback = 0;
        return line->size +
               1;    // in case of end of file and not terminated by line_breaker, the return value is bigger by 1
...
static inline int directed_readline_filereader(FileReader *fr, String *line) {
    ...
    while(fr->fidx < fr->files->size) {
        ...
        if(fr->flag == 0) {
            switch(fc->file_attr) {
                case FILEREADER_ATTR_TEXT: break;
                case FILEREADER_ATTR_STDIN:
                    _file = fc->_file = stdin;
                    _read = fc->_read = _read_data_file;
                    _close = fc->_close = NULL;
...
static inline int readseq_filereader(FileReader *fr, BioSequence *seq) {
    ...
    if(fr->line->string[0] == '>') {
        ...
        if((n = readline_filereader(fr))) {
            if(seq->attr & BIOSEQ_ATTR_QLT)
                append_string(seq->qlt, fr->line->string, fr->line->size);
        } else {
            return FILEREADER_TYPE_FASTQ;
        }
        return FILEREADER_TYPE_FASTQ;
