...
#define cmp_2nums_proc(a, b) \
    if((a) < (b))            \
        ...
        return 1;
#define num_cmp_script(e1, e2, obj, val_macro)  \
    ((val_macro(e1, obj) == val_macro(e2, obj)) \
         ? 0                                    \
         : ((val_macro(e1, obj) < val_macro(e2, obj)) ? -1 : 1))
#define cmpgt_2nums_proc(a, b) \
    if((a) < (b))              \
        return 0;              \
    else if((a) > (b))         \
        return 1;
...
#define psort_array(rs_ary, rs_size, e_type, ncpu, is_a_greater_than_b)       \
    do {                                                                      \
        ...
        if(psrt->divide) {                                                    \
            divide_array(psrt->rs + psrt->beg, psrt->end - psrt->beg, e_type, \
                         is_a_greater_than_b, psrt->div);                     \
            psrt->div += psrt->beg;                                           \
        } else {                                                              \
            sort_array(psrt->rs + psrt->beg, psrt->end - psrt->beg, e_type,   \
                       is_a_greater_than_b);                                  \
        }                                                                     \
...
#define define_quick_sort(name, e_type, is_greater_func)         \
    static inline void name(e_type *rs, size_t n, void *obj) {   \
        ...
        while(x) {                                               \
            ...
            e = stack[x][1];                                     \
            m = s + (e - s) / 2;                                 \
            if(is_greater_func(rs[s], rs[m], obj) > 0) {         \
                t = rs[s];                                       \
                rs[s] = rs[m];                                   \
                rs[m] = t;                                       \
            }                                                    \
        ...
        for(i = 0; i < n; i++) {                                 \
            x = 0;                                               \
            for(j = n - 1; j > i; j--) {                         \
                if(is_greater_func(rs[j - 1], rs[j], obj) > 0) { \
                    t = rs[j - 1];                               \
                    rs[j - 1] = rs[j];                           \
                    rs[j] = t;                                   \
                    x = 1;                                       \
...
#define define_merge(name, e_type, cmp_func, output_func)                             \
    static inline void name(e_type *list1, size_t size1, e_type *list2, size_t size2, \
                            void *ref) {                                              \
        size_t i, j;                                                                  \
        i = j = 0;                                                                    \
    ...
    static inline size_t name##_files(FILE **files, int n, void *ref) {               \
        ...
        while(1) {                                                                    \
            ...
            for(i = 0; i < n; i++) {                                                  \
                if(flags[i] == 0) {                                                   \
                    flags[i] =                                                        \
                        (fread(es + i, sizeof(e_type), 1, files[i]) == 1) ? 1 : 2;    \
                }                                                                     \
                if(flags[i] == 1) {                                                   \
                    if(min == -1) {                                                   \
                        min = i;                                                      \
...
#define define_search_array(name, e_type, cmp_func)                                      \
    static inline long long name(e_type *array, long long size, e_type key, void *ref) { \
        ...
        if(i < size && cmp_func(array[i], key, ref) == 0)                                \
            return i;                                                                    \
        else                                                                             \
            return -(i + 1);                                                             \
        if(ref) return 0;                                                                \
