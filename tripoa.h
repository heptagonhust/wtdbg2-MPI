...
static inline TriPOG *init_tripog(u4i seqmax, int shuffle, int winlen, int winmin,
...
static inline void fwdbitpush_tripog(TriPOG *tp, u8i *bits, u8i off, u4i len, u2i refbeg,
                                     u2i refend) {
    if(!tp->shuffle && tp->seqs->nseq >= tp->seqmax) return;
...
static inline void revbitpush_tripog(TriPOG *tp, u8i *bits, u8i off, u4i len, u2i refbeg,
                                     u2i refend) {
    if(!tp->shuffle && tp->seqs->nseq >= tp->seqmax) return;
...
static inline void direct_run_tripog(TriPOG *tp) {
    ...
    for(ridx = 0; ridx < tp->seqs->nseq; ridx++) {
        fwdbitpush_pog_core(g, tp->seqs->rdseqs->bits, tp->seqs->rdoffs->buffer[ridx],
                            tp->seqs->rdlens->buffer[ridx], tp->rbegs->buffer[ridx],
                            tp->rends->buffer[ridx]);
    }
    end_pog(g);
    if(g->cns->size == 0) {
        fast_fwdbits2basebank(tp->cns, tp->seqs->rdseqs->bits,
                              tp->seqs->rdoffs->buffer[tp->longest_idx],
                              tp->seqs->rdlens->buffer[tp->longest_idx]);
    } else {
...
static inline void shuffle_reads_by_kmers_tripog(TriPOG *tp) {
    ...
    sort_array(kidxs->buffer, kidxs->size, u4i,
               num_cmpgt(kords->buffer[b], kords->buffer[a]));
    if(cns_debug > 1) {
        for(i = 0; i < kidxs->size; i++) {
            fprintf(stderr, "SHUFFLE[%u] %u\t%0.4f\n", i, kidxs->buffer[i],
                    kords->buffer[kidxs->buffer[i]]);
        }
    }
    for(i = 0; i < kidxs->size; i++) {
...
static inline void end_tripog(TriPOG *tp) {
    ...
    for(ridx = 1; ridx < tp->seqs->nseq; ridx++) {
        ...
        bitseq_basebank(tp->seqs->rdseqs, tp->seqs->rdoffs->buffer[0] + b, tp->winlen,
                        tp->qry->buffer);
        rlen = tp->seqs->rdlens->buffer[ridx];
        clear_and_encap_u1v(tp->ref, rlen);
        bitseq_basebank(tp->seqs->rdseqs, tp->seqs->rdoffs->buffer[ridx], rlen,
                        tp->ref->buffer);
        R = ksw_align(tp->winlen, tp->qry->buffer, rlen, tp->ref->buffer, 4, tp->matrix,
                      -(tp->pogs[0]->par->I + tp->pogs[0]->par->D) / 2, 1, KSW_XSTART,
                      NULL);
        if(R.qb <= -1 || R.tb <= -1 || R.qe <= -1 || R.te <= -1) {
            if(cns_debug > 1) {
                fprintf(stderr, "FAILED_ALIGN: READ%u [%d,%d=%d][%d,%d=%d]\n", ridx, R.qb,
                        R.qe, R.qe - R.qb, R.tb, R.te, R.te - R.tb);
            }
            if(tp->fail_skip) {
    ...
    for(ridx = 0; ridx < tp->seqs->nseq; ridx++) {
        ...
        fwdbitpush_pog(g, tp->seqs->rdseqs->bits,
                       tp->seqs->rdoffs->buffer[ridx] + tp->regs[0]->buffer[ridx],
        ...
        for(ridx = 0; ridx < tp->seqs->nseq; ridx++) {
            ...
            if(cns_debug > 1) {
                fprintf(stderr, "BREAKPOINT[%u:%u]: %u/%u\n", i, ridx,
                        tp->regs[0]->buffer[ridx], tp->seqs->rdlens->buffer[ridx]);
            }
            i++;
        }
    }
    ...
    for(ridx = 0; ridx < tp->seqs->nseq; ridx++) {
        if(tp->regs[0]->buffer[ridx] == MAX_U2) continue;
        fwdbitpush_pog(g, tp->seqs->rdseqs->bits,
                       tp->seqs->rdoffs->buffer[ridx] + tp->regs[1]->buffer[ridx],
                       tp->seqs->rdlens->buffer[ridx] - tp->regs[1]->buffer[ridx]);
    }
    if(0) {
    ...
    for(ridx = 0; ridx < tp->seqs->nseq; ridx++) {
        if(tp->regs[0]->buffer[ridx] == MAX_U2) continue;
        revbitpush_pog(g, tp->seqs->rdseqs->bits, tp->seqs->rdoffs->buffer[ridx],
                       tp->regs[0]->buffer[ridx]);
    }
    if(0) {
