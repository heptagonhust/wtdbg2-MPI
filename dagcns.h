...
static inline dagedge_t *find_edge_by_node_dagcns(DAGCNS *g, uint32_t nid1, uint32_t nid2,
                                                  int dir) {
    dagnode_t *n;
    dagedge_t *e;
...
static inline dagedge_t *add_edge_dagcns(DAGCNS *g, uint32_t nid1, uint32_t nid2,
                                         int dir) {
    dagnode_t *n;
    dagedge_t *e;
    uint32_t eid;
...
static inline dagedge_t *prepare_edge_dagcns(DAGCNS *g, uint32_t nid1, uint32_t nid2,
                                             int dir) {
    dagedge_t *e;
...
static inline void fprint_dot_bdpgraph(GEGraph *g, bdpnodev *bnodes, bdpedgev *bedges,
    ...
    for(i = 1; i < g->edges->size; i++) {
        ...
        fprintf(out, " N%llu -> N%llu [label=\"%c%c:%d:%c:%.4Lf\" color=%s]\n",
                (u8i)e->node1, (u8i)e->node2, "+-"[e->dir1], "+-"[e->dir2], e -> cov,
                "MIDX"[be->cigar], be -> prob, colors[e->dir1][e->dir2]);
        fprintf(out, " N%llu -> N%llu [label=\"%c%c:%d:%c:%.4Lf\" color=%s]\n",
                (u8i)e->node2, (u8i)e->node1, "-+"[e->dir2], "-+"[e->dir1], e -> cov,
                "MIDX"[be->cigar], be -> prob, colors[!e->dir2][!e->dir1]);
    }
    fprintf(out, "}\n");
    fclose(out);
}

static inline u4i dp_matrix2alignment_graph(DAGCNS *dag, u1i *query, u1i *z, int x, int y,
                                            GEGraph *g, bdpnodev *bnodes,
                                            bdpedgev *bedges) {
    ge_node_t *gn, *gn2;
    ge_edge_ref_t *gf;
    ge_edge_t *ge;
    ...
    for(i = 0; i < g->nodes->size; i++) {
        ...
        if(gn->edges[0].cnt || gn->edges[1].cnt) {
            cnt++;
            continue;
        }
        gn->closed = 1;
    ...
    for(i = 1; i < g->edges->size; i++) {
        ...
        be->prob = cigar_probs[be->cigar] +
                   ref_bdpnodev(bnodes, ge->node1)->probs[ge->dir1] +
                   ref_bdpnodev(bnodes, ge->node2)->probs[!ge->dir2];
...
static inline u4i branched_dynamic_programming_alignment(DAGCNS *g, u1i *query, int ql,
                                                         GEGraph *geg, bdpnodev *bnodes,
                                                         bdpedgev *bedges,
                                                         u1v *mem_buffer) {
    u4i nbeg;
    int *rh, *re;
    ...
    n_col = tl;
    encap_u1v(mem_buffer, kswx_roundup8x((tl + 2) * sizeof(int)) +
                              kswx_roundup8x((tl + 2) * sizeof(int)) +
                              kswx_roundup8x(((long long)ql) * n_col));
    rh = (int *)(mem_buffer->buffer + mem_buffer->size);
    re = (int *)(mem_buffer->buffer + mem_buffer->size +
                 kswx_roundup8x((tl + 2) * sizeof(int)));
    z = (mem_buffer->buffer + mem_buffer->size + kswx_roundup8x((tl + 2) * sizeof(int)) +
         kswx_roundup8x((tl + 2) * sizeof(int)));
    W = g->W;
    M = g->M;
    X = g->X;
...
static inline void bdpgraph2dagcns(DAGCNS *dg, GEGraph *gg, bdpnodev *bnodes,
                                   bdpedgev *bedges, u4i nbeg, bdplinkv *stack) {
    dagnode_t *dn, *dn2;
    dagedge_t *de;
    ge_node_t *gn, *gn2;
    ge_edge_ref_t *gf;
    ...
    while(stack->size) {
        ...
        if(dn && dn2 && dn != dn2) {
            de = prepare_edge_dagcns(dg, offset_dagnodev(dg->nodes, dn),
                                     offset_dagnodev(dg->nodes, dn2), 0);
            de->score += be->prob;
        }
        if(gn2->unvisit == 0) {
            push_bdplinkv(
                stack, (bdp_link_t){offset_genodev(gg->nodes, gn2),
                                    dn2 ? offset_dagnodev(dg->nodes, dn2) : 0xFFFFFFFFU});
        }
        geg_end_iter_edges();
    }
...
static inline void merge_nodes_core_dagcns(DAGCNS *g, uint32_t nid, u32list *stack,
                                           u32list *cache[4], int dir) {
    dagnode_t *n0, *n2, *n;
    dagedge_t *e, *e2, *e1;
    ...
    while(pop_u32list(stack, &nid)) {
        ...
        while(1) {
            ...
            if(e2->links[!dir] ==
               0xFFFFFFFFU) {    // check whether there is only one edge from n -(!dir)-> n0
        ...
        for(base = 0; base < 4; base++) {
            ...
            for(i = 0; i < cache[base]->size; i++)
                ref_dagedgev(g->edges, cache[base]->buffer[i])->visit = 1;
            ...
            for(i = 1; i < cache[base]->size; i++) {
                ...
                while(eid != 0xFFFFFFFFU) {
                    ...
                    e = prepare_edge_dagcns(g, nid1, e2->nodes[dir], dir);
                    {
                        e1 = ref_dagedgev(
                            g->edges,
                            cache[base]->buffer
                                [0]);    // memory referred by e1 may be freed in prepare_edge_dagcns
                    }
                ...
                while(eid != 0xFFFFFFFFU) {
                    e2 = ref_dagedgev(g->edges, eid);
                    remove_edge_dagcns(
                        g, eid);    // e2->links retain the same values after removing
                    eid = e2->links[dir];
                }
            }
            //n = ref_dagnodev(g->nodes, e1->nodes[dir]);
...
static inline void print_local_dot_dagcns(DAGCNS *g, uint32_t nid, int distance,
                                          FILE *out) {
    u32list *stack;
    u32hash *hash;
    dagnode_t *n, *n1, *n2;
        ...
        for(dir = 0; dir < 1; dir++) {
            ...
            while(eid != 0xFFFFFFFFU) {
                ...
                fprintf(out, "N%d_%d_%c -> N%d_%d_%c [label=\"%d:%0.6f\" color=%s]\n",
                        id1, n1->pos, "ACGT"[n1->base], id2, n2 -> pos, "ACGT"[n2->base],
                        e -> cov, e -> score, e -> visit ? "blue" : "black");
...
static inline void fprint_local_dot_dagcns(DAGCNS *g, u4i nid, int level, char *prefix,
                                           char *suffix) {
    FILE *out;
    out = open_file_for_write(prefix, suffix, 1);
...
static inline void gen_consensus_dagcns(DAGCNS *g, u32list *map) {
    ...
    queue = init_u32fifo();
    if(queue ==
       NULL) {    // un-reachable, but is used to call fprint_local_dot_dagcns in gdb Debug
    ...
    if(map && g->cns_head < g->backbone_size) {
        while(lst < g->cns_head) {
            push_u32list(map, g->cns->size);
            lst++;
        }
    }
