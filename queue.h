...
#define define_queue_core(list_type, e_type, size_type, inc_size)                      \
    ...
    typedef struct {                                                                   \
        ...
        size_type *trash;                                                              \
        size_type trash_size, trash_cap;                                               \
        size_type off;                                                                 \
        size_type cap;                                                                 \
    } list_type;                                                                       \
                                                                                       \
    static inline size_t list_type##_obj_desc_cnt(void *list, int idx) {               \
        if(idx == 0)                                                                   \
            ...
            return ((list_type *)list)->size * sizeof(size_type);                      \
        else if(idx == 2)                                                              \
            return ((list_type *)list)->trash_size * sizeof(size_type);                \
        else                                                                           \
            return 1;                                                                  \
    }                                                                                  \
                                                                                       \
    ...
    static inline void push_##list_type(list_type *list, e_type e) {                   \
        ...
        if(list->trash_size) {                                                         \
            ...
            encap_list((void **)&(list->buffer), sizeof(e_type), list->off, list->cap, \
                       1, 0, 0);                                                       \
            list->cap = encap_list((void **)&(list->links), sizeof(size_type),         \
    ...
    static inline int pop_##list_type(list_type *list, e_type *e) {                    \
        ...
        *e = list->buffer[list->head];                                                 \
        list->trash_cap = encap_list((void **)&list->trash, sizeof(size_type),         \
