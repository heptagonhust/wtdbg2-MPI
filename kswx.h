...
static inline void kswx_push_cigars(u32list *cigars, uint32_t *cigar, size_t size) {
    ...
    if(cigars->size && (cigars->buffer[cigars->size - 1] & 0x0F) == (cigar[0] & 0x0FU)) {
        ...
        append_array_u32list(cigars, cigar, size);
}
...
static inline void kswx_overlap_align_core(kswx_t *xs[2], u32list *cigars[2], int qlen,
                                           uint8_t *query, int tlen, uint8_t *target,
                                           int strand, int M, int X, int I, int D, int E,
                                           u8list *mem_cache) {
    kswx_t x;
    int *rh, *re;
    uint8_t *z, *zi, d;
    ...
    tl = tlen;
    n_col = tl;
    encap_u8list(mem_cache, kswx_roundup8x((tl + 2) * sizeof(int)) +
                                kswx_roundup8x((tl + 2) * sizeof(int)) +
                                kswx_roundup8x(((long long)ql) * n_col));
    rh = (int *)(mem_cache->buffer + mem_cache->size);
    re = (int *)(mem_cache->buffer + mem_cache->size +
                 kswx_roundup8x((tl + 2) * sizeof(int)));
    z = (mem_cache->buffer + mem_cache->size + kswx_roundup8x((tl + 2) * sizeof(int)) +
         kswx_roundup8x((tl + 2) * sizeof(int)));
...
static inline kswx_t kswx_extend_align_shift_core(int qlen, uint8_t *query, int tlen,
                                                  uint8_t *target, int strand,
                                                  int init_score, int W, int M, int X,
                                                  int I, int D, int E, int T,
                                                  u8list *mem_cache, u32list *cigars) {
    kswx_t x;
    int *rh, *re, *zb;
    uint8_t *z, *zi;
    int8_t *qp, *qpp;
    ...
    encap_u8list(mem_cache, kswx_roundup8x(mem_cache->size) - mem_cache->size +
                                ...
                                kswx_roundup8x(((long long)ql) * n_col) +
                                kswx_roundup8x(tl + 2) * sizeof(int8_t) *
                                    4);    // assume only A,C,G,T
    rh = (int *)(mem_cache->buffer + kswx_roundup8x(mem_cache->size));
    re = (int *)(mem_cache->buffer + kswx_roundup8x(mem_cache->size) +
                 kswx_roundup8x((tl + 2) * sizeof(int)));
    zb = (int *)(mem_cache->buffer + kswx_roundup8x(mem_cache->size) +
                 kswx_roundup8x((tl + 2) * sizeof(int)) * 2);
    z = (mem_cache->buffer + kswx_roundup8x(mem_cache->size) +
         kswx_roundup8x((tl + 2) * sizeof(int)) * 2 +
         kswx_roundup8x((ql + 2) * sizeof(int)));
    qp = (int8_t *)(mem_cache->buffer + kswx_roundup8x(mem_cache->size) +
                    kswx_roundup8x((tl + 2) * sizeof(int)) * 2 +
                    kswx_roundup8x((ql + 2) * sizeof(int)) +
                    kswx_roundup8x(((long long)ql) * n_col));
    for(c = i = 0; c < 4; c++) {
...
static inline kswx_t kswx_extend_align_core(int qlen, uint8_t *query, int tlen,
                                            uint8_t *target, int strand, int init_score,
                                            int W, int M, int X, int I, int D, int E,
                                            int T, u8list *mem_cache, u32list *cigars) {
    kswx_t x;
    int *rh, *re;
    uint8_t *z, *zi, d;
    int ql, tl, n_col;
...
static inline kswx_t kswx_mismatch_free_extend_align_core(
    int qlen, uint8_t *query, int tlen, uint8_t *target, int strand, int init_score,
    int W, int M, int I, int D, int E, int T, u8list *mem_cache, u32list *cigars) {
    kswx_t x;
    int *rh, *re;
    uint8_t *z, *zi, d;
    int ql, tl, n_col;
...
static inline kswx_t kswx_extend_align(int qlen, uint8_t *query, int tlen,
                                       uint8_t *target, int strand, int init_score, int W,
                                       int M, int X, int I, int D, int E, int T,
    ...
    x = kswx_extend_align_shift_core(qlen, query, tlen, target, strand, init_score, W, M,
                                     X, I, D, E, T, caches, cigars);
    free_u8list(caches);
    *n_cigar = cigars->size;
    *cigar = cigars->buffer;
    free(cigars);
...
static inline kswx_t kswx_flexible_banded_affine_alignment(
    uint8_t *query, int ql, uint8_t *target, int tl, int M, int X, int I, int D, int E,
    int T, int is_local, int *bands[2], u8list *mem, u32list *cigars) {
    kswx_t x;
    int *rh, *re;
    uint8_t *z, *zi;
    long long tot_z;
...
static inline kswx_t kswx_refine_alignment(uint8_t *query, int qb, uint8_t *target,
                                           int tb, int W, int M, int X, int I, int D,
                                           int E, u32list *cigars, u8list *mem_cache,
                                           u32list *cigars2) {
    kswx_t y;
    int *rh, *re;
    int *zb, *ze, *zw;
...
static inline kswx_t kswx_refine_alignment_5q(uint8_t *query, int qb, uint8_t *target,
                                              int tb, int W, uint8_t QCLP, uint8_t QMIS,
                                              uint8_t QDEL, uint8_t *qvs[7],
                                              u32list *cigars, u8list *mem_cache,
                                              u32list *cigars2) {
    kswx_t y;
    ...
    for(i = 0; i < ql; i++) {
        ...
        qins = qvs[2][i + qb];
        if(i + 1 == ql)
            qdel[0] = qdel[1] = qdel[2] = qdel[3] = QCLP;
        else {
            for(j = 0; j < 4; j++) {
                if(j == qvs[6][i + 1 + qb]) {
                    qdel[j] = qvs[3][i + 1 + qb];
...
static inline kswx_t kswx_refine_affine_alignment_5q(
    uint8_t *query, int qb, uint8_t *target, int tb, int W, uint8_t QCLP, uint8_t QMIS,
    uint8_t QDEL, uint8_t QEXT, uint8_t *qvs[7], u32list *cigars, u8list *mem_cache,
    u32list *cigars2) {
    kswx_t y;
...
static inline void kswx_cigar2string(String *str, int n, uint32_t *cigar) {
    ...
    for(i = 0; i < n; i++) {
        ...
        if(op > 2) {
            fprintf(stderr,
                    " -- CIGAR only support M(0),I(1),D(2) cigar, but met ?(%d) in %s -- "
                    "%s:%d --\n",
...
static inline void kswx_cigar2pairwise(String *alns[2], uint8_t *seqs[2], int n,
                                       uint32_t *cigar) {
    char *p1, *p2;
    ...
    for(i = 0; i < n; i++) {
        ...
        switch(op) {
            ...
            default:
                fprintf(stderr,
                        " -- CIGAR only support M(0),I(1),D(2) cigar, but met ?(%d) in "
                        "%s -- %s:%d --\n",
                        op, __FUNCTION__, __FILE__, __LINE__);
                exit(1);
        }
...
static inline void kswx_cigar2zigar(String *str, int n, uint32_t *cigar) {
    ...
    for(i = 0; i <= n; i++) {
        if(i < n) {
            ...
            if(op > 2) {
                fprintf(stderr,
                        " -- ZIGAR only support M(0),I(1),D(2) cigar, but met ?(%d) in "
                        "%s -- %s:%d --\n",
                        op, __FUNCTION__, __FILE__, __LINE__);
                exit(1);
            }
        ...
        if(*p) {
            switch(*p) {
                ...
                default:
                    op = 3;
                    fprintf(stderr,
                            " -- ZIGAR only support M(0),I(1),D(2) cigar, but met %c(?) "
                            "in %s -- %s:%d --\n",
...
static inline kswr_t kswx_extend_core(int qlen, uint8_t *query, int tlen, uint8_t *target,
                                      kswr_t r, int m, const int8_t *matrix, int w, int I,
                                      int D, int E, int T) {
    ...
    if(T < 0) {
        ...
        do {
            ...
            if(r.tb >= r.qb) {
                ...
                y2 = r.qb;
                revseq_bytes(target + r.tb - y1, y1);
                revseq_bytes(query + r.qb - y2, y2);
                score = ksw_extend2(y2, query + r.qb - y2, y1, target + r.tb - y1, m,
                                    matrix, -D, -E, -I, -E, w, -T, -1, r.score, &x2, &x1,
                                    &x3, &gscore, &max_off);
                revseq_bytes(target + r.tb - y1, y1);
                revseq_bytes(query + r.qb - y2, y2);
                ...
                revseq_bytes(target + r.tb - y1, y1);
                revseq_bytes(query + r.qb - y2, y2);
                score = ksw_extend2(y1, target + r.tb - y1, y2, query + r.qb - y2, m,
                                    matrix, -I, -E, -D, -E, w, -T, -1, r.score, &x1, &x2,
                                    &x3, &gscore, &max_off);
                revseq_bytes(target + r.tb - y1, y1);
                revseq_bytes(query + r.qb - y2, y2);
                if(gscore <= 0 || gscore <= score + T) {
        ...
        do {
            ...
            if(tlen - r.te >= qlen - r.qe) {
                ...
                y2 = qlen - r.qe;
                score = ksw_extend2(y2, query + r.qe, y1, target + r.te, m, matrix, -D,
                                    -E, -I, -E, w, -T, -1, r.score, &x2, &x1, &x3,
                                    &gscore, &max_off);
                if(gscore <= 0 || gscore <= score + T) {
                    r.te += x1;
                    r.qe += x2;
                ...
                score = ksw_extend2(y1, target + r.te, y2, query + r.qe, m, matrix, -I,
                                    -E, -D, -E, w, -T, -1, r.score, &x1, &x2, &x3,
                                    &gscore, &max_off);
                if(gscore <= 0 || gscore <= score + T) {
                    r.te += x1;
                    r.qe += x2;
...
static inline kswx_t kswx_gen_cigar_core2(int qlen, uint8_t *query, int tlen,
                                          uint8_t *target, kswr_t r, int m,
                                          const int8_t *matrix, int w, int I, int D,
                                          int E, int *_n_cigar, uint32_t **_cigar) {
    kswx_t x;
    uint32_t *cigar;
    ...
    if(w >= 0) {
        ...
        w2 = (((r.te - r.tb < r.qe - r.qb) ? r.te - r.tb : r.qe - r.qb) * matrix[0] -
              r.score) /
                 (-E) +
             1;
        ...
        w = -w;
    x.score = ksw_global2(r.qe - r.qb, query + r.qb, r.te - r.tb, target + r.tb, m,
                          matrix, -D, -E, -I, -E, w, &n_cigar, &cigar);
    x.tb = r.tb;
    x.te = r.te;
    x.qb = r.qb;
    x.qe = r.qe;
    ...
    for(i = x1 = x2 = 0; i < n_cigar; i++) {
        ...
        switch(op) {
            case 0:
                for(j = 0; j < len; j++) {
                    if(query[r.qb + x1 + j] == target[r.tb + x2 + j])
                        x.mat++;
                    else
                        x.mis++;
                }
...
static inline kswx_t kswx_gen_cigar_core(int qlen, uint8_t *query, int tlen,
                                         uint8_t *target, kswr_t r, int m,
                                         const int8_t *matrix, int w, int I, int D,
                                         int E) {
    kswx_t x;
    ...
    n_cigar = 0;
    cigar = NULL;
    x = kswx_gen_cigar_core2(qlen, query, tlen, target, r, m, matrix, w, I, D, E,
                             &n_cigar, &cigar);
    if(cigar) free(cigar);
    return x;
}

static inline kswx_t kswx_align(int qlen, uint8_t *query, int tlen, uint8_t *target,
                                int m, const int8_t *matrix, int w, int I, int D, int E,
                                int T) {
    kswr_t r;
    r = ksw_align2(qlen, query, tlen, target, m, matrix, -D, -E, -I, -E, KSW_XSTART,
                   NULL);
...
static inline kswr_t kswx_align_no_stat(int qlen, uint8_t *query, int tlen,
                                        uint8_t *target, int m, const int8_t *matrix,
                                        int w, int I, int D, int E, int T) {
    kswr_t r;
    r = ksw_align2(qlen, query, tlen, target, m, matrix, -D, -E, -I, -E, KSW_XSTART,
                   NULL);
...
static inline kswx_t kswx_align_with_cigar(int qlen, uint8_t *query, int tlen,
                                           uint8_t *target, int m, const int8_t *matrix,
                                           int w, int I, int D, int E, int T,
                                           int *n_cigar, uint32_t **cigar) {
    kswr_t r;
    ...
    r.te++;
    r = kswx_extend_core(qlen, query, tlen, target, r, m, matrix, w, I, D, E, T);
    return kswx_gen_cigar_core2(qlen, query, tlen, target, r, m, matrix, w, I, D, E,
                                n_cigar, cigar);
}

// qb, qe, tb, te are the positions of seed match region, will be extended to whole reads
static inline kswx_t kswx_fast_align(int qlen, uint8_t *query, int qb, int qe, int tlen,
                                     uint8_t *target, int tb, int te, int W, int M, int X,
                                     int I, int D, int E, int T, float min_sm,
                                     String *cigar_str) {
    kswr_t r;
    kswx_t x, x1;
    ...
    r = ksw_align2(qe - qb, query + qb, te - tb, target + tb, m, matrix, -D, -E, -I, -E,
                   KSW_XSTART, NULL);
    if(r.qb <= -1 || r.tb <= -1 || r.qe <= -1 || r.te <= -1) return KSWX_NULL;
    r.qe++;
    r.te++;
    ...
    w = (r.qe - r.qb < r.te - r.tb) ? ((r.te - r.tb) - (r.qe - r.qb))
                                    : ((r.qe - r.qb) - (r.te - r.tb));
    if(w < W) w = W;
    if(cigar_str)
        x = kswx_gen_cigar_core2(qe - qb, query + qb, te - tb, target + tb, r, m, matrix,
                                 w, I, D, E, n_cigar + 1, cigar + 1);
    else
        x = kswx_gen_cigar_core2(qe - qb, query + qb, te - tb, target + tb, r, m, matrix,
                                 w, I, D, E, NULL, NULL);
    ...
    x.te += tb;
    if(cigar_str)
        x1 = kswx_extend_align(qlen - x.qe, query + x.qe, tlen - x.te, target + x.te, 1,
                               x.score, W, M, X, I, D, E, T, n_cigar + 2, cigar + 2);
    else
        x1 = kswx_extend_align(qlen - x.qe, query + x.qe, tlen - x.te, target + x.te, 1,
                               x.score, W, M, X, I, D, E, T, NULL, NULL);
    x.score = x1.score;
    x.aln += x1.aln;
    x.mat += x1.mat;
    x.mis += x1.mis;
    ...
    x.te += x1.te;
    if(cigar_str)
        x1 = kswx_extend_align(x.qb, query + x.qb - 1, x.tb, target + x.tb - 1, -1,
                               x.score, W, M, X, I, D, E, T, n_cigar + 0, cigar + 0);
    else
        x1 = kswx_extend_align(x.qb, query + x.qb - 1, x.tb, target + x.tb - 1, -1,
                               x.score, W, M, X, I, D, E, T, NULL, NULL);
    x.score = x1.score;
