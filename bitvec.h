...
static inline size_t bitvec_obj_desc_cnt(void *bitv, int idx) {
    switch(idx) {
        ...
        case 1:
            return ((BitVec *)bitv)->sums ? (((BitVec *)bitv)->sum_size * 2 + 1) * 8 : 0;
        case 2: return ((BitVec *)bitv)->hash ? (((BitVec *)bitv)->hash_size) * 8 : 0;
        default: return 0;
    }
}

static const obj_desc_t bitvec_obj_desc = {
    "bitvec_obj_desc",
    sizeof(BitVec),
    3,
    {1, 1, 1},
    {offsetof(BitVec, bits), offsetof(BitVec, sums), offsetof(BitVec, hash)},
    {(obj_desc_t *)&OBJ_DESC_DATA, (obj_desc_t *)&OBJ_DESC_DATA,
...
static inline void zeros_bitvec(BitVec *bitv) {
    memset(bitv->bits, 0, bitv->n_cap / 8);
}
...
static inline void ones_bitvec(BitVec *bitv) {
    memset(bitv->bits, 0xFFU, bitv->n_cap / 8);
}
...
static inline void flip_bitvec(BitVec *bitv, u8i idx) {
    bitv->bits[idx >> 6] ^= 1LLU << (idx & 0x3FU);
}

static inline void one_bitvec(BitVec *bitv, u8i idx) {
    bitv->bits[idx >> 6] |= 1LLU << (idx & 0x3FU);
}

static inline void zero_bitvec(BitVec *bitv, u8i idx) {
    bitv->bits[idx >> 6] &= ~(1LLU << (idx & 0x3FU));
}
...
static inline u8i get_bitvec(BitVec *bitv, u8i idx) {
    return (bitv->bits[idx >> 6] >> (idx & 0x3FU)) & 0x01LLU;
}
...
static inline void one2bitvec(BitVec *bitv) {
    encap_bitvec(bitv, 1);
    one_bitvec(bitv, bitv->n_bit);
    bitv->n_bit++;
}

static inline void zero2bitvec(BitVec *bitv) {
    encap_bitvec(bitv, 1);
    zero_bitvec(bitv, bitv->n_bit);
    bitv->n_bit++;
}

static inline u8i get_2bitvec(BitVec *bitv, u8i idx) {
    return (bitv->bits[idx >> 5] >> ((idx & 0x1FU) << 1)) & 0x03LLU;
}

static inline void set_2bitvec(BitVec *bitv, u8i idx, u8i v) {
    bitv->bits[idx >> 5] = (bitv->bits[idx >> 5] & (~(0x03LLU << ((idx & 0x1FU) << 1)))) |
                           ((v & 0x03LLU) << ((idx & 0x1FU) << 1));
}
...
static inline void end_bitvec(BitVec *bitv) {
    encap_bitvec(bitv, 1);
    one_bitvec(bitv, bitv->n_bit);
}
...
static inline void begin_iter_bitvec(BitVec *bitv) {
    bitv->iter_idx = -1;
}
