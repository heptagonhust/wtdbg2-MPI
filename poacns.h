...
static const POGPar DEFAULT_POG_PAR = (POGPar){
    0, 0, POG_ALNMODE_OVERLAP, 0, 1, 0, 0, 96, 16, 8, 2, -5, -2, -4, -1, -3, 20, 3, 0.5};
...
static inline void push_pog(POG *g, char *seq, u1i len) {
    push_pog_core(g, seq, len, 0, 0);
}

static inline void fwdbitpush_pog_core(POG *g, u8i *bits, u8i off, u4i len, u2i refbeg,
                                       u2i refend) {
    if(g->seqs->nseq < POG_RDCNT_MAX) {
        len = num_min(len, POG_RDLEN_MAX);
...
static inline void fwdbitpush_pog(POG *g, u8i *bits, u8i off, u4i len) {
    fwdbitpush_pog_core(g, bits, off, len, 0, 0);
}

static inline void revbitpush_pog_core(POG *g, u8i *bits, u8i off, u4i len, u2i refbeg,
                                       u2i refend) {
    if(g->seqs->nseq < POG_RDCNT_MAX) {
        len = num_min(len, POG_RDLEN_MAX);
...
static inline void revbitpush_pog(POG *g, u8i *bits, u8i off, u4i len) {
    revbitpush_pog_core(g, bits, off, len, 0, 0);
}
...
static inline void print_seqs_pog(POG *g, char *prefix, char *suffix) {
    ...
    for(i = 0; i < g->seqs->nseq; i++) {
        ...
        println_fwdseq_basebank(g->seqs->rdseqs, g->seqs->rdoffs->buffer[i],
                                g->seqs->rdlens->buffer[i], out);
    }
    fclose(out);
}
...
static inline pog_edge_t *add_edge_pog(POG *g, pog_node_t *u, pog_node_t *v, int cov,
                                       int is_aux) {
    pog_edge_t *e, *f, *p;
    u4i eidx;
#if DEBUG
...
// OVERLAP
static inline void sse_band_row_rdaln_pog(POG *g, u4i nidx1, u4i nidx2, u4i seqlen,
...
static inline u2i get_rdbase_pog(POG *g, u4i rid, u4i pos) {
    ...
    if(pos >= POG_BASEWIDTH) {
        return sub32seqbits(g->seqs->rdseqs->bits, seqoff + pos + 1 - POG_BASEWIDTH) >>
               (64 - (POG_BASEWIDTH << 1));
    } else {
...
static inline int _alignment2graph_pog(POG *g, u4i rid, int xb, int xe) {
        ...
        for(i = seqlen - 1; (int)i > x; i--) {
            if(my_print) {
                fprintf(stderr, "BT[%u] y=N%u_R%u_%u_%c x=%d:%c z=%d\n", rid, nidx,
                        v->rid, v->pos, bit_base_table[(v->base) & 0x03], i,
                        bit_base_table[get_basebank(g->seqs->rdseqs, seqoff + i)], -1);
                fflush(stderr);
            }
    ...
    while(1) {
        ...
        if(my_print) {
            ...
            fprintf(
                stderr, "BT[%u] y=N%u_R%u_%u_%c x=%d:%c z=%d\n", rid, nidx, w->rid,
                w->pos, bit_base_table[(w->base) & 0x03], x,
                x >= 0 ? bit_base_table[get_basebank(g->seqs->rdseqs, seqoff + x)] : '*',
                bt);
            fflush(stderr);
        }
        if(bt == POG_DP_BT_M) {
            if(btx) {
                ...
                xb = x;
                if(nidx == POG_HEAD_NODE || nidx + 1 >= g->nodes->size ||
                   g->nodes->buffer[nidx].rid != g->nodes->buffer[nidx + 1].rid) {
                    nidx = POG_HEAD_NODE;
                } else {
                    nidx++;
                }
...
static inline int align_rd_pog(POG *g, u2i rid) {
    ...
    seqinc = seqlex + 8;    // seqlex, max_idx, beg, end, and paddings
#if __BYTE_ORDER == 1234
    SMASK = _mm_setr_epi8(0, 2, 4, 6, 8, 10, 12, 14, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80,
                          0x80, 0x80);
#else
    // TODO: need to test in BIG_ENDIAN
    SMASK = _mm_setr_epi8(1, 3, 5, 7, 9, 11, 13, 15, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80,
                          0x80, 0x80);
#endif
    // set query prof
    ...
    while(pop_u4v(g->stack, &nidx)) {
        ...
        if(my_print) {
            fprintf(stderr, "NODEALN[%u:R%u:%u] %d-%d:%d:%d %d %d\n", nidx, u->rid,
                    u->pos, u->rbeg, u->rend, u->rmax, g->rows->buffer[u->roff + u->rmax],
                    g->btds->buffer[u->coff + u->rmax] >> 2,
                    g->btds->buffer[u->coff + u->rmax] & 0x03);
        }
        if(g->par->alnmode == POG_ALNMODE_OVERLAP) {
            if(u->rend >= seqlen &&
               g->rows->buffer[u->roff + x] > score) {    // overlap alignment
            ...
            if(0) {
                for(i = u->rbeg; i < u->rend; i++) {
                    g->btvs->buffer[u->voff + i - u->rbeg] =
                        (u1i)g->btds->buffer[u->coff + i];
                }
            } else {
                memcpy(g->btvs->buffer + u->voff, g->btds->buffer + u->coff + u->rbeg,
                       (u->rend - u->rbeg) * sizeof(u2i));
            }
        }
    }
    ...
    if(cns_debug > 1) {
        fprintf(stderr, "ALIGN[%03d] len=%u ref=%d,%d aligned=%d,%d score=%d\n", rid,
                g->seqs->rdlens->buffer[rid], g->sbegs->buffer[rid],
                g->sends->buffer[rid], xb, xe + 1, score);
    }
    return score;
}
            ...
            if(Int(i) > winl) {
                if(winy >= Int(0.9 * winl)) {
                    ...
                    if(beg >= 0) {
                        ...
                        if(cns_debug > 1) {
                            fprintf(stderr,
                                    " -- remove low quality fragment %d - %d: ", beg,
                                    end);
                            for(j = beg; j < end; j++) {
    ...
    for(i = 0; i < g->msa_len; i++) {
        ...
        if(s[i] == 4) {
            ...
            for(j = beg; j < end; j++) {
                ...
                if(Int(bcnts[s[i]][off + j] + 1) >=
                   Int(bcnts[6][off + j] - bcnts[s[i]][off + j])) {
...
static inline void realign_msa_pog(POG *g) {
    ...
    for(ridx = 0; ridx < g->seqs->nseq; ridx++) {
        ...
        memcpy(g->msa->buffer + g->msa_len * ridx,
               g->msa->buffer + g->msa_len * g->seqs->nseq, g->msa_len);
    }
}

static const float homo_merging_cmp_norm[20] = {0.95, 0.90, 0.80, 0.75, 0.70, 0.70, 0.65,
                                                0.60, 0.60, 0.55, 0.50, 0.45, 0.40, 0.30,
...
static inline void dp_call_cns_pog(POG *g) {
    ...
    for(col = 0; col < g->msa_len; col++) {
        ...
        if(cns_debug > 2) {
            fprintf(stderr, "[DPCNS%03u]\t[%2d, %2d, %2d, %2d, %2d]", col, bcnts[0],
                    bcnts[1], bcnts[2], bcnts[3], bcnts[4]);
            //fprintf(stderr, "\t[%0.1f, %0.1f, %0.1f, %0.1f, %0.1f]", dp1[0], dp1[1], dp1[2], dp1[3], dp1[4]);
            fprintf(stderr, "\t[%0.1f:%d, %0.1f:%d, %0.1f:%d, %0.1f:%d, %0.1f:%d]\n",
                    dp2[0], bts[0], dp2[1], bts[1], dp2[2], bts[2], dp2[3], bts[3],
                    dp2[4], bts[4]);
        }
    }
    // Backtrace consensus sequence
...
static inline void gen_cns_pog(POG *g) {
    ...
    if(g->par->refmode) {
        rid = 0;
        {
            r = ref_u1v(g->msa, g->msa_len * rid);
            cbeg = 0;
            while(cbeg < g->msa_len && r[cbeg] == 4) {
                hcovs[cbeg] = 0;
                cbeg++;
            }
            end = g->msa_len;
            while(cend > cbeg && r[cend - 1] == 4) {
                cend--;
                hcovs[cend] = 0;
            }
        }
    ...
    for(idx = cbeg; idx <= cend; idx++) {
        if(idx < cend) {
            ...
            } else if(hcovs[idx] >= min_cnt && cnts[max] >= mcnt &&
                      cnts[4] - (g->seqs->nseq - hcovs[idx]) <
                          (g->seqs->nseq - cnts[4])) {
                s[idx] = max;
                end = idx;
                flush = 1;
        ...
        if(flush) {
            ...
            if(idx == cend || s[lst] != max) {
                ...
                if(cns_debug > 1) {
                    fprintf(stderr, "HOMO[%3u] %4u\t%c\t%u\t%u:%u\t%u:%u\t%u\t%u\t%0.2f",
                            lst, cnl, bit_base_table[s[lst]], cl, bl, bm, dl, dm, runlen,
                            hcovs[lst], fl);
                    if(cl != bl) {
                        fprintf(stderr, " *\n");
                    } else {
                ...
                if(cl != bl) {
                    if(bl > cl + 1 && bl > Int(fl) + 1)
                        bl = Int(fl) + 1;    // Don't correct too much
                    if(bl + 1 < cl && (float)bl < fl) bl = cl - 1;
                    if(bl < cl && cl == dl &&
                       dm >= Int(0.8 * bm)) {    // prefer increase runlen
...
static inline void end_pog(POG *g) {
    ...
    if(g->par->refmode) {
        ...
        for(i = 0; i < g->sbegs->size; i++) {
            if(g->sbegs->buffer[i] < margin || g->sbegs->buffer[i] + margin >= reflen)
                continue;
        ...
        for(i = 0; i < g->sends->size; i++) {
            if(g->sends->buffer[i] < margin || g->sends->buffer[i] + margin >= reflen)
                continue;
                ...
                if(ready) {
                    ...
                    do {
                        ...
                        if(xidx != POG_TAIL_NODE) {
                            set_u1v(g->msa, x->rid * g->msa_len + x->coff,
                                    (x->base) & 0x03);
                        }
                        if(x->edge) {
                            push_u4v(g->stack, xidx);
