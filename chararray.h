...
static const obj_desc_t string_obj_desc = {"string_obj_desc",
                                           sizeof(String),
                                           1,
                                           {1},
...
static inline void encap_string(String *str, string_size_t inc) {
    if(inc + str->size + 1 > str->capacity) {
        if(inc + str->size + 1 < 0xFFFFFFF) {
            ...
            str->capacity +=
                (((inc + str->size + 1) - str->capacity + 0xFFFFFFF - 1) / 0xFFFFFFF) *
                0xFFFFFFF;
        }
...
static inline void uc_string(String *str) {
    ...
    for(i = 0; i < str->size; i++) {
        if(str->string[i] >= 'a' && str->string[i] <= 'z')
            str->string[i] = str->string[i] + 'A' - 'a';
    }
}
...
static inline void clear_string(String *str) {
    str->size = 0;
    str->string[0] = 0;
}
...
static inline string_size_t split_vstring(VString *str, char separator, VStrv *vstrs,
                                          string_size_t cut) {
    VString *vstr;
    string_size_t n_tab, i, s;
    for(i = s = n_tab = 0; i < str->size; i++) {
...
static inline void free_vstring(VString *str) {
    free(str);
}
