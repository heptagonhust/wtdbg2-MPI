        ...
        if((size_t)((size) + 1) > (size_t)(cap)) {                                       \
            ...
            if((ary) == NULL) {                                                          \
                fprintf(stderr,                                                          \
                        ...
                        "--\n",                                                          \
                        (unsigned long long)(sizeof(e_type) * (cap)), __FUNCTION__,      \
                        __FILE__, __LINE__);                                             \
...
#define array_heap_pop(ary, len, cap, e_type, cmp_expr)            \
    ({                                                             \
        ...
        if(len) {                                                  \
            _ret_ = ary[0];                                        \
            array_heap_remove(ary, len, cap, e_type, 0, cmp_expr); \
        } else                                                     \
            memset(&_ret_, 0xFFU, sizeof(e_type));                 \
        _ret_;                                                     \
    })
...
#define define_list_core(list_type, e_type, size_type, inc_size)                         \
    ...
    typedef struct {                                                                     \
        ...
        size_type cap;                                                                   \
        size_type off;                                                                   \
        u4i mem_zero : 1, n_head : 31;                                                   \
    } list_type;                                                                         \
                                                                                         \
    static inline size_t list_type##_obj_desc_cnt(void *list, int idx) {                 \
        if(idx == 0)                                                                     \
            return ((list_type *)list)->size * sizeof(e_type);                           \
        else                                                                             \
            return 0;                                                                    \
    }                                                                                    \
                                                                                         \
    ...
    static const obj_desc_t list_type##_obj_desc = {                                     \
        .tag = TOSTR(_list_##list_type),                                                 \
        .size = sizeof(list_type),                                                       \
        .n_child = 1,                                                                    \
        .mem_type = {1},                                                                 \
        .addr = {offsetof(list_type, buffer)},                                           \
        .desc = {(struct obj_desc_t *)&OBJ_DESC_DATA},                                   \
        .cnt = list_type##_obj_desc_cnt,                                                 \
        .post = list_type##_obj_desc_post_load};                                         \
                                                                                         \
    static inline void adv_##list_type##_init(list_type *list, size_type init_size,      \
                                              int mem_zero, u4i n_head) {                \
        ...
        if(n_head) {                                                                     \
            ...
            list->buffer = (e_type *)calloc(list->cap, sizeof(e_type));                  \
                                                                                         ...
                                                                                         \
    static inline list_type *adv_init_##list_type(size_type init_size, int mem_zero,     \
                                                  u4i n_head) {                          \
    ...
    static inline void renew_##list_type(list_type *list, size_type init_size) {         \
        ...
        adv_##list_type##_init(list, init_size, list->mem_zero, list->n_head);           \
                                                                                         ...
                                                                                         \
    static inline size_type count_##list_type(list_type *list) {                         \
        return list->size;                                                               \
    }                                                                                    \
                                                                                         \
    static inline void clear_##list_type(list_type *list) {                              \
        list->size = 0;                                                                  \
        list->off = 0;                                                                   \
    }                                                                                    \
                                                                                         \
    static inline void zeros_##list_type(list_type *list) {                              \
        memset(list->buffer, 0, list->cap * sizeof(e_type));                             \
    }                                                                                    \
                                                                                         \
    static inline void encap_##list_type(list_type *list, size_type n) {                 \
        list->cap = encap_list((void **)&list->buffer, sizeof(e_type), list->size,       \
    ...
    static inline void encap_and_zeros_##list_type(list_type *list, size_type n) {       \
        if(((size_t)list->size) + ((size_t)n) <= (size_t)list->cap) {                    \
            ...
            if((size_t)(list->size + n) != ((size_t)list->size) + ((size_t)n)) {         \
                fprintf(                                                                 \
                    stderr,                                                              \
                    " -- elements size exceed %s's data type %s in %s -- %s:%d --\n",    \
    ...
    static inline void trunc_##list_type(list_type *list, size_type size) {              \
        ...
        list->size -= size;                                                              \
                                                                                         ...
                                                                                         \
    static inline void set_##list_type##_size(list_type *list, size_type size) {         \
        list->size = size;                                                               \
    }                                                                                    \
                                                                                         \
    static inline void inc_##list_type(list_type *list, size_type size) {                \
        ...
        list->size += size;                                                              \
                                                                                         ...
                                                                                         \
    static inline void lazy_push_##list_type(list_type *list, e_type e) {                \
    ...
    static inline void insert_##list_type(list_type *list, size_type idx, e_type e) {    \
        ...
        if(idx == list->size) {                                                          \
            ...
            memmove(list->buffer + idx + 1, list->buffer + idx,                          \
                    (list->size - idx) * sizeof(e_type));                                \
            list->buffer[idx] = e;                                                       \
        }                                                                                \
        list->size++;                                                                    \
    }                                                                                    \
                                                                                         \
    static inline void insert_array_##list_type(list_type *list, size_type idx,          \
                                                e_type *es, size_type size) {            \
        if(idx > list->size) return;                                                     \
        encap_##list_type(list, size);                                                   \
        if(idx == list->size) {                                                          \
        } else {                                                                         \
            memmove(list->buffer + idx + size, list->buffer + idx,                       \
                    (list->size - idx) * sizeof(e_type));                                \
        }                                                                                \
    ...
    static inline void remove_##list_type(list_type *list, size_type idx) {              \
        if(idx >= list->size) return;                                                    \
        if(idx + 1 < list->size) {                                                       \
            memmove(list->buffer + idx, list->buffer + idx + 1,                          \
                    (list->size - idx - 1) * sizeof(e_type));                            \
        }                                                                                \
        list->size--;                                                                    \
    }                                                                                    \
                                                                                         \
    static inline void remove_array_##list_type(list_type *list, size_type off,          \
                                                size_type len) {                         \
        if(off >= list->size) return;                                                    \
        if(off + len < list->size) {                                                     \
            memmove(list->buffer + off, list->buffer + off + len,                        \
                    (list->size - off - len) * sizeof(e_type));                          \
            list->size -= len;                                                           \
        } else {                                                                         \
            list->size = off;                                                            \
        }                                                                                \
                                                                                         ...
                                                                                         \
    static inline void set_##list_type(list_type *list, size_type idx, e_type e) {       \
        list->buffer[idx] = e;                                                           \
    }                                                                                    \
                                                                                         \
    static inline e_type get_##list_type(list_type *list, size_type idx) {               \
        return list->buffer[idx];                                                        \
    }                                                                                    \
                                                                                         \
    ...
    static inline size_type offset_##list_type(list_type *list, e_type *e) {             \
        return e - list->buffer;                                                         \
                                                                                         ...
                                                                                         \
    static inline e_type *next_ref_##list_type(list_type *list) {                        \
        encap_##list_type(list, 1);                                                      \
        list->size++;                                                                    \
        return list->buffer + list->size - 1;                                            \
    }                                                                                    \
                                                                                         \
    static inline e_type *ref_next_##list_type(list_type *list) {                        \
        list->size++;                                                                    \
        return list->buffer + list->size - 1;                                            \
    }                                                                                    \
                                                                                         \
    static inline e_type *as_array_##list_type(list_type *list) {                        \
        return list->buffer;                                                             \
    }                                                                                    \
                                                                                         \
    static inline void reverse_##list_type(list_type *list) {                            \
        ...
        while(i < j) {                                                                   \
            ...
            j--;                                                                         \
                                                                                         ...
                                                                                         \
    static inline void sub_reverse_##list_type(list_type *list, size_type beg,           \
                                               size_type end) {                          \
        size_type i, j;                                                                  \
        e_type t;                                                                        \
        if(end <= beg) return;                                                           \
    ...
    static inline void append_##list_type(list_type *list1, list_type *list2) {          \
        ...
        memcpy(list1->buffer + list1->size, list2->buffer,                               \
               sizeof(e_type) * list2->size);                                            \
        list1->size += list2->size;                                                      \
    }                                                                                    \
                                                                                         \
    static inline void append_array_##list_type(list_type *list1, e_type *ary,           \
                                                size_type size) {                        \
        if(size == 0) return;                                                            \
    ...
    static inline size_t mem_size_##list_type(list_type *list) {                         \
        return ((sizeof(list_type) + 7) / 8) * 8 +                                       \
               (list->size * sizeof(e_type) + 7) / 8 * 8;                                \
    }                                                                                    \
                                                                                         \
    static inline size_t mem_dump_##list_type(list_type *list, FILE *out, void *addr) {  \
        ...
        for(i = (sizeof(list_type) + 7) / 8 * 8 - sizeof(list_type); i > 0; i--)         \
            fwrite(&v, 1, 1, out);                                                       \
        fwrite(list->buffer, sizeof(e_type), list->size, out);                           \
        for(i = (sizeof(e_type) * list->size + 7) / 8 * 8 - sizeof(e_type) * list->size; \
            i > 0; i--)                                                                  \
            fwrite(&v, 1, 1, out);                                                       \
        return size;                                                                     \
    }                                                                                    \
    ...
    static inline void free_##list_type(list_type *list) {                               \
        if(list) {                                                                       \
            free(list->buffer - list->n_head);                                           \
            free(list);                                                                  \
        }                                                                                \
    }                                                                                    \
                                                                                         \
    static inline void list_type##_free(list_type *list) {                               \
        free(list->buffer);                                                              \
        list->buffer = NULL;                                                             \
    }
...
#define define_list_ext(list_type, e_type, size_type, cmp_func)                    \
    static inline size_type delete_##list_type(list_type *list, e_type e) {        \
        ...
        for(i = list->size; i > 0; i--) {                                          \
            if(cmp_func(list->buffer[i - 1], e, NULL) == 0) {                      \
                if(i < list->size) {                                               \
                    memmove(list->buffer + i - 1, list->buffer + i,                \
                            (list->size - i) * sizeof(e_type));                    \
                }                                                                  \
                list->size--;                                                      \
                ret++;                                                             \
            }                                                                      \
    ...
    static inline size_type occ_##list_type(list_type *list, e_type e) {           \
        ...
        return n;                                                                  \
                                                                                   ...
                                                                                   \
    static inline size_type replace_##list_type(list_type *list, e_type from,      \
                                                e_type to) {                       \
        size_type i, ret;                                                          \
        ret = 0;                                                                   \
        ...
        return ret;                                                                \
                                                                                   ...
                                                                                   \
    static inline size_type locate_##list_type(list_type *list, e_type e,          \
                                               size_type start) {                  \
        size_type i;                                                               \
    ...
    define_quick_sort(sort_##list_type##_core, e_type, cmp_func);                  \
                                                                                   \
    static inline void sort_##list_type(list_type *list) {                         \
        sort_##list_type##_core(ref_##list_type(list, 0), count_##list_type(list), \
                                NULL);                                             \
    }
...
static inline size_t cplist_deep_obj_desc_cnt(void *list, int idx) {
    if(idx == 0)
        return ((cplist *)list)->size;
    else
        return 1;
}
static const obj_desc_t cplist_deep_obj_desc = {
    .tag = "cplist_deep_dump",
    .size = sizeof(cplist),
    .n_child = 1,
    .mem_type = {3},
    .addr = {offsetof(cplist, buffer)},
    .desc = {(struct obj_desc_t *)&OBJ_DESC_CHAR_ARRAY},
    .cnt = cplist_deep_obj_desc_cnt,
    .post = NULL};
...
#define define_recycle_list_array(name, list_type)                   \
    ...
    static inline list_type *fetch_##name(name *g) {                 \
        list_type *v;                                                \
        if(g->dustbin->size)                                         \
            v = (list_type *)g->dustbin->buffer[--g->dustbin->size]; \
        else                                                         \
            v = init_##list_type(4);                                 \
        return v;                                                    \
    }                                                                \
...
#define define_recycle_list(list_type, e_type, size_type, e_init, e_free)              \
    ...
    static inline void recyc_##list_type(list_type *list, size_type idx) {             \
        list->rcap = encap_list((void **)&list->recyc, sizeof(size_type), list->rsize, \
                                list->rcap, 1, 0, 0);                                  \
