...
int usage() {
    printf(
        ...
        " -d <string> Reference sequences for SAM input, will invoke sorted-SAM input "
        "mode\n"
        ...
        " -f          Force overwrite\n"
        " -j <int>    Expected max length of node, or say the overlap length of two "
        "adjacent units in layout file, [1500] bp\n"
        ...
        " -B <int>    Bandwidth, [96]\n"
        " -W <int>    Window size in the middle of the first read for fast align "
        "remaining reads, [200]\n"
        "             If $W is negative, will disable fast align, but use the abs($W) as "
...
int main(int argc, char **argv) {
    ...
    while((c = getopt(argc, argv,
                      "hvVt:d:rp:ui:o:fj:S:B:W:w:Ab:M:X:I:D:E:H:R:c:C:F:N:x:")) != -1) {
        switch(c) {
            case 'h': return usage();
    ...
    if(dbfs->size == 0) {
        ...
        cc = init_ctgcns(wb, iter_wtlayblock, info_wtlayblock, ncpu, shuffle, seqmax,
                         winlen, winmin, fail_skip, reglen, &par);
        ...
        if(reglen > wsize || 2 * reglen < wsize) {
            fprintf(stderr,
                    " -- SAM Input mode: -w wsize(%d), -j reglen(%d), MUST has reglen <= "
                    "wsize and 2 * reglen >= wsize in %s -- %s:%d --\n",
                    wsize, reglen, __FUNCTION__, __FILE__, __LINE__);
            fflush(stderr);
            return 1;
        }
        refs = init_seqbank();
        ...
        while(readseq_filereader(db, seq)) {
            push_seqbank(refs, seq->tag->string, seq->tag->size, seq->seq->string,
                         seq->seq->size);
        }
        free_biosequence(seq);
        close_filereader(db);
        sb = init_samblock(refs, fr, wsize, reglen, sam_present);
        cc = init_ctgcns(sb, iter_samblock, info_samblock, ncpu, shuffle, seqmax, 0, 0,
                         fail_skip, UInt((wsize - reglen) * 1.2 + 100), &par);
        cc->print_progress = 100;
        if(print_lay) {
            print_lays_ctgcns(cc, out);
        } else {
