...
typedef struct {
    uint64_t off : 40, len : 23, solid : 1;
} blk_t;
...
static inline DBG *init_dbg(uint32_t ksize) {
    ...
    if(ksize > DBGCNS_KMER_MAX_SIZE) {
        fprintf(stderr,
                " -- ksize MUST be no greater than %d, but is %d in %s -- %s:%d --\n",
                DBGCNS_KMER_MAX_SIZE, ksize, __FUNCTION__, __FILE__, __LINE__);
        fflush(stderr);
        exit(1);
    }
...
static inline int add_kmer_dbg(DBG *g, u8i kmer, u4i ridx, uint8_t fbase, uint8_t rbase,
                               int inc) {
    dbgcns_kmer_t *k;
    u8i krev;
    uint32_t *u;
    ...
    if(k->visit != ridx + 1 && k->cov < DBGCNS_KMER_MAX_NODE_COV &&
       (inc || k->cov == 0)) {
        k->cov++;
    }
    k->visit = ridx + 1;
    if(dir) {
        if(fbase < 4) {
            fbase = (~fbase) & 0x03;
            if(k->edges[1][fbase] < DBGCNS_KMER_MAX_EDGE_COV &&
               (inc || k->edges[1][fbase] == 0))
                k->edges[1][fbase]++;
        }
        if(rbase < 4) {
            if(k->edges[0][rbase] < DBGCNS_KMER_MAX_EDGE_COV &&
               (inc || k->edges[0][rbase] == 0))
                k->edges[0][rbase]++;
        }
    } else {
        if(fbase < 4) {
            if(k->edges[0][fbase] < DBGCNS_KMER_MAX_EDGE_COV &&
               (inc || k->edges[0][fbase] == 0))
                k->edges[0][fbase]++;
        }
        if(rbase < 4) {
            rbase = (~rbase) & 0x03;
            if(k->edges[1][rbase] < DBGCNS_KMER_MAX_EDGE_COV &&
               (inc || k->edges[1][rbase] == 0))
                k->edges[1][rbase]++;
        }
    }
    return exists;
...
static inline void add_seq_dbg(DBG *g, u4i ridx, uint8_t *seq, uint32_t len,
                               int cov_inc) {
    u8i kmer;
    uint32_t i;
    uint8_t b, f, r;
...
static inline void print_kmers_dbg(DBG *g, FILE *out) {
    ...
    for(i = 1; i < g->kmers->size; i++) {
        ...
        fprintf(out, "%s\t%d\t%d,%d,%d,%d\t%d,%d,%d,%d\n", seq, k->cov, k->edges[0][0],
                k->edges[0][1], k->edges[0][2], k->edges[0][3], k->edges[1][0],
                k->edges[1][1], k->edges[1][2], k->edges[1][3]);
    }
}
...
static inline CNS *init_cns(uint32_t ksize, int Z, int W, int M, int X, int I, int D,
    ...
    memset(cns->dps->buffer, 0,
           sizeof(dbgcns_dp_t));    // no need, it is always zero-filled
...
static inline int dbg_cns_core(CNS *cns) {
    ...
    dp_idx =
        cns->heap->buffer
            [0];    //array_heap_pop(cns->heap->buffer, cns->heap->size, cns->heap->cap, uint32_t, num_cmp(cns->dps->buffer[b].score, cns->dps->buffer[a].score));
    ...
    if(dp->qpos + cns->W < cns->qtop) {
        ...
        for(i = cns->heap->size; i > 0; i--) {
            if(cns->dps->buffer[cns->heap->buffer[i - 1]].qpos + cns->W < cns->qtop) {
                array_heap_remove(
                    cns->heap->buffer, cns->heap->size, cns->heap->cap, uint32_t, i - 1,
                    num_cmp(cns->dps->buffer[b].score, cns->dps->buffer[a].score));
            }
        }
    }
    ...
    if(exists) {
        ...
        if(dp->score > dp2->score) {
            *u = dp_idx;
            ...
            array_heap_remove(
                cns->heap->buffer, cns->heap->size, cns->heap->cap, uint32_t, 0,
                num_cmp(cns->dps->buffer[b].score, cns->dps->buffer[a].score));
            return DBGCNS_CNS_CUT;
        }
    } else {
    ...
    array_heap_remove(cns->heap->buffer, cns->heap->size, cns->heap->cap, uint32_t, 0,
                      num_cmp(cns->dps->buffer[b].score, cns->dps->buffer[a].score));
    ...
    for(b = 0; b < 4; b++) {
        ...
        dp2->score = score;
        dp2->bt_idx = dp_idx;
        array_heap_push(cns->heap->buffer, cns->heap->size, cns->heap->cap, uint32_t,
                        cns->dps->size - 1,
                        num_cmp(cns->dps->buffer[b].score, cns->dps->buffer[a].score));
        nadd++;
        // deletion
        if(dp->path != DBGCNS_PATH_I) {
            ...
            dp2->bt_idx = dp_idx;
            array_heap_push(
                cns->heap->buffer, cns->heap->size, cns->heap->cap, uint32_t,
                cns->dps->size - 1,
                num_cmp(cns->dps->buffer[b].score, cns->dps->buffer[a].score));
            nadd++;
        }
...
static inline int hierarchical_clustering_edge_links(FBG *fbg, fbg_edge_t *e,
                                                     linkgrpv *grps, double max_var) {
    fbg_link_t *lnk;
    u4i lidx, gidx;
    u4i i, b;
    double sum, avg;
    ...
    for(i = 1; i < grps->size; i++) {
        if(grps->buffer[i].len - avg > avg * max_var) {
            ...
            avg = sum / (i - b + 1);
            if(avg - grps->buffer[b].len > avg * max_var ||
               grps->buffer[i].len - avg > avg * max_var) {
                gidx++;
                sum = avg = grps->buffer[i].len;
                b = i;
            }
...
static inline int revise_edge_fbg(FBG *fbg, u4i kidx, u4i eidx, linkgrpv *grps,
                                  double max_var) {
    fbg_kmer_t *k;
    fbg_edge_t *e, *p;
    fbg_link_t *lnk;
    ...
    if(DBGCNS_DEBUG) {
        if(ng > 1) {
            fprintf(stderr, "REVISE K%d -> K%d cov = %d into %d edges --\n", k->off,
                    ref_fbgkmerh(fbg->kmers, e->node)->off, e->cov, ng);
            fflush(stderr);
        }
    }
...
static inline void build_DirectFuzzyBruijnGraph(CNS *cns, u4i ridx, u4i cov_cutoff,
                                                double max_dist_var) {
    DBG *g;
    dbgcns_kmer_t *k;
    fbg_kmer_t A, *a;
    ...
    if(cov_cutoff) {
        ...
        sort_array(cns->fbg->mats->buffer, cns->fbg->mats->size, rd_kmer_t,
                   num_cmpgtx(g->kmers->buffer[b.kidx].cov, g->kmers->buffer[a.kidx].cov,
                              a.kidx, b.kidx));
        ...
        for(i = 0; i < cns->fbg->mats->size; i++) {
            ...
            if(rk->koff - beg < 10) {
                if(g->kmers->buffer[rk->kidx].cov >
                   g->kmers->buffer[cns->fbg->mats->buffer[j].kidx].cov) {
                    j = i;
                }
                continue;
            }
    ...
    for(r = 0; r < cns->qblks->size; r++) {
        ...
        for(i = 0; i < len;) {
            ...
            push_rdkmerv(
                cns->fbg->mats,
                (rd_kmer_t){r, offset_fbgkmerh(cns->fbg->kmers, a), i - g->ksize, 0});
        }
    }
    sort_array(cns->fbg->mats->buffer, cns->fbg->mats->size, rd_kmer_t,
               num_cmpgtx(a.ridx, b.ridx, a.kidx, b.kidx));
    for(i = 1; i < cns->fbg->mats->size; i++) {
        if(cns->fbg->mats->buffer[i - 1].ridx == cns->fbg->mats->buffer[i].ridx &&
           cns->fbg->mats->buffer[i - 1].kidx == cns->fbg->mats->buffer[i].kidx) {
    ...
    sort_array(cns->fbg->mats->buffer, cns->fbg->mats->size, rd_kmer_t,
               num_cmpgtx(a.ridx, b.ridx, a.koff, b.koff));
    // add edges
    ...
    for(beg = end = 0; beg < cns->fbg->mats->size; beg = end) {
        ...
        for(; end < cns->fbg->mats->size; end++) {
            ...
            } else if(cns->fbg->mats->buffer[end].ridx == r)
                continue;
            else {
                rr = cns->fbg->mats->buffer[end].ridx;
                break;
            }
        }
    ...
    if(0) {
        ...
        while((a = ref_iter_fbgkmerh(cns->fbg->kmers))) {
            ...
            while(eidx) {
                e = ref_fbgedgev(cns->fbg->edges, eidx);
                eidx = e->next;
                revise_edge_fbg(cns->fbg, offset_fbgkmerh(cns->fbg->kmers, a),
                                offset_fbgedgev(cns->fbg->edges, e), cns->fbg->grps,
                                max_dist_var);
            }
        }
    }
        ...
        while(eidx) {
            ...
            if(e->cov >= 3 || e->key || e->select) {
                ...
                while(lidx) {
                    ...
                    fprintf(out,
                            "\tK%d -> K%d [label=\"R%04d_%d_%d(%d:%d)\" color=%s%s]\n",
                            k->off, n->off, l->ridx, l->roff, l->rlen, e->cov, e->dist,
                            l->key ? "blue" : (e->most ? "green" : "black"),
                            l->select ? " style=dashed" : "");
                    lidx = l->next;
                }
            }
            eidx = e->next;
...
static inline void DP_best_path_DirectFuzzyBruijnGraph(CNS *cns) {
    ...
    while((k = ref_iter_fbgkmerh(fbg->kmers))) {
        ...
        if(k->closed) continue;
        if(k->off < nboff) {
            nb = offset_fbgkmerh(fbg->kmers, k);
            nboff = k->off;
        }
        if(k->off >= neoff) {
            ne = offset_fbgkmerh(fbg->kmers, k);
            neoff = k->off;
        }
    }
        ...
        while(eidx) {
            ...
            if(e->key == 1) {
                ...
                if(var > max_dist_var)
                    ...
                    score += var * dist_var + (e->cov <= 1 ? alt_one : 0);
            }
            if(score > n->bt_score) {
                n->bt_score = score;
                n->bt_node = kidx;
    ...
    if(ne != nb && k->bt_edge == 0) {
        ...
        fclose(out);
        fprintf(stderr,
                " -- something wrong, nb = %d(K%04d), ne = %d(K%04d) in %s -- %s:%d --\n",
                nb, fbg->kmers->array[nb].off, ne, fbg->kmers->array[ne].off,
...
static inline int correct_struct_DirectFuzzyBruijnGraph(CNS *cns, u4i ridx) {
    ...
    if(DBGCNS_DEBUG) {
        fprintf(stderr, " -- select seq[%d] len=%d in %s -- %s:%d --\n", ridx,
                cns->qblks->buffer[ridx].len, __FUNCTION__, __FILE__, __LINE__);
        fflush(stderr);
    }
    if(kidx == 0xFFFFFFFFU) {
        append_array_u1v(fbg->starseq, cns->qseqs->buffer + cns->qblks->buffer[ridx].off,
                         cns->qblks->buffer[ridx].len);
        return 0;
    }
    chg = 0;
    off = 0;
    while(1) {
        ...
        if(off < k->off) {
            ...
            append_array_u1v(fbg->starseq,
                             cns->qseqs->buffer + cns->qblks->buffer[ridx].off + off,
                             k->off - off);
            off = k->off;
        }
        ...
        if(DBGCNS_DEBUG) {
            fprintf(stderr, "-- %d + %d bases from seq[%d], offset %d + %d -> %d\n",
                    (int)fbg->starseq->size, lnk->rlen, lnk->ridx, off, n->off - off,
                    n->off);
        }
        append_array_u1v(
            fbg->starseq,
            cns->qseqs->buffer + cns->qblks->buffer[lnk->ridx].off + lnk->roff,
            lnk->rlen);
        off = n->off;
    }
    k = NULL;
    if(off < cns->qblks->buffer[ridx].len) {
        if(DBGCNS_DEBUG) {
            fprintf(stderr, "-- %d + %d bases from seq[%d], offset %d -> %d\n",
                    (int)fbg->starseq->size, cns->qblks->buffer[ridx].len - off, ridx,
                    off, cns->qblks->buffer[ridx].len);
        }
        append_array_u1v(fbg->starseq,
                         cns->qseqs->buffer + cns->qblks->buffer[ridx].off + off,
                         cns->qblks->buffer[ridx].len - off);
    }
    return chg;
}
    ...
    for(; i + cns->g->ksize < cns->cns->size; i++) {
        if(cns->cns->buffer[i] == b) {
            ...
            if(brun >= 3 && brun + 2 <= cns->g->ksize) {
                ...
                kmer = (kmer & xmask[0]) | (((u8i)b) << (cns->g->ksize / 2 * 2 - 2)) |
                       ((kmer & xmask[1]) >> 2);
                cns->g->kmers->buffer[0].mer = kmer;
                u = get_dbgcnskmerhash(cns->g->khash, 0);
                ...
                if(kcnts[0] > kcnts[1] &&
                   kcnts[1] >= kcnts[2]) {    // there is a insertion base
                    brun--;
                    chg++;
                    if(DBGCNS_DEBUG) {
...
static inline int run_cns(CNS *cns, int candidate_mode, int corr_struct) {
    ...
    if(candidate_mode == 4) {    // longest
        for(i = 0; i < cns->qblks->size; i++) {
            ...
            if(cns->qblks->buffer[i].len > cns->qblks->buffer[cns->qidx].len)
                cns->qidx = i;
        }
        ...
        cns->qidx = quick_median_array(
            idxs->buffer, idxs->size, u2i,
            num_cmpgt(cns->qblks->buffer[a].len, cns->qblks->buffer[b].len));
    } else {
        fprintf(stderr, " -- Unknown candidate mode %d in %s -- %s:%d --\n",
                candidate_mode, __FUNCTION__, __FILE__, __LINE__);
        fflush(stderr);
    }
    ...
    if(candidate_mode == 0) {
        ...
        for(i = cns->qblks->size; i > 0; i--) {
            ...
            cov = kmer_cov_seq_dbg(cns->g,
                                   cns->qseqs->buffer + cns->qblks->buffer[i - 1].off,
                                   cns->qblks->buffer[i - 1].len, (i - 1 + 1) % 255) *
                  1.0 / cns->qblks->buffer[i - 1].len;
            if(cov > max) {
                max = cov;
                cns->qidx = i - 1;
            }
        ...
        // revise cns->avg_cov
        cns->avg_cov =
            cns->avg_cov * cns->qblks->buffer[0].len / cns->qblks->buffer[cns->qidx].len;
    }
    if(corr_struct) {
        ...
        run_core_cns(cns, cns->qseqs->buffer + cns->qblks->buffer[cns->qidx].off,
                     cns->qblks->buffer[cns->qidx].len);
    }
