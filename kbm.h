...
static const obj_desc_t kbmreadv_deep_obj_desc = {.tag = "kbmreadv_deep_obj_desc",
...
static const obj_desc_t kbm_obj_desc = {
    .tag = "kbm_obj_desc",
    .size = sizeof(KBM),
    .n_child = 10,
...
static inline void free_kbmpar(KBMPar *par) {
    free(par);
}
...
static inline void bitpush_kbm(KBM *kbm, char *tag, int taglen, u8i *seqs, u8i seqoff,
                               u4i seqlen) {
    kbm_read_t *rd;
    char *ptr;
    if(taglen) {
...
// rs[0]->n_head MUST >= 1
static inline void split_FIXP_kmers_kbm(BaseBank *rdseqs, u8i offset, u4i length,
                                        u1i ksize, u1i psize, u4i kmod, kmeroffv *rs[2]) {
    kmer_off_t *kp;
...
static inline void query_index_kbm(KBMAux *aux, char *qtag, u4i qidx, BaseBank *rdseqs,
...
static inline void print_exists_index_kbm(KBM *kbm, char *qtag, BaseBank *rdseqs,
                                          u8i seqoff, u4i seqlen, kmeroffv *kmers[2],
                                          FILE *out) {
    KBMPar *par;
    kbm_kmer_t *u;
    kbm_kaux_t *x;
...
static inline void print_hit_kbm(KBM *kbm, char *qtag, u4i qlen, kbm_map_t *hit,
...
static inline void push_kmer_match_kbm(KBMAux *aux, int dir, kbm_dpe_t *p) {
    ...
    for(i = 0; i <= dp->kms->size; i++) {
        ...
        if(e->bidx == bb && e->poff / KBM_BIN_SIZE == qb / KBM_BIN_SIZE) {
            ...
            if(KBM_LOG >= KBM_LOG_ALL) {
                fprintf(
                    KBM_LOGF, "KBMLOG%d [x=%d, y=%d(%s:%d), off=%d cnt=%d, mat=%d]\n",
                    __LINE__, qb / KBM_BIN_SIZE, bb,
                    aux->kbm->reads->buffer[aux->kbm->bins->buffer[bb].ridx].tag,
                    bb - aux->kbm->reads->buffer[aux->kbm->bins->buffer[bb].ridx].binoff,
                    c->koff, c->kcnt, c->kmat);
            }
#endif
            while(bb < e->bidx) {
...
static inline int simple_chain_all_maps_kbm(kbm_map_t *srcs, u4i size,
                                            BitsVec *src_cigars, kbm_map_t *dst,
                                            BitsVec *dst_cigars, float max_aln_var) {
    kbm_map_t *hit;
    u4i i, x, y, z, f;
    if(size < 2) return 0;
