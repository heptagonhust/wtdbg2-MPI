...
int usage(int ret) {
    fprintf(
        ...
        " It defines two TAGs in each GZIP header, ZS: block size, ZX: random access "
        "index.\n"
        " Program pgzf can decompress .pgzf and .gz files. When decompressing .gz "
        "files,\n"
...
int main(int argc, char **argv) {
    ...
    if(0 && del) {
        if(outf == NULL && overwrite == 0) {
            if(optind < argc) {
                fprintf(stderr,
                        " ** WARNNING: won't delete input files. To force delete input "
                        "files, please specify -o or/and -f\n");
            }
            del = 0;
        }
    }
            ...
            if(overwrite == 0) {
                ...
                for(c = optind; c < argc; c++) {
                    if(strcmp(outf, argv[c]) == 0) {
                        fprintf(stderr,
                                " ** ERROR: The same file in INPUT and OUTPUT, '%s'\n",
                                outf);
                        return 1;
                    }
                }
    ...
    if(rw == PGZF_MODE_R) {
        if(outf == NULL || is_dir) {
            for(c = optind; c < argc; c++) {
                if(strlen(argv[c]) < 4 ||
                   strcasecmp(argv[c] + strlen(argv[c]) - 3, ".gz")) {
                    fprintf(stderr,
                            " ** ERROR: cannot auto generate output file name for '%s'\n",
                            argv[c]);
                    return 1;
                } else if(is_dir) {
                    char *rtag;
        ...
        if(outf && !is_dir) {
            ...
            for(c = optind; c < argc; c++) {
                if(strlen(argv[c]) >= 4 &&
                   strcasecmp(argv[c] + strlen(argv[c]) - 3, ".gz") == 0) {
                    fprintf(stderr, " ** ERROR: file seems already compressed '%s'\n",
                            argv[c]);
                    return 1;
                } else if(strcmp(argv[c], "-") == 0) {
                    fprintf(stderr,
                            " ** ERROR: Please specify output file when read from STDIN "
                            "'%s'\n",
                            argv[c]);
                    return 1;
                } else if(is_dir) {
