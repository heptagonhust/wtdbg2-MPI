...
static inline void reset_kbmblock(KBMBlock *kb, char *rdtag, u4i chridx, BaseBank *rdseq,
                                  u8i rdoff, u4i rdlen, KBMAux *aux) {
    lay_seq_t *stop;
    kb->chridx = chridx;
    kb->cidx = chridx;
    kb->bidx = 0;
...
static inline lay_seq_t *_push_padding_ref_kbmblock(KBMBlock *kb, lay_seq_t *sq) {
    ...
    while(kb->chridx == sq->chridx && kb->bidx <= sq->bidx) {
        ...
        fast_fwdbits2basebank(st->seq, kb->rdseq->bits, off + st->rdoff,
                              st->rend - st->rbeg);
        array_heap_push(
            kb->heap->buffer, kb->heap->size, kb->heap->cap, u4i,
            offset_layseqr(kb->seqs, st),
            num_cmpx(ref_layseqr(kb->seqs, a)->bidx, ref_layseqr(kb->seqs, b)->bidx,
                     ref_layseqr(kb->seqs, a)->rdidx, ref_layseqr(kb->seqs, b)->rdidx));
        if(st->rend >= len) {
            kb->chridx = MAX_U4;
            kb->bidx = 0;
            break;
    ...
    do {
        ...
        while(kb->lidx == MAX_U4) {
            while(kb->hidx < kb->aux->hits->size) {
                if(kb->aux->hits->buffer[kb->hidx].mat == 0)
                    kb->hidx++;
                else
                    break;
            }
                    ...
                    if(bt & 0b010) {
                        {
                            if(sc) {
                                ...
                                if(hit->qdir) {
                                    revbits2basebank(sc->seq, kb->aux->kbm->rdseqs->bits,
                                                     tsoff + rdlen - (rdoff + len), len);
                                } else {
                                    fwdbits2basebank(sc->seq, kb->aux->kbm->rdseqs->bits,
                                                     tsoff + rdoff, len);
                                }
                                sc->rend = off;
                            }
                            if(sl) {
                                ...
                                if(hit->qdir) {
                                    revbits2basebank(sl->seq, kb->aux->kbm->rdseqs->bits,
                                                     tsoff + rdlen - (rdoff + len), len);
                                } else {
                                    fwdbits2basebank(sl->seq, kb->aux->kbm->rdseqs->bits,
                                                     tsoff + rdoff, len);
                                }
                                sl->rend = off;
                            }
                    ...
                    if(off == nxt) {
                        if(sl) {
                            if(sl->rend > sl->rbeg) {
                                ...
                                array_heap_push(
                                    kb->heap->buffer, kb->heap->size, kb->heap->cap, u4i,
                                    offset_layseqr(kb->seqs, sl),
                                    num_cmpx(ref_layseqr(kb->seqs, a)->bidx,
                                             ref_layseqr(kb->seqs, b)->bidx,
                                             ref_layseqr(kb->seqs, a)->rdidx,
                                             ref_layseqr(kb->seqs, b)->rdidx));
                            }
                            sl = NULL;
            ...
            if(sl && sl->rend > sl->rbeg) {
                ...
                array_heap_push(kb->heap->buffer, kb->heap->size, kb->heap->cap, u4i,
                                offset_layseqr(kb->seqs, sl),
                                num_cmpx(ref_layseqr(kb->seqs, a)->bidx,
                                         ref_layseqr(kb->seqs, b)->bidx,
                                         ref_layseqr(kb->seqs, a)->rdidx,
        ...
        if(kb->lidx != MAX_U4) {
            ...
            array_heap_remove(
                kb->heap->buffer, kb->heap->size, kb->heap->cap, u4i, 0,
                num_cmpx(ref_layseqr(kb->seqs, a)->bidx, ref_layseqr(kb->seqs, b)->bidx,
                         ref_layseqr(kb->seqs, a)->rdidx,
                         ref_layseqr(kb->seqs, b)->rdidx));
...
static inline int map_kbmpoa(CTGCNS *cc, KBMAux *aux, char *rdtag, u4i qidx,
                             BaseBank *rdseq, u8i seqoff, u4i seqlen, u4i corr_min,
                             u4i corr_max, float corr_cov, FILE *layf) {
    KBMBlock *kb;
    u4i i;
