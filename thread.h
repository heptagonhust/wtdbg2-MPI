...
#define thread_begin_func_core(tname)                                \
    inline void *thread_##tname##_func(void *obj) {                  \
...
#define thread_beg_func_inline(tname)                                \
    inline void *thread_##tname##_func(void *obj) {                  \
        struct tname##_struct *tname = (struct tname##_struct *)obj; \
        int tname##_var_i, tname##_var_j;                            \
        struct tname##_struct *tname##_params
...
#define thread_begin_init(tname, n_thread)                                         \
    assert(n_thread > 0);                                                          \
    tname##_params =                                                               \
...
#define thread_end_init(tname)                                                        \
    if(pthread_create(tname##_pids + tname##_i, NULL, thread_##tname##_func,          \
                      (void *)tname) != 0) {                                          \
...
#define thread_export(tname, obj)                                                       \
    thread_export_core(tname, (obj)->tname, (obj)->tname##_params, (obj)->tname##_pids, \
                       (obj)->tname##_mlock, (obj)->tname##_rwlock, (obj)->tname##_i,   \
                       (obj)->tname##_j, (obj)->tname##_var_next)
...
#define thread_import(tname, obj)                                                       \
    thread_import_core(tname, (obj)->tname, (obj)->tname##_params, (obj)->tname##_pids, \
                       (obj)->tname##_mlock, (obj)->tname##_rwlock, (obj)->tname##_i,   \
                       (obj)->tname##_j, (obj)->tname##_var_next)
...
#define thread_count_all(tname, expr) \
    ({                                \
        int ret = 0;                  \
        thread_beg_iter(tname);       \
        if((expr)) {                  \
...
#define thread_waitfor_all_idle(tname) \
    {                                  \
        thread_begin_iter(tname);      \
        thread_waitfor_idle(tname);    \
        thread_end_iter(tname);        \
    }
#define thread_wait_all(tname) thread_waitfor_all_idle(tname)

#define thread_apply_all(tname, expr)   \
    {                                   \
        thread_begin_iter(tname);       \
        (expr);                         \
        thread_wake(tname);             \
        thread_end_iter(tname);         \
        thread_waitfor_all_idle(tname); \
    }

#define thread_begin_close(tname)                                          \
    for(tname##_i = 0; tname##_i < tname##_params[0].n_cpu; tname##_i++) { \
        tname = tname##_params + tname##_i;                                \
        pthread_mutex_lock(&tname->_COND_LOCK);                            \
        tname->running = 0;                                                \
...
#define thread_run(tname, ncpu, vars_expr, init_expr, free_expr, pre_expr, loop_expr, \
                   post_expr, invoke_expr)                                            \
    {                                                                                 \
        thread_beg_def(tname);                                                        \
        vars_expr thread_end_def(tname);                                              \
        thread_begin_func_core(tname);                                                \
        pre_expr thread_beg_loop(tname);                                              \
        loop_expr thread_end_loop(tname);                                             \
        post_expr thread_end_func(tname);                                             \
        {                                                                             \
            thread_preprocess(tname);                                                 \
            thread_beg_init(tname, ncpu);                                             \
            init_expr thread_end_init(tname);                                         \
            invoke_expr thread_beg_close(tname);                                      \
            free_expr thread_end_close(tname);                                        \
        }                                                                             \
    }

#define THREAD_EXPR(...) __VA_ARGS__

#define thread_fast_run(tname, ncpu, loop_expr)                                     \
    thread_run(                                                                     \
        tname, ncpu, , , ,                                                          \
        THREAD_EXPR(int NCPU; int TIDX; NCPU = tname->n_cpu; TIDX = tname->t_idx;), \
        THREAD_EXPR(loop_expr;), ,                                                  \
        THREAD_EXPR(thread_wake_all(tname); thread_wait_all(tname);))
